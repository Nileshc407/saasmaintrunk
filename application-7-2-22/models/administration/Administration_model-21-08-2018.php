...
/********************************************************AMIT 03-05-2017 *******************/	
	...
	/**************************************BUY and Free*******************************************/
		...
		public function offer_rule_list($limit,$start,$Company_id)
		{
			$this->db->limit($limit,$start);
			$this->db->select('*');
			$this->db->order_by('Offer_id','desc');
			$this->db->from('igain_offer_master as dp');
			$this->db->join('igain_company_merchandise_catalogue as e','e.Company_merchandise_item_id = dp.Company_merchandise_item_id');
			$this->db->where('dp.Company_id',$Company_id);			
			$query = $this->db->get();
			if ($query->num_rows() > 0)
			{
		...
		public function get_offer_item_details($Company_id,$Todays_date)
		{
			...
			$this->db->where(array('Company_id'=>$Company_id,'Valid_from <= '=>$Todays_date,'Valid_till >= '=>$Todays_date,'show_item'=>1,'Active_flag'=>1,'Offer_flag' => 1));
			$query = $this->db->get();
			// echo $this->db->last_query();
			if ($query->num_rows() > 0)
			{
		...
		public function check_item_offers($Offer_name,$Company_id,$Item_id,$Buy_item,$Free_item)
		{
			$this->db->select('*');
			$this->db->from('igain_offer_master');
			$this->db->where(array('Company_id' =>$Company_id,'Company_merchandise_item_id' =>$Item_id,'Buy_item' =>$Buy_item,'Free_item' =>$Free_item));			
			$sqlqury = $this->db->get();
			// echo $this->db->last_query();
			if($sqlqury->num_rows() > 0)
			{
		...
		public function delete_offer_rule($Offer_id,$Company_id)
		{
		...
		public function edit_offer_rule($offer_id,$Company_id)
		{
			$this->db->Select("OM.Offer_id,OM.Company_id,OM.Company_merchandise_item_id,OM.Offer_name,OM.Buy_item,OM.Free_item,OM.Free_item_id,OM.From_date,OM.Till_date,OM.Active_flag,e.Merchandize_item_name,e.Company_merchandize_item_code");			
