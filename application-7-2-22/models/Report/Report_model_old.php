...
/*********** Akshay Work Start ****************************/
	...
	function get_merchant_report($start_date,$end_date,$Company_id,$seller_id,$transaction_type_id,$report_type,$login_enroll)
    {
		...
		if($report_type == 1)
		{
			...
			$fields = array(
						'Trans_type' => array('type' => 'VARCHAR','constraint' => '50'),
						'companyName' => array('type' => 'VARCHAR','constraint' => '50'),
						'sellerName' => array('type' => 'VARCHAR','constraint' => '50'),
						'top_up' => array('type' => 'VARCHAR','constraint' => '50'),
						'purchase_amt' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'reedem_pt' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'balance_to_pay' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'loyalty_pts_gain' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'seller_enrollid' => array('type' => 'INT','constraint' => '11'),
					);
		...
		if($report_type == 0)
		{
			...
			$fields = array(
						'Trans_id' => array('type' => 'INT','constraint' => '11'),
						'Trans_type' => array('type' => 'VARCHAR','constraint' => '50'),
						'Trans_type_id' => array('type' => 'INT','constraint' => '11'),
						'GiftCardNo' => array('type' => 'INT','constraint' => '11'),
						...
						'Manual_billno' => array('type' => 'VARCHAR','constraint' => '50'),
						'companyName' => array('type' => 'VARCHAR','constraint' => '50'),
						'sellerName' => array('type' => 'VARCHAR','constraint' => '50'),
						'top_up' => array('type' => 'VARCHAR','constraint' => '50'),
						'purchase_amt' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'reedem_pt' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'balance_to_pay' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'payment_type' => array('type' => 'VARCHAR','constraint' => '50'),
						'cardId' => array('type' => 'VARCHAR','constraint' => '50'),
						'Bill_no' => array('type' => 'VARCHAR','constraint' => '50'),
						'loyalty_pts_gain' => array('type' => 'DECIMAL','constraint' => '25,0'),
						'seller_enrollid' => array('type' => 'INT','constraint' => '11'),
	...
	public function get_seller_excel_report_details($temp_table,$Report_type)	//,$limit,$start
	{
		...
		{
			$this->db->order_by('sellerName,Trans_id' , 'desc');
			$this->db->select("Trans_date,sellerName as Merchant_name,CONCAT(First_name, ' ', Last_name) as Member_name,cardId as Membership_ID,Walkin_customer,Manual_billno,Trans_type,top_up as Bonus_points,purchase_amt,reedem_pt,Quantity,balance_to_pay, loyalty_pts_gain,Remarks as Remarks,seller_enrollid as Merchant_id");
			//,seller_enrollid as Merchant_id
			$query = $this->db->get($temp_table);
			if ($query->num_rows() > 0)
			{
...
/*********** Sandeep Work Start ****************************/
	...
	public function get_deposit_transactions($limit,$start,$Company_id,$start_date,$end_date,$Trans_type,$seller_id,$enrollID)
	{
		...
		$temp_table = $enrollID.'deposit_topup_rpt';		
			...
			$fields20 = array(
						'Transaction_type' => array('type' => 'VARCHAR','constraint' => '50'),
						'Company_name' => array('type' => 'VARCHAR','constraint' => '50'),
						'Merchant_name' => array('type' => 'VARCHAR','constraint' => '50'),
						'Transaction_date' => array('type' => 'DATE'),
						'Exception_transaction' => array('type' => 'VARCHAR','constraint' => '20'),
						'Topup_amount' => array('type' => 'DECIMAL','constraint' => '25,0'),
...
/*********** Sandeep Work Start ****************************/
	...
	/*
		...
		*/
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Loyalty_pts,Bill_no,TT.Trans_type_id,Seller,TT.Trans_type,IT.Card_id as Membership_ID,IT.Company_id,Card_id2 as Transfer_to,Transfer_points,TM.Tier_name,IT.Remarks as Remarks');
 ...
 {
  $this->db->select('joined_date,IE.Card_id as Membership_ID,First_name,Middle_name,Last_name,(Current_balance-Blocked_points) as Total_current_balance,User_email_id,total_purchase AS Total_Purchase_Amount,SUM(`Loyalty_pts`+`Topup_amount`+`Transfer_points`) AS Total_Gained_Points,Total_reddems as Total_Redeemed_Points,SUM(balance_to_pay) as Total_balance_to_pay,User_activated');
