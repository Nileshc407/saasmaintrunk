...
class Enroll_model extends CI_Model {
   ...
   /*************************************Akshay*******************************/
     function fastenroll($Customer_topup)
    {
	...
	{	 
		...
		{  
			$query =  $this->db->select('User_email_id')
			   ->from('igain_enrollment_master')
			->where(array('User_email_id' => $userEmailId, 'Company_id' => $Company_id))->get();
		}
		else
		{
			$query =  $this->db->select('User_email_id')
			   ->from('igain_enrollment_master')
			->where(array('User_email_id' => $userEmailId))->get();	
		}
	...
	function enrollment($filepath,$Customer_topup)
    {		
...
/***************AMIT CHANGED END********************************************************/		
   ...
   {	   
	 ...
	 public function get_total_merchant($Company_id)
	{
	  ...
	  $this->db->from('igain_enrollment_master'); 
	  $this->db->where(array('User_id  ' => '2','Company_id' => $Company_id));
	  $company_sql= $this->db->get();
 ...
 //******************** Sandeep Work start **************************
  ...
  function validate_member($Company_id)
  {
