...
Class Segment_model extends CI_Model
{
   ...
   /*  if($limit != "" || $start =! "" )
	...
	public function edit_segment_code($Company_id,$SegmentCode)
	{
		$this->db->select('*');
		$this->db->from('igain_segment_master');
		$this->db->where(array('Company_id'=>$Company_id, 'Segment_code' =>$SegmentCode, 'igain_segment_master.Active_flag' =>1));
		$this->db->join('igain_segment_type_master', 'igain_segment_master.Segment_type_id = igain_segment_type_master.Segment_type_id');
        $query = $this->db->get();
		// echo $this->db->last_query();
		if($query -> num_rows() > 0)
	...
	public function Insert_segment($Company_id,$enroll)
	{
		...
		for($i=0;$i<=11;$i++)
		{
			...
			if(isset($_REQUEST["SegmentType_".$i]) !='')
			{
				...
				if($Combi_SegmentType==2 || $Combi_SegmentType==3 || $Combi_SegmentType==4 || $Combi_SegmentType==5 || $Combi_SegmentType==6 || $Combi_SegmentType==7 )//Sex,Country,State,City,Zipcode
				{
	...
	public function delete_segment_child($Company_id,$Segment_code)
	{
