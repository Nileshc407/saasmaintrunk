...
//*****stamp flag item offers ***********
	...
	function get_claim_code($Company_id)
	{
		$claim_code = $this->get_code();
		$gclaim_code_exist = $this->check_claim_code($claim_code,$Company_id);
		
		if($gclaim_code_exist > 0)
		{
			$this->get_claim_code($Company_id);
		}
	...
	function check_claim_code($claim_code,$Company_id)
	{
		$query =  $this->db->select('Trans_id')
				   ->from('igain_pos_transaction_temp')
				   ->where(array('Claim_code' => $claim_code, 'Company_id' => $Company_id))->get();
				   
		return $query->num_rows();
	}
	function check_unclaimed_bill_no($Bill_no,$Pos_outlet_id,$Company_id,$Bill_date_time)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($Bill_date_time)); 
		$end_date=date("Y-m-d 23:59:59", strtotime($Bill_date_time));
		
		$this->db->select('Bill_no');
		$this->db->from('igain_pos_transaction_temp');
		$this->db->where(array('Bill_no' => $Bill_no, 'Company_id' => $Company_id, 'Outlet_id' =>$Pos_outlet_id, 'Claimed_flag' => 0));
		$this->db->where('Trans_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
		$query = $this->db->get();		
		return $query->num_rows();
	}	
	function get_unclaimed_bill($Bill_no,$Pos_outlet_id,$Company_id,$Bill_date_time)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($Bill_date_time)); 
		$end_date=date("Y-m-d 23:59:59", strtotime($Bill_date_time));
	...
	function check_claimed_bill_no($Bill_no,$Pos_outlet_id,$Company_id,$Bill_date_time)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($Bill_date_time)); 
		$end_date=date("Y-m-d 23:59:59", strtotime($Bill_date_time));
		...
		$this->db->select('Bill_no');
		$this->db->from('igain_transaction');
		$this->db->where(array('Manual_billno' => $Bill_no, 'Company_id' => $Company_id, 'Seller' =>$Pos_outlet_id,'Trans_type' => 2, 'Channel_id' => 0));
		$this->db->where('Trans_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
		$query = $this->db->get();		
		return $query->num_rows();
	}	
	function get_claimed_bill($Bill_no,$Pos_outlet_id,$Company_id,$Bill_date_time)
	{
		$start_date=date("Y-m-d 00:00:00", strtotime($Bill_date_time)); 
		$end_date=date("Y-m-d 23:59:59", strtotime($Bill_date_time));
	...
	public function Insert_pos_transaction_temp($data)
	{
		$this->db->insert('igain_pos_transaction_temp', $data);		
		if($this->db->affected_rows() > 0)
		{
			return $this->db->insert_id();
		}
