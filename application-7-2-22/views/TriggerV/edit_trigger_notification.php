...
$todays = date("Y-m-d");
		...
		foreach($trigger_info as $trig)
		{
			...
			$Threshold_value2 = $trig->Threshold_value2;
			$Operator_id = $trig->Operator_id;
			$Merchant_type = $trig->Merchant_type;
		}
 ?>
			...
			<div class="element-wrapper">
			  ...
			  <?php
				...
				<?php $attributes = array('id' => 'formValidate');
					echo form_open('TriggerC/update_trigger_notification',$attributes); ?>
				<div class="col-sm-8">
				  ...
				  <div class="form-group">
					...
					<input class="form-control"  name="Triggername" id="Triggername"  type="text" placeholder="Enter Trigger Name"  required="required" data-error="Please enter trigger name" value="<?php echo $Trigger_notification_name; ?>" >
				  ...
				  <div class="form-group" id="grid-container">
					 <label for=""> <span class="required_info" style="color:red;">*</span> Criteria </label><hr>
						<div class="grid-item">
						<input type="radio" name="Criteria" id="inlineRadio1" value="1" onclick="show_hide_div(this.value);"  required="required"  <?php if($Trigger_notification_type == 1){ echo "checked"; }else{ echo "disabled";} ?> >Member Defined
						</div>	
						<div class="grid-item">
						<input type="radio" name="Criteria" id="inlineRadio3" value="3" onclick="show_hide_div(this.value);"  required="required"  <?php if($Trigger_notification_type == 3){ echo "checked"; }else{ echo "disabled";} ?>> Auction Defined
						</div>	

						<div class="grid-item">
						<input type="radio" name="Criteria" id="inlineRadio2" value="2" onclick="show_hide_div(this.value);" required="required"  <?php if($Trigger_notification_type == 2){ echo "checked"; }else{ echo "disabled";} ?>> Loyalty Rule Defined
						</div>	
						<div class="grid-item">
						<input type="radio" name="Criteria" id="inlineRadio4" value="4" onclick="show_hide_div(this.value);"  required="required"  <?php if($Trigger_notification_type == 4){ echo "checked"; }else{ echo "disabled";} ?>> Game Defined
						</div>	
						<div class="help-block form-text with-errors form-control-feedback"></div>
				   </div> 
				   <?php if($Trigger_notification_type == 1){?>
					...
					<option value="">Select Tier </option>
						...
						<?php
							...
							{
								if($Tier_id == $row->Tier_id)
								{
									echo '<option value='.$row->Tier_id.' selected >'.$row->Tier_name.'</option>';
								}
								else 	if($row->Active_flag == 1)
								{
					...
					if($Trigger_notification_type == 2){ 
					...
					<?php
						...
						{
							if($Loyalty_rule == $lp->Loyalty_name)
							{
								echo "<option value='".$lp->Loyalty_name."' selected >".$lp->Loyalty_name."</option>";
							}
							else if($todays >= $lp->From_date && $todays <= $lp->Till_date)
							{
					...
					if($Trigger_notification_type == 3){ 
					?>	
					...
					<option value="">Select Auction </option>
						...
						<?php
							...
							{
								if($Auction_id == $row2->Auction_id)
								{
									echo '<option value='.$row2->Auction_id.' selected>'.$row2->Auction_name.'</option>';
								}
								else if( ($todays >= $row2->From_date) && ($todays <= $row2->To_date) )
								{
					...
					if($Trigger_notification_type == 4){ 
					?>
					...
					<option value="">Select Competition Game </option>
						...
						$todays = date("Y-m-d");
							...
							{
								if($Company_game_id == $row3->Company_game_id)
								{
									echo '<option value='.$row3->Company_game_id.' selected >'.$row3->Game_name.'</option>';
								}
								else if( ($todays >= $row3->Competition_start_date) && ($todays <= $row3->Competition_end_date) )
								{
					...
					<?php } 
					if($Trigger_notification_type == 1 )
					{ 
					
					?>	
					<div class="form-group" id="Threshold_block">
					<label for=""> <span class="required_info">*</span> Threshold Value </label>								
					<input class="form-control"  name="Thresholdvalue" id="Thresholdvalue"  type="text" onblur="check_threshold_value_with_criteria(this.value);" value="<?php echo $Threshold_value; ?>" data-error="Please enter threshold value" onkeyup="this.value=this.value.replace(/\D/g,'')">
				  ...
				  <input type="hidden" name="triggerid" value="<?php echo $Trigger_notification_id; ?> ">
...
var triggerName = '<?php echo $Trigger_notification_name; ?>';	
var Trigger_notification_type='<?php echo $Trigger_notification_type; ?>';
var Tier_id='<?php echo $Tier_id; ?>';
var Auction_id='<?php echo $Auction_id; ?>';
var Loyalty_rule='<?php echo $Loyalty_rule; ?>';

if(Trigger_notification_type==1)//Member Defined
{
	next_tier_info(Tier_id);
}
if(Trigger_notification_type==2)//Loyalty Defined
{
	loyalty_program_info(Loyalty_rule);
} 
