		...
		<div class="row">
		  <div class="col-sm-12">
			...
			<?php 	$attributes = array('id' => 'formValidate');
					echo form_open_multipart('Call_center/Update_Call_center_query_type',$attributes); ?>
					
					  <div class="form-group">
						<label for=""><span class="required_info">* </span>Query Type</label>
						<input class="form-control" type="text" name="Query_type_name" id="Query_type_name" value="<?php echo $query_type_details->Query_type_name; ?>" placeholder="Enter query type" data-error="Please enter query type" required="required">
					  ...
					  <div class="form-group" id="User_Type">
						...
						if($UserType != NULL)
						{
							...
							{	?>		
							<option value="<?php echo $row1->User_id; ?>" <?php if($query_type_details->User_type_id == $row1->User_id) { ?> selected <?php  } ?> ><?php echo $row1->User_type; ?> 
							</option>
					<?php	}
						}
						 ?>
					  ...
					  <div class="form-group" id="User_Name"> 
							...
							<?php $data['get_user'] = $ci_object->CallCenter_model->Fetch_query_user_id($Company_id,$query_type_details->Query_type_id); ?>
							...
							<?php	 
								foreach($query_type_details_child as  $qtccu)
								{
									$Cqc_Enrollment_id[]=$qtccu->Enrollment_id;
								}
								foreach($company_cc_user as  $company_ccu)
								{
									if(in_array($company_ccu->Enrollement_id, $Cqc_Enrollment_id))
									{
									 ?>
										<option value="<?php echo $company_ccu->Enrollement_id;?>" selected ><?php echo $company_ccu->First_name.' '.$company_ccu->Last_name;?></option>
									<?php 
									}
									else								
									{
									?>
										<option value="<?php echo $company_ccu->Enrollement_id;?>"><?php echo  $company_ccu->First_name.' '.$company_ccu->Last_name;?></option>
									<?php
									}
								}
							?>
					  ...
					  <div class="form-buttons-w">
						...
						<input type="hidden" name="QueryType_id" value="<?php echo $query_type_details->Query_type_id ?>">
					  </div>
					<?php echo form_close(); ?>		  
				  </div>
				</div>
