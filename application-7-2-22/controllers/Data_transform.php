  ...
  class Data_transform extends CI_Controller {
      ...
      if ($this->session->userdata('logged_in')) {
        ...
        if ($Seller_id == 0) {//All
          ...
          if (count($exist_merchant_array) == 0) {
            $this->session->set_flashdata("success_code", "Data map created Successfuly for All Merchant(s)!!!");
            ...
            $this->session->set_flashdata("success_code", "Data map created Successfuly!!!<br>Data map already exist for Merchant(s) ($Join_names) !!!");
          } elseif (count($exist_merchant_array) != 0 && $flag == 0) {
            $this->session->set_flashdata("error_code", "Data map already exist for All Merchant(s)  !!!");
          }
        } else {
          ...
          if ($result_count == 0) {
            ...
            if ($result222 == true) {
              $this->session->set_flashdata("success_code", "Data map created Successfuly!!");
              ...
              $this->session->set_flashdata("success_code", "Data map not created Successfuly!!");
            }
          } else {
            $this->session->set_flashdata("error_code", "Data map already exist for Merchant ($Merchant)!!");
          }
        }
    ...
    public function edit_data_map() {
      if ($this->session->userdata('logged_in')) {
        ...
        /* -----------------------Pagination--------------------- */
        $config = array();
        $config["base_url"] = base_url() . "/index.php/Data_transform/new_data_mapping";
        $total_row = $this->Data_transform_model->data_map_count($Company_id);
        $config["total_rows"] = $total_row;
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        ...
        if ($_POST == NULL) {
         ...
         /*  $seller_id = $this->input->post("seller_id");
          ...
          foreach ($UploadData as $row) {
            ...
            /* echo "----------Start-----for-----Customer_enroll_id---".$Customer_enroll_id."----------------<br><br>";
              echo "-------Member Details----------------<br><br>";
              echo "----member_Tier---".$member_Tier."---<br>";
