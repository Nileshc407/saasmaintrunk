          ...
          if ($select_cust == 2) {//SINGLE CUSTOMER
            ...
            if (count($data["Records1"]->Enrollement_id) != 0) {
              ...
              if ($report_type == 1) {//details
                ...
                $data["Trans_Records"] = $this->Coal_Report_model->get_cust_trans_summary_all($Company_id, $Enrollement_id, $start_date, $end_date, $transaction_type_id, $Tier_id, $start, $limit);
                $data["Count_Records"] = $this->Coal_Report_model->get_cust_trans_summary_all($Company_id, $Enrollement_id, $start_date, $end_date, $transaction_type_id, $Tier_id, '', '');
              }
            } else {
              $this->session->set_flashdata("error_code_CR", "Membership ID not exist");
              redirect(current_url());
    ...
    public function export_customer_report() {
      if ($this->session->userdata('logged_in')) {
        ...
        if ($select_cust == 2) {//SINGLE CUSTOMER
          ...
          if ($Report_type == 1) {//details
            $data["Trans_Records"] = $this->Coal_Report_model->get_cust_trans_details_reports($Company_id, $start_date, $end_date, $Enrollement_id, $transaction_type_id, $Tier_id);
          } else {
            $data["Trans_Records"] = $this->Coal_Report_model->get_cust_trans_summary_all($Company_id, $Enrollement_id, $start_date, $end_date, $transaction_type_id, $Tier_id);
          }
        } else { //ALL CUSTOMERS
          $Single_cust_membership_id = '';
          $Enrollement_id = 0;
