	...
	public function Edit_Merchandize_Partners()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			else
			{
				...
				{
					$All_Active_Merchandize_Items_Records = $this->Catelogue_model->Get_Merchandize_Items('', '',$Company_id,1);
					if($All_Active_Merchandize_Items_Records!=NULL)
					{
						foreach($All_Active_Merchandize_Items_Records as $Val)
						{
							if($Val->Partner_id==$lv_Partner_id)
							{	
								...
								$New_redeem_points=($New_Billing_price*$Company_Redemptionratio);
								$Post_data2 = array
								(
									'Update_user_id'=>$data['enroll'],
									'Update_date'=>date("Y-m-d H:i:s"),
	...
	public function Create_Merchandize_Category()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			{	
				...
				{
					//$this->session->set_flashdata("upload_error_code",$this->upload->display_errors());
					$this->session->set_flashdata("success_code","Merchandise Category  Created Successfuly!!");
				...
				if($Item_id !=NULL)
				{
					...
					{ 
						...
						else if($Voucher_status== 20) //Delivered
						{
							...
							$result = $this->Catelogue_model->Update_Order_Status($data2,$Membership_id,$data['Company_id'],$Evoucher_no,$Order_no);
