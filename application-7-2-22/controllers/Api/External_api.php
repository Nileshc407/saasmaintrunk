...
class External_api extends CI_Controller 
{
	public function __construct()
	{
		...
		$this->load->model('Api/External_api_model'); 
		$this->load->model('Api/External_model');	
	...
	public function ExternalPOS_api()
	{
		$date = new DateTime();
		...
		{
			$Check_company_exist = $this->External_api_model->Check_compnay_by_username($API_Company_username);							
			if($Check_company_exist !=NULL)
			{
				...
				if($Password_match == 0)
				{
					$result = $this->External_api_model->check_company_key_valid($Company_id);// Check Company Key validation
				...
				/***************************Fetch Member Info**************************/
					if($API_flag == 5)
					{
						$Channel_Details = $this->External_api_model->get_3rd_party_channel_details($Company_id,$API_Channel_password);
						if($Channel_Details != Null)
						{
							...
							$result = $this->External_api_model->get_pos($Membership_id,$Company_id,$phnumber); //Get Customer details	
						...
						if($Channel_Details != Null)
						{
							...
							if($result!=NULL)
							{
								...
								if($Company_meal_flag == 0 && $Daily_points_consumption_flag == 0)
								{
									if($Total_reddems == 0)
									{
										$result51 = $this->External_api_model->Get_Member_Tier_Details($lv_member_Tier_id,$Company_id);
								...
								{
									$result52 = $this->External_api_model->Get_Member_Tier_Details($lv_member_Tier_id,$Company_id);
									$Tier_consumption_limit = $result52->Redeemtion_limit;
									
									$Consumption_details = $this->External_api_model->Get_points_consumption_details($Cust_enrollement_id,$Company_id);
								...
								$result1 = $this->External_api_model->Get_outlet_details($Pos_outlet_id,$Company_id); //Get Outlet Details
								...
								if($result1!=NULL)
								{
									...
									/*****************validate items 03-08-2021*****************************/
										if($Pos_bill_items != Null)
										{
											...
											{ 
												...
												$ItemDetails = $this->External_api_model->Get_item_details($ItemCode,$Company_id);
									...
									/*****************validate items 03-08-2021*****************************/
										...
										{	
											$result221 = $this->External_api_model->Check_redeem_request_issent($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
											...
											if($result221 != Null)
											{
												...
												if($Earlier_request_flag == 0)
												{
													...
													if($currentTime > $cenvertedTime)
													{
														...
														$this->External_api_model->update_cust_old_redeem_request($data1,$Cust_enrollement_id,$Company_id,$Seller_id,$ChannelCompanyId);
													}
												}
											}
											
											$result22 = $this->External_api_model->Check_redeem_request_issent($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
											...
											if($result22==NULL) 
											{
												...
												/*****************11-3-2021*******************/
													...
													$Redeem_request = $this->External_api_model->Cust_redeem_request_insert($data);
											...
											$result2 = $this->External_api_model->Get_cust_approved_request($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
				...
				/**********************Validate Voucher Request*******************************/
					if($API_flag == 7) 
					{
						...
						if($Channel_Details != Null)
						{
							...
							$POS_Discount = str_replace( ',', '', $POS_Discount);
								...
								$result1 = $this->External_api_model->Get_outlet_details($Outlet_no,$Company_id); //Get Outlet Details
								...
								if($result1!=NULL)
								{
									...
									{	
										...
										/*******************api logic start********************/
										$Voucher_result = $this->External_api_model->Validate_discount_voucher($Card_id,$Company_id,$Discount_voucher_code,$Voucher_amount);
										if($Voucher_result != Null)
										{
											...
											{	
												$Product_Voucher_Details = $this->External_api_model->Get_Product_Voucher_Details($Gift_card_id,$Cust_enrollement_id,$Company_id);
												...
												if($Product_Voucher_id ==0) // product voucher in percentage
												{
													...
													{ 
														...
														if($ItemDetails !=NULL)
														{
															...
															$CheckItemTempCart = $this->External_api_model->GetItemsDetails($Company_id,$Cust_enrollement_id,$ItemCode,$Outlet_no,$ChannelCompanyId);
															...
															else
															{
																...
																$this->External_api_model->insert_item($data79);
															}
													...
													$GetItems= $this->External_api_model->Get_items($Company_id,$Cust_enrollement_id,$Outlet_no,$ChannelCompanyId);
													if($GetItems != Null)
													{
												...
												/**********************stamp new logic 02-05-2021**************************/
													$Get_lowest_sent_vouchers= $this->External_api_model->Get_lowest_sent_vouchers($Card_id,$Company_id,$Gift_card_id);
													...
													$delete_temp_cart = $this->External_api_model->delete_pos_temp_cart_data($Company_id,$Cust_enrollement_id,$Outlet_no,$ChannelCompanyId);
					...
					if($API_flag == 8) // 3rd Party Pos Ordering
					{ 
						$Channel_Details = $this->External_api_model->get_3rd_party_channel_details($Company_id,$API_Channel_password);
						if($Channel_Details != Null)
						{
							...
							$cust_details = $this->External_api_model->cust_details_from_card($Company_id,$cardId,$phnumber); //Get Customer details
							if($cust_details !=NULL)
							{
								...
								{
									$Redeem_details=$this->External_api_model->Check_redeem_details($Customer_enroll_id,$Company_id,$Pos_outlet_id,$ChannelCompanyId,$RedeemedConfirmationCode,$Pos_points_redeemed,$Pos_bill_no);
								...
								$result01 = $this->External_api_model->check_pos_bill_no($Pos_bill_no,$Pos_outlet_id,$Company_id,$Pos_bill_date_time);				
								if ($result01 > 0)
								{
							...
							if($Pos_bill_items != Null)
							{
								...
								{ 
									...
									if($ItemDetails !=NULL)
									{
										...
										// $ItemCodeArr[$ItemCode]=$item['Item_Qty'];
										$CheckItemTempCart = $this->External_api_model->GetItemsDetails($Company_id,$Cust_enrollement_id,$ItemCode,$delivery_outlet,$ChannelCompanyId);
										if($CheckItemTempCart != Null)
										{
											$TempQty = $CheckItemTempCart->Item_qty;
										...
										else
										{
											...
											$this->External_api_model->insert_item($data78);
										}
									}
									else
									{
										$delete_temp_cart = $this->External_api_model->delete_pos_temp_cart_data($Company_id,$Cust_enrollement_id,$delivery_outlet,$ChannelCompanyId);
										$ResponseData = array("Error_flag" => 3103,
																"Message" => "Invalid Item_Num Or Item not exist.",
																"Item_Num" => $ItemCode,
																"Membership_id" => $CardId,
							...
							else
							{
								...
								$Pos_bill_items = $this->External_api_model->Get_temp_cart_items($Company_id,$Cust_enrollement_id,$delivery_outlet,$ChannelCompanyId);
							/*****************check bill amount and item amount 27-09-2021***********************/	
								$Bill_total_array = array();
							...
							/************************************************************/
								if($Pos_voucher_no)
								{
									$Voucher_result = $this->External_api_model->Validate_discount_voucher($CardId,$Company_id,$Pos_voucher_no,$Pos_voucher_amount);
									if($Voucher_result == Null)
									{
										$delete_temp_cart = $this->External_api_model->delete_pos_temp_cart_data($Company_id,$Cust_enrollement_id,$delivery_outlet,$ChannelCompanyId);
									...
									else
									{
										...
										if($Card_Payment_Type_id == 997) //product voucher
										{	
											$Product_Voucher_Details = $this->External_api_model->Get_Product_Voucher_Details($Pos_voucher_no,$Customer_enroll_id,$Company_id);
											...
											/**********************stamp new logic 02-05-2021**************************/
												$Get_lowest_sent_vouchers= $this->External_api_model->Get_lowest_sent_vouchers($CardId,$Company_id,$Pos_voucher_no);
												// 1 Kenya Tea + 1 Chai latte + 1 Lemon Tea
												if($Get_lowest_sent_vouchers != NULL)
												{
													$RemQTY=0;
											...
											{	
												...
												{ 
													...
													$ItemDetails1 = $this->External_api_model->Get_item_details($ItemCode,$Company_id);
							...
							/********************09-04-2021 validate gift card**********************/
								if($Pos_giftcard_no)
								{
									$Giftcard_result = $this->External_api_model->Validate_gift_card($Company_id,$Pos_giftcard_no);
							...
							/********************09-04-2021 validate gift card**********************/
								$get_city_state_country = $this->External_api_model->Fetch_city_state_country($Company_id,$Customer_enroll_id);
								...
								$result = $this->External_api_model->Get_outlet_details($Pos_outlet_id,$Company_id); //Get Outlet Details
								if($result!=NULL)
								{
									...
									// {
											...
											{ 
												...
												$Item_price = str_replace( ',', '', $Item_price);
													...
													$DiscountResult = $this->External_api_model->get_discount_value("",$ItemCode,$Item_price,$Company_id,$Pos_outlet_id,$Customer_enroll_id,$lv_member_Tier_id,0,$API_flag_call);
											...
											foreach($Itemcategory_ids as $Itemcategory_id)
											{
												...
												$CatDiscountResult = $this->External_api_model->get_category_discount_value($Itemcategory_id,"",$Item_price,$Company_id,$Pos_outlet_id,$Customer_enroll_id,$lv_member_Tier_id,0,0,$API_flag_call);
										...
										///**************** sandeep category discount logic 27-01-2020 ***************************
											...
											$DiscountResult12 = $this->External_api_model->get_discount_value($Itemcategory_id,$ItemCode,$Item_price,$Company_id,$Pos_outlet_id,$Customer_enroll_id,$lv_member_Tier_id,$order_sub_total,$API_flag_call);
									...
									if($grand_total < 0 )
									{
										...
										$Extra_earn_points_Loyalty_pts = array();
											foreach ($Pos_bill_items as $item)
											{
												...
												/********Get Merchandize item name********/
													$result = $this->External_api_model->Get_merchandize_item($Item_code,$Company_id);
													...
													$Item_branch = $this->External_api_model->get_items_branches($Company_merchandize_item_code,$Merchandize_partner_id,$Company_id);
													...
													{
													/**********Get Seller Details**********/
														$Seller_result = $this->External_model->Get_Seller_details($sellerID,$Company_id);	
													...
													/**********Get Seller Details**********/
														...
														$loyalty_prog = $this->External_model->get_tierbased_loyalty($Company_id,$seller_id,$lv_member_Tier_id,$lv_date_time2);
														...
														if($loyalty_prog != NULL )
														{
															...
															$lp_details = $this->External_model->get_loyalty_program_details($Company_id,$seller_id,$prog,$lv_date_time);
															...
															//********* 10-7-2020- nilesh ****channel and payment ***************
																if($Trans_Channel_flag==1)
																{
																	if($member_Tier_id == $lp_Tier_id  && $Loyalty_at_flag == 1 && $Trans_Channel_id == $Trans_Channel )
																	{
																		for($i=0;$i<=count($value)-1;$i++)
																		{
																			if($i<count($value)-1 && $value[$i+1] != "")
																			{
																				...
																				{
																					$loyalty_points = $this->External_model->get_discount($transaction_amt,$dis[$i]);
																	...
																	{
																		$loyalty_points = $this->External_model->get_discount($transaction_amt,$dis[0]);
																		$points_array[] = $loyalty_points;
																		$gained_points_fag = 1;
															...
															//********* 10-7-2020- nilesh ****channel and payment ***************
																...
																{											
																	$Get_segments2 = $this->External_model->edit_segment_id($Company_id,$Segment_id);
																	...
																	foreach($Get_segments2 as $Get_segments)
																	{
																		...
																		$Get_segments = $this->External_model->Get_segment_based_customers($lv_Cust_value,$Get_segments->Operator,$Get_segments->Value,$Get_segments->Value1,$Get_segments->Value2);
													...
													/******************Get Supper Seller Details*********************/
														$Super_seller_flag = 1;
														$result = $this->External_model->Get_Seller($Super_seller_flag,$Company_id);				   
														$seller_id = $result->Enrollement_id;
														$seller_fname = $result->First_name;
														$seller_lname = $result->Last_name;
												...
												//***********18-04-2020-allow to redeem 1 point extra****************/
													...
													$data123 = array('Company_id' => $Company_id,
															...
															$Transaction_detail = $this->External_model->Insert_online_purchase_transaction($data123);
													...
													/******Insert online purchase log tbl entery*******/	
														...
														$result_log_table = $this->External_api_model->Insert_log_table($Company_id,$enroll,$username,$Seller_name,$Todays_date,$what,$where,$userid,$opration,$opval,$firstName,$lastName,$To_enrollid);	
													/***Insert online purchase log tbl entery******/
													}
													...
													{		
														...
														{
															$Get_loyalty = $this->External_model->Get_loyalty_details_for_online_purchase($Applied_loyalty_id[$l]);
														   ...
														   $child_data = array(					
															...
															$child_result = $this->External_model->insert_loyalty_transaction_child($child_data);
														}
													}
											...
											/**********************Stamp offer logic 02-02-2021**************************/
												if($Stamp_item_flag == 1)
												{ 
													$Product_offers = $this->External_api_model->get_product_offers($Merchandise_item_id,$Merchandize_category_id,$Company_id,$seller_id);
													
													if($Product_offers != null)
													{		
														foreach($Product_offers as $offer)
														{	
															$Offers_items = $this->External_api_model->get_offer_selected_items($offer['Offer_code'],$Company_id);
															...
															if($Offers_items != NULL)
															{
																...
																{
																	$Total_item_purchase[] = $this->External_api_model->get_item_purchase_count($rec->Company_merchandize_item_code,$Company_id,$Customer_enroll_id,$offer['From_date'],$offer['Till_date']);
																}
															}
															...
															{
																$Total_sent_voucher = $this->External_api_model->member_sent_offers($Company_id,$Customer_enroll_id,$offer['Offer_code'],$offer['Free_item_id']);
																...
																for($A=1;$A<= $Need_to_Send_Voucher;$A++)
																{
																	...
																	$FreeItem = $this->External_api_model->get_offer_free_items($offer['Offer_code'],$Company_id);
																	...
																	foreach($FreeItem as $FreeItem)
																	{
																		...
																		$this->External_api_model->insert_product_voucher($data76);
																	}
															...
															//****insert in gift card tbl ***********
																	...
																	$this->External_api_model->insert_voucher_in_gift_card($data77);
															//****insert in gift card tbl ***********
										...
										/************* Update Current Balance ******************/
											...
											$this->External_model->update_cust_balance($up,$cid,$Company_id);	
											
											$bill_no = $bill + 1;
											$billno_withyear = $str.$bill_no;
											$result4 = $this->External_model->update_billno($seller_id,$billno_withyear);
										/*********** Update Current Balance ***************/
									...
									$DiscountResultVal = $this->External_api_model->get_payment_type_discount_value($Payment_type_id,$Company_id,$Pos_outlet_id,$Customer_enroll_id,$lv_member_Tier_id,$grand_total);
					...
					{ 						
						$Partner_id=0;
							...
							$Validate_seller=$this->External_api_model->Check_seller_exist($Seller_id,$Company_id);
							...
							else
							{
								...
								$Validate_Outlet=$this->External_api_model->Check_outlet_exist($Seller_id,$Outlet_id,$Company_id);
								...
								/* $Get_brand_items=$this->External_api_model->Get_brand_items($Seller_id,$Company_id);
								if($Get_brand_items != NULL )	
								{
									foreach($Get_brand_items as $row)
									{
										$Check_itm_trans=$this->External_api_model->Check_item_trans($row->Company_merchandize_item_code,$Company_id);
										if($Check_itm_trans > '0')
										{
											...
											$Update_Menu_Items = $this->External_api_model->update_Menu_Items($data,$row->Company_merchandize_item_code,$Company_id);
										}
										
										else
										{
											$Delete_Items = $this->External_api_model->Delete_brand_item($row->Company_merchandize_item_code,$Company_id);
										}
									}
								} */
							...
							$Delete_pos_api_err = $this->External_api_model->delete_pos_api_error($Company_id,$Seller_id,$Outlet_id);
							//--------------------------------------------------------------
							...
							if(isset($inputArgs['Product_Groups']))
							{
								$Entry_flag=1;
								 ...
								 {
									$Check_product_group = $this->External_api_model->Check_product_group($val['Product_Group_Id'],$Company_id);
									...
									{
										/* $Check_product_group2 = $this->External_api_model->Check_product_group2($val->Product_Group_Id);
										if($Check_product_group2 == 0)
										{ */
											 $Insert_product_group = $this->External_api_model->insert_product_group($Product_group_data);
										//}
										/* else
										{
											$Error_data = array(					
											...
											$Insert_error_pos_api = $this->External_api_model->Insert_error_pos_api($Error_data);
										} */
									}
									else
									{
										$Update_product_group = $this->External_api_model->Update_product_group($Product_group_data,$val['Product_Group_Id'],$Company_id);
									}
								 ...
								 foreach($inputArgs['Product_SubGroups'] as $val)
								 {
									 ...
									 if($Check_product_group != NULL)
									{
										...
										$Product_subgroup_data = array(					
										 ...
										 $Check_product_subgroup = $this->External_api_model->Check_product_subgroup($val['Product_SubGroup_Id'],$Company_id);
										 
										
										if($Check_product_subgroup == NULL)
										{
											/* $Check_product_subgroup2 = $this->External_api_model->Check_product_subgroup2($val->Product_SubGroup_Id);
											
											if($Check_product_subgroup2 == 0)
											{ */
												$Insert_product_subgroup = $this->External_api_model->insert_product_subgroup($Product_subgroup_data);
										...
										{
											$Update_product_subgroup = $this->External_api_model->update_product_subgroup($Product_subgroup_data,$val['Product_SubGroup_Id'],$Company_id);
										}
									}
									else
									{ 
							...
							if(isset($inputArgs['Menu_Groups']))
							{
								$Entry_flag=1;
								 ...
								 {
									 $Check_Menu_Groups = $this->External_api_model->Check_Menu_Groups($val['Menu_Group_Id'],$Company_id);
									
										 if($Check_Menu_Groups == NULL)
										{
										
											/* $Check_Menu_Groups2 = $this->External_api_model->Check_Menu_Groups2($val->Menu_Group_Id);
											...
											{ */
												...
												$Insert_Menu_Groups = $this->External_api_model->insert_Menu_Groups($Menu_Groups_data);
											/* }
											else
											{
												$Error_data = array(					
										...
										else
										{
											...
											$Update_Menu_Groups = $this->External_api_model->update_Menu_Groups($Menu_Groups_data,$val['Menu_Group_Id'],$Company_id);
										}
										...
										if($Check_product_subgroup != NULL)
										{
											...
											if($Check_Menu_Groups != NULL)
											{
												$Merchandize_category_id=$Check_Menu_Groups->Merchandize_category_id;
													...
													$Check_Condiments = $this->External_api_model->Check_Item_Num($val['Item_Num'],$Company_id);
													
													/*******************check item linked outlets*********/
														$Check_linkedItems = $this->External_api_model->Check_pos_item_linked_outlets($Outlet_id,$Company_id,$val['Item_Num']);
														...
														if($Check_linkedItems == 0)
														{
															...
															$item_outlet_data = array(					
															 ...
															 $insert_item_outlet = $this->External_api_model->insert_item_linked_outlet($item_outlet_data);
													...
													if($Check_Condiments == 0)
													{
														...
														$Condiments_data = array(					
														 ...
														 $insert_Condiments = $this->External_api_model->insert_Condiments($Condiments_data);
													...
													else
													{
														...
														$Check_price = $this->External_api_model->Check_pos_price_child($Company_id,$val['Item_Num'],$val['Item_Rate']);
														...
														$update_Condiments = $this->External_api_model->update_Condiments($Condiments_data,$val['Item_Num'],$Company_id);
													}
							...
							if(isset($inputArgs['Condiment_Groups']))
							{
								...
								{
									$delete_Condiment_Groups = $this->External_api_model->delete_Condiment_Groups($val['Condiment_Group_Num'],$Company_id);
									 
									if(isset($val['Condiment_Nums']))
									{
										foreach($val['Condiment_Nums'] as $val2)
										{
											 $Check_Condiments = $this->External_api_model->Check_Item_Num($val2,$Company_id);
											 
										
											 if($Check_Condiments > 0)
											{
												$Menu_group_id = $this->External_api_model->Get_Category_id_item($val2,$Company_id);
												...
												$insert_Condiment_Groups = $this->External_api_model->insert_Condiment_Groups($Condiment_Groups_data);
												...
												$insert_Condiment_Groups = $this->External_api_model->insert_Condiment_Groups($Condiment_Groups_data2);
											...
											else
											{
												...
												$Insert_error_pos_api = $this->External_api_model->Insert_error_pos_api($error_pos_api);
											}
										}
									}
								...
								$Entry_flag=1;
								 foreach($inputArgs['Menu_Items'] as $val)
								 {
									...
									if(isset($val['Required_Condiment']))
									{
										...
										{
											$Check_Condiments = $this->External_api_model->Check_Condiments($val2,$Company_id);
									...
									$Check_Item_Num = $this->External_api_model->Check_Item_Num($val['Item_Num'],$Company_id);
									$delete_cond = $this->External_api_model->delete_Menu_condiments_data($val['Item_Num'],$Company_id);
									...
									if(isset($val['Required_Condiment']))
									{
										...
										foreach($Required_Condiment['Condiment_Group_Nums'] as $val2)
										{
											...
											if($val2!='0')
											{
												$Condiment_item_code = $this->External_api_model->Select_Condiment_item_code($val2,$Company_id,'14');
												...
												if($Condiment_item_code!=NULL)
												{
													foreach($Condiment_item_code as $val3)
													{
														...
														$insert_Menu_condiments_data = $this->External_api_model->insert_Menu_condiments_data($Menu_condiments_data);
													}
												}
											}
									...
									if(isset($val['Allowed_Condiment']))
									{
										...
										foreach($Allowed_Condiment['Condiment_Group_Nums'] as $val2)
										{
											$Condiment_item_code = $this->External_api_model->Select_Condiment_item_code($val2,$Company_id,'15');
											if($Condiment_item_code!=NULL)
											{
												foreach($Condiment_item_code as $val3)
												{
									...
									if($Check_Item_Num == 0 )
									{
										...
										/* $Check_Item_Num2 = $this->External_api_model->Check_Item_Num2($val->Item_Num);
										...
										{ */
											...
											{	
												...
												$Insert_Menu_Items = $this->External_api_model->insert_Menu_Items($Menu_Items_data);
									...
									else
									{
										if($Error_flag == 0)
										{
											...
											$Update_Menu_Items = $this->External_api_model->update_Menu_Items($Menu_Items_data,$val['Item_Num'],$Company_id);
										}
									}
							...
							if(isset($inputArgs['Combo_Meals']))
							{
								...
								foreach($inputArgs['Combo_Meals'] as $val)
								{
									...
									if(isset($val['Combo_Meal_Main_Items']))
									{
										...
										/* foreach($val->Combo_Meal_Main_Items as $val3)
										{
											...
											$Check_Item_Num = $this->External_api_model->Check_Item_Num($val3->Item_Num,$Company_id);
											if($Check_Item_Num==0)
											{
									...
									if(isset($val['Combo_Meal_Side_Groups']))
									{
										foreach($val['Combo_Meal_Side_Groups'] as $val4)
										{
											// echo "<br><br>Item_Num :: ".$val3->Item_Num;
											/****sandeep**
											$Check_Menu_Groups = $this->External_api_model->Check_Menu_Groups($val4->Side_Group_Id,$Company_id);
										...
										$this->process_items($val4['Combo_Meal_Sides'],$Company_id,$Seller_id,$Partner_id,$Outlet_id);
												...
												{
													$Check_Item_Num = $this->External_api_model->Check_Item_Num($val5->Item_Num,$Company_id);
													if($Check_Item_Num == 0)
													{
									...
									//{
										...
										$Check_product_group = $this->External_api_model->Check_product_group($Combo_Meal_Item['Product_Group_Id'],$Company_id);
										...
										$Check_product_subgroup = $this->External_api_model->Check_product_subgroup($Combo_Meal_Item['Product_SubGroup_Id'],$Company_id);
										...
										/**********5.Check Valid Menu_Group_Id**************/
											$Check_Menu_Groups = $this->External_api_model->Check_Menu_Groups($Combo_Meal_Item['Menu_Group_Id'],$Company_id);
										...
										$Check_Item_Num = $this->External_api_model->Check_Item_Num($Menu_Item_code,$Company_id);
										
										
										/*******************check item linked outlets*********/
										$Check_linkedItems = $this->External_api_model->Check_pos_item_linked_outlets($Outlet_id,$Company_id,$Menu_Item_code);
										if($Check_linkedItems == 0)
										{	
											//***************Items linked to outlet
											$item_outlet_data = array(					
										...
										if($Check_Item_Num == 0)
										{
											...
											/* $Check_Item_Num2 = $this->External_api_model->Check_Item_Num2($Menu_Item_code);
											...
											{ */
												...
												if($Error_flag==0)
												{
													...
													$Insert_Menu_Items = $this->External_api_model->insert_Menu_Items($Combo_Items_data);
												}
											/* 	
											}
											else
										...
										else
										{
											...
											if($Error_flag==0)
											{
												$Check_price = $this->External_api_model->Check_pos_price_child($Company_id,$Menu_Item_code,$Combo_Meal_Item['Item_Rate']);
												...
												$Update_Menu_Items = $this->External_api_model->update_Menu_Items($Combo_Items_data,$Menu_Item_code,$Company_id);
											}
									...
									if($Error_flag==0)
									{
										$Delete_combo_child = $this->External_api_model->Delete_item_combo_child($Company_id,$Menu_Item_code,'MAIN');
										if(isset($val['Combo_Meal_Main_Items']))
										{
											foreach($val['Combo_Meal_Main_Items'] as $val3)
											{
								...
								if($Error_flag==0)
								{
									...
									$Delete_combo_child = $this->External_api_model->Delete_item_combo_child($Company_id,$Menu_Item_code,'SIDE');
									...
									// print_r($val->Combo_Meal_Side_Groups);
										...
										{
											$Side_option=1;
											$Delete_side_items = $this->External_api_model->Delete_side_group_child($Company_id,$Menu_Item_code);
											foreach($val['Combo_Meal_Side_Groups'] as $val4)
											{
												...
												if(isset($val4['Combo_Meal_Sides']))
												{
													...
													foreach($val4['Combo_Meal_Sides'] as $val5)
													{
														...
														/* $Check_Menu_Groups2 = $this->External_api_model->Check_Menu_Groups2($val5->Menu_Group_Id);
														
														if($Check_Menu_Groups2 > 0)
														{ */
														$Check_Menu_Groups = $this->External_api_model->Check_Menu_Groups($val5['Menu_Group_Id'],$Company_id);
							...
							/********************************************************************************/
									...
									$Error_Records= $this->External_api_model->Select_POS_API_Error_Records($Company_id,$Seller_id,$Outlet_id);
									...
									if($Error_Records != NULL)
									{
									  ...
									  $pos_html = "<p>Hello,</p>
										 ...
										 $this->email->bcc(array('rakesh@miraclecartes.com','amitk@miraclecartes.com','demoloyalty@novacom.co.ke'));
...
//*** sandeep **** 
					/**********3.Check Valid Product_Group_Id**************/	
					$Check_product_group = $this->External_api_model->Check_product_group($val3['Product_Group_Id'],$Company_id);
					...
					/**********4.Check Valid Product_SubGroup_Id**************/											
					$Check_product_subgroup = $this->External_api_model->Check_product_subgroup($val3['Product_SubGroup_Id'],$Company_id);
					...
					/**********5.Check Valid Menu_Group_Id**************/
						$Check_Menu_Groups = $this->External_api_model->Check_Menu_Groups($val3['Menu_Group_Id'],$Company_id);
						...
						{	
							...
							$Check_linkedItems = $this->External_api_model->Check_pos_item_linked_outlets($Outlet_id,$Company_id,$val3['Item_Num']);
							if($Check_linkedItems == 0)
							{
							//***************Items linked to outlet
								$item_outlet_data = array(					
