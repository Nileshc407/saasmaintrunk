...
Class SegmentC extends CI_Controller 
{
	...
	public function Segment()
	{
		...
		{	
			...
			/*-----------------------Pagination---------------------*/		
			$config = array();
			$config["base_url"] = base_url() . "/index.php/SegmentC/Segment";
			$total_row = $this->Segment_model->Segment_count($Company_id);	//echo "--total_row --".$total_row ;	
			...
			$data["segment_results"] = $this->Segment_model->Segment_list($config["per_page"], $page,$Company_id);
	...
	public function delete_segment_id()
	{
		...
		{	
			...
			{			
				...
				// $Company_id = $_GET['CompanyId'];		
				/*********Delete from child table*************************/	
				$Get_segment_code = $this->Segment_model->Get_segment_code($Segmentid);
				$Segment_type_id= $Get_segment_code->Segment_type_id;
				$Segment_code= $Get_segment_code->Segment_code;

				if($Segment_type_id==13)//Item Category
				{
					$delete_child = $this->Segment_model->delete_segment_child($Company_id,$Segment_code);
					$delete_master_item = $this->Segment_model->delete_segment_master_item($Company_id,$Segment_code);
				}
				...
				/*else 
				{
					$this->output->set_output("Sorry, Segment deleted error!!");
					$this->session->set_flashdata("error_code","Sorry, Segment Criteria Not Deleted!!");
				}*/
...
/*************************AMIT KAMBLE 04-02-2020 **************************/
	...
	public function Save_items_segment()
	{
		...
		$this->output->set_content_type('application/json');
		$this->output->set_output(json_encode(array('linked_itemcode'=> $Item_code2,'linked_Category_id'=> $Category_id,'Linked_items'=> $theHTMLResponse)));
	}
