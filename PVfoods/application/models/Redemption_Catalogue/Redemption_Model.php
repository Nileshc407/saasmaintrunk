...
class Redemption_Model extends CI_Model 
{
	public function get_all_items($limit,$start,$Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_item_type) 
	{
		...
		if($Merchandize_category_id==0)//ALL
		{
			...
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Link_to_Member_Enrollment_flag'=> 0,'A.Send_once_year'=> 0,'A.Send_other_benefits'=> 0,));
		}
		else
		{
			...
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_category_id'=>$Merchandize_category_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Link_to_Member_Enrollment_flag' => 0,'A.Send_once_year' => 0,'A.Send_other_benefits' => 0));
		}
	...
	function Get_Merchandize_Items_Count($Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_item_type)
	{
		...
		if($Merchandize_category_id==0)//ALL
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'Link_to_Member_Enrollment_flag'=> 0,'Send_once_year'=> 0,'Send_other_benefits'=> 0));
		...
		else
		{
			...
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_category_id'=>$Merchandize_category_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'Link_to_Member_Enrollment_flag' => 0,'Send_once_year' => 0,'Send_other_benefits' => 0));
		}
		if($Sort_by_merchant!=0)//ALL merchants
		{
			$this->db->where(array('A.Seller_id'=>$Sort_by_merchant));
	...
	function Get_Company_evoucherPartner($Company_id)
	{
		$this->db->select('*');
		$this->db->from('igain_partner_master');
		$this->db->where(array('Company_id'=>$Company_id,'Partner_name'=>'e-Gifting Partner'));
		$sql=$this->db->get();
		if($sql->num_rows()>0)
		{
			return $sql->row();
	...
	public function Insert_eVouchar_placeOrder_response($data)
	{
		$this->db->insert('igain_thirdparty_evouchar_order_log',$data);
		if($this->db->affected_rows() > 0)
		{
			return $this->db->insert_id();
		}
	...
	function Get_Merchandize_Item_details_byCode($Item_code)  
	{ 
		$this->db->select( 'Company_merchandize_item_code,Merchandize_item_name,Merchandise_item_description,Billing_price_in_points,Item_image1,Item_image2,Item_image3,Item_image4,Thumbnail_image1,Thumbnail_image2,Thumbnail_image3,Thumbnail_image4,Company_id,Delivery_method,Company_merchandise_item_id,Brand_flag,Item_Brand,Colour_flag,Item_Colour,Dimension_flag,Item_Dimension,Weight_flag,Item_Weight,Size_flag,Manufacturer_flag,Item_Manufacturer,Merchant_flag,Seller_id,Size_chart,Size_chart_image,Weight_unit_id,Merchandize_item_type'); 
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Active_flag'=>1,'show_item'=>1,'Company_merchandize_item_code'=>$Item_code));
		$sql=$this->db->get();
		// echo "<br>".$this->db->last_query();
		if($sql->num_rows()>0)
		{
	...
	{
		$this->db->where(array('Company_id'=>$Company_id,'Pid'=>$productId,'PO_number'=>$poNumber));
		$this->db->update('igain_thirdparty_evouchar_order_log',array('Status'=>1));
		// echo "<br>".$this->db->last_query();
		if($this->db->affected_rows() > 0)
		{
			return true;
