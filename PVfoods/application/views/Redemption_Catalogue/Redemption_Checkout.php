								...
								<tbody>
									...
									{		
										...
										<!--	<td><b></b>&nbsp;<?php if($item["Size"] == 1) { $item_Size="Small"; } elseif($item["Size"] == 2) { $item_Size ="Medium"; } elseif($item["Size"] == 3) { $item_Size ="Large"; } elseif($item["Size"] == 4) { $item_Size ="Extra Large"; } elseif($item["Size"] == 0) { $item_Size = "-";}
											...
											<td>
												<a href="JavaScript:void(0);" onclick="delete_item('<?php echo $item['Item_code']; ?>','<?php echo $item['Merchandize_item_name']; ?>','<?php echo $item['Branch']; ?>','<?php echo $item['Total_points']; ?>','<?php echo $item['Size']; ?>','<?php echo $item["Redemption_method"]; ?>')">
									...
									<tr>
										...
										<th colspan="2"><b><div id="Sub_total"></b>&nbsp;<?php 
										echo ($lv_Sub_Total); ?> <?php echo $Company_Details->Currency_name; ?></div></th>
									</tr>
							
									<tr>
										<th colspan="6"><?php echo ('Grand Total'); ?></th>
										<th colspan="2"><b><div id="Grand_total"></b>&nbsp;<?php 
...
alert("Size "+Size); */
		...
		var Grand_total2=<?php echo $Total_Redeem_points; ?>;
		var Total_Redeem_points=(Grand_total2-Total_points);
		$.ajax({
			type:"POST",
			dataType: 'json',
