		...
		echo form_open_multipart('Shopping/CheckoutPayment',array('onsubmit'=>'return Validate_form();','id' => 'SubmitForm')); ?>
			...
			<div class="accordion" id="accordion">
									...
									<?php foreach($DiscountVouchers as $v){
										// echo"--".$Card_balance."---".$Discount_percentage."---<br>";
											if($v['Discount_percentage'] > 0){ ?>
												...
												<option value="<?php echo $v['Gift_card_id']."_".$v['Discount_percentage']."_2"; ?>"><?php echo $v['Gift_card_id']; ?>: &nbsp;&nbsp;<?php echo $v['Discount_percentage']; ?>(%)
												</option>
											
											<?php } else {
											?>
												<option value="<?php echo $v['Gift_card_id']."_".$v['Card_balance']."_1"; ?>">									
									...
									<?php  if($RevenueVouchers) { ?>
										...
										?>
											<option value="<?php echo $v1['Gift_card_id']."_".$v1['Card_balance']."_1"; ?>">									
												<?php echo $v1['Gift_card_id']; ?>: <?php echo $Symbol_of_currency; ?> <?php echo number_format($v1['Card_balance'],2); ?>
												</option>								
										...
										foreach($Prod as $v2){
											// echo"--".$Card_balance."---".$Discount_percentage."---<br>";
												if($v2['Discount_percentage'] > 0){ ?>
													...
													<option value="<?php echo $v2['Gift_card_id']."_".$v2['Discount_percentage']."_2"; ?>"><?php echo $v2['Gift_card_id']; ?>: &nbsp;&nbsp;<?php echo $v2['Discount_percentage']; ?>(%)
													</option>
												
												<?php } else {
												?>
													<option value="<?php echo $v2['Gift_card_id']."_".$v2['Card_balance']."_1"; ?>">									
													<?php echo $v2['Gift_card_id']; ?>: <?php echo $Symbol_of_currency; ?> <?php echo number_format($v2['Card_balance'],2); ?>
													</option>
												<?php 
												}
										}
				...
				<div class="item">
					...
					<div class="accordion-header">
						<button type="button" onclick="ApplyPaymentRule(5);" class="btn" data-toggle="collapse" data-target="#collapse3" aria-expanded="false" id="rdm_mpesa">
							MPESA Transaction
						</button>
					</div>
		...
		function Call_API()
		{
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					$("#Paid_by_MPESA").val(response2.MpesaPaidAmount); // Nilesh 04-7-2020
