...
{  
	...
	function getchatsnew($Company_id,$Cust_email_id,$user)
	{
		$this->db->select('*');
		$this->db->from('igain_call_center_live_chat');
		// $this->db->where("(To_email_id = '".$Cust_email_id."' AND read=0)");
		$this->db->where("(To_email_id = '".$Cust_email_id."' AND Company_id = '".$Company_id."')");
	...
	function checkchatsnew($Company_id,$Cust_email_id,$user)
	{
	...
	function getchatsall($Company_id,$Cust_email_id,$user)
	{
		$Todays=date("Y-m-d");//2017-08-23 05:47:00
		$this->db->select('*');
		$this->db->from('igain_call_center_live_chat');
		$this->db->where("(read=1 AND Sent_date >='".$Todays."') AND (To_email_id ='".$Cust_email_id."' OR From_email_id ='".$Cust_email_id."' ) AND (Company_id ='".$Company_id."')");
		$this->db->order_by('Id','ASC');
		$result = $this->db->get();
		//echo $this->db->last_query(); 
		return $result->result_array(); 
	...
	function get_enrollment_details($userId,$Sub_seller_admin,$Company_id)
	{
		$this->db->from('igain_enrollment_master');
		$this->db->where(array('User_id' => $userId, 'Sub_seller_admin' => $Sub_seller_admin, 'Company_id' => $Company_id, 'User_activated' => 1));		
		$sql = $this->db->get();
	...
	function Chack_user_status($login_enrollId,$Company_id)
	{
		$this->db->select("*");
		$this->db->from('igain_session_tbl');
		$this->db->where(array('enrollId' =>$login_enrollId, 'Company_id' =>$Company_id, 'description' =>1));		
		$sql = $this->db->get();
		if($sql -> num_rows() == 1)
		{
			return $sql->row();
