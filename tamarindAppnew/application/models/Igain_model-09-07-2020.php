...
/********************************************Akshay Start*******************************/
	 ...
	 function get_discount_value($Itemcategory_id,$ItemCode,$Item_price,$Company_id,$delivery_outlet,$cust_enroll,$Tier_id,$grandTotal,$channel)
	{
		...
		if ($sql->num_rows() > 0)
		{
			...
			//		echo "in seller ".$step."<br>";
				...
				if($row->Discount_rule_for == 4 && $step == 2 && $grandTotal > 0 && $channel == $row->Channel_id) // on transaction channel
				{
	...
	function get_member_product_inprecentage_vouchers($Card_id,$Company_id,$item_code)
	{
		...
		$this->db->select("igain_giftcard_tbl.Gift_card_id,igain_giftcard_tbl.Card_balance,igain_giftcard_tbl.Valid_till,igain_giftcard_tbl.Discount_percentage,B.Company_merchandize_item_code");
