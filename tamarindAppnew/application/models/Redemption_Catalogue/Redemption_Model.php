...
{
	public function get_all_items($limit,$start,$Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Merchandize_item_type) 
	{
		$Todays_date=date("Y-m-d");
		$this->db->from('igain_company_merchandise_catalogue as A');	
		if($Merchandize_category_id==0)//ALL
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_item_type'=>$Merchandize_item_type,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Ecommerce_flag'=> 0,'A.Link_to_Member_Enrollment_flag'=> 0,'A.Send_once_year'=> 0,'A.Send_other_benefits'=> 0,));
		}
		else
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_item_type'=>$Merchandize_item_type,'A.Merchandize_category_id'=>$Merchandize_category_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'A.Ecommerce_flag' => 0,'A.Link_to_Member_Enrollment_flag' => 0,'A.Send_once_year' => 0,'A.Send_other_benefits' => 0));
		}
	...
	public function get_search_items($limit,$start,$Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Merchandize_item_type,$Serach_key) 
	{
		$Todays_date=date("Y-m-d");
		$this->db->from('igain_company_merchandise_catalogue as A');	
	...
	function Get_Merchandize_Items_Count($Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Merchandize_item_type)
	{
		$Todays_date=date("Y-m-d");
		...
		if($Merchandize_category_id==0)//ALL
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_item_type'=>$Merchandize_item_type,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'Ecommerce_flag'=> 0,'Link_to_Member_Enrollment_flag'=> 0,'Send_once_year'=> 0,'Send_other_benefits'=> 0));
		}
		else
		{
			$this->db->where(array('A.Company_id'=>$Company_id,'A.Merchandize_item_type'=>$Merchandize_item_type,'A.Merchandize_category_id'=>$Merchandize_category_id,'A.Valid_from <='=>$Todays_date,'A.Valid_till >='=>$Todays_date,'A.show_item'=>1,'A.Active_flag'=>1,'Ecommerce_flag' => 0,'Link_to_Member_Enrollment_flag' => 0,'Send_once_year' => 0,'Send_other_benefits' => 0));
		}
		if($Sort_by_merchant!=0)//ALL merchants
		{
			$this->db->where(array('A.Seller_id'=>$Sort_by_merchant));
	...
	function get_total_redeeem_points($enroll)
	{
		...
		$this->db->join('igain_enrollment_master as D', 'B.Seller_id=D.Enrollement_id','left');
	...
	function Get_Merchandize_Category($Company_id,$Merchandize_item_type)
	{
		...
		$this->db->where(array('A.Active_flag'=>1,'A.Company_id'=>$Company_id,'C.Merchandize_item_type'=>$Merchandize_item_type));
...
/**************************************Nilesh start***********************************/
	...
	public function fetch_max_price($Company_id,$Merchandize_item_type)    
	{
		$this->db->select_max('Billing_price_in_points');
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Company_id' => $Company_id,'Merchandize_item_type' => $Merchandize_item_type,'show_item'=>1,'Active_flag'=>1,'Ecommerce_flag'=>0)); 
	...
	public function fetch_min_price($Company_id,$Merchandize_item_type)    
	{
	...
	public function get_all_products($Company_id,$Merchandize_item_type)
	{
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Company_id' => $Company_id,'Merchandize_item_type' => $Merchandize_item_type));
		$sql = $this->db->get();
	...
	function Get_Merchandize_item_brand($Company_id,$Merchandize_item_type)
	{
		$this->db->select('Item_Brand');
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Active_flag'=>1,'Brand_flag'=>1,'Company_id'=>$Company_id,'Merchandize_item_type'=>$Merchandize_item_type));
		$this->db->group_by("Item_Brand");
		$sql=$this->db->get();
		if($sql->num_rows()>0)
		{
	...
	function Get_Merchandize_item_Gender_flag($Company_id,$Merchandize_item_type)
	{
		$this->db->select('Gender_flag');
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Active_flag'=>1,'Company_id'=>$Company_id,'Merchandize_item_type'=>$Merchandize_item_type));
		$this->db->group_by("Gender_flag");
		$sql=$this->db->get();
		if($sql->num_rows()>0)
		{
	...
	function Get_item_details_size($Company_id,$Company_merchandize_item_code)
	{
        ...
        function Get_item_details_by_code($Company_id,$Company_merchandize_item_code)
	{
		$this->db->select('*');
		$this->db->from('igain_company_merchandise_catalogue');
		$this->db->where(array('Company_id'=>$Company_id,'Company_merchandize_item_code'=>$Company_merchandize_item_code));
