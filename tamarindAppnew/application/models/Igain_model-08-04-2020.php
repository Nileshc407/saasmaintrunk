...
class Igain_model extends CI_Model
{
	...
	function FetchSellerCommunication_App($Company_id,$enroll,$sellerID)
	{  
		$Todays_date=date("Y-m-d"); 
		$this->db->select("DISTINCT(Communication_id), communication_plan,description,Offer_description1,First_name,Last_name,igain_cust_notification.Company_id,Photograph,Birthday_flag");
		$this->db->from('igain_cust_notification');
		$this->db->join('igain_seller_communication', 'igain_cust_notification.Communication_id = igain_seller_communication.Id');
		$this->db->join('igain_enrollment_master', 'igain_cust_notification.seller_id = igain_enrollment_master.Enrollement_id');
		...
		if($sellerID != 0){
			$this->db->where("igain_seller_communication.seller_id",$sellerID);
		}
		$this->db->order_by('igain_cust_notification.Id', 'DESC');
	...
	{ 
		$this->db->select("B.communication_plan,B.description,CONCAT(A.First_name, ' ', A.Last_name) as Merchant_name,A.Photograph,C.Offer_description1");
		$this->db->from('igain_seller_communication as B');
		$this->db->join('igain_enrollment_master as A', 'B.seller_id = A.Enrollement_id');
		$this->db->join('igain_cust_notification as C', 'B.id = C.Communication_id');
		// 
		
		
		$this->db->where(array('B.Company_id' => $Company_id));
		$this->db->where_in('B.id', $Communication_id);
		$this->db->group_by('C.Communication_id');
...
/********************************************Akshay Start*******************************/
	...
	public function Search_outlets($Company_id,$Serach_key)
	{
		$this->db->select("*");
	  $this->db->from('igain_enrollment_master');
		  $this->db->where(array('User_id' => '2','User_activated' => '1','Super_seller' => '0','Sub_seller_admin' => '1','Sub_seller_Enrollement_id' => '0','Company_id' => $Company_id));
	...
	{	
		$this->db->where(array('Enrollement_id' => $Enrollement_id, 'Company_id' => $Company_id, 'Seller_id' => $Seller_id, 'Confirmation_code' => $Confirmation_code, 'Confirmation_flag' => 0)); 
	...
	function FetchSellerdetails_google_map($Company_id,$Cust_lat,$Cust_long,$sellerID)
	{
		...
		/* -----Ravi---02-12-2019---'Sub_seller_Enrollement_id' => '0',-- */
			...
			if($sellerID != 0){
				...
				$this->db->where('Sub_seller_Enrollement_id',$sellerID);
			} else {
				$this->db->where('Sub_seller_Enrollement_id',0);
			}
			$this->db->order_by('Enrollement_id', 'DESC');
			$SubsellerSql = $this->db->get();
			// echo $this->db->last_query();
	...
	function check_gift_card_id($gift_cardid,$Company_id)
	{
	 ...
	 function get_discount_value($Itemcategory_id,$ItemCode,$Item_price,$Company_id,$delivery_outlet,$cust_enroll,$Tier_id,$grandTotal)
	{
		...
		//		echo "for Discount_id  ".$row->Discount_id."for ItemCode  ".$ItemCode."--in discount_Amt  ".$discount_Amt."--<br>";
			...
			/* echo "<br>---validTillalid_till--".$validTill;
			echo "<br>--in total voucherAmt  ".$discountVoucherAmt."--<br>";
			echo "<br>--in total DiscountAmt  ".$discountAmt."--<br>";  */
	...
	function FetchSubSellerdetails($Company_id,$Seller_id)
	{
	  $this->db->select("*");
	  $this->db->from('igain_enrollment_master');
	  $this->db->where(array('User_id' => '2','User_activated' => '1','Super_seller' => '0','Sub_seller_admin' => '0','Sub_seller_Enrollement_id' =>$Seller_id,'Company_id' => $Company_id));
	  $this->db->order_by('Enrollement_id', 'DESC');
	  // $this->db->limit('4');
	  $SubsellerSql = $this->db->get(); 
	  // echo $this->db->last_query()."<br>";
