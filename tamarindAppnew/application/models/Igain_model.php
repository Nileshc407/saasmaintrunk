...
class Igain_model extends CI_Model
{
	...
	function FetchSellerdetails($Company_id)
	{
	  ...
	  // $this->db->order_by('Enrollement_id', 'DESC');
	  $ids = "130, 134, 135, 131,132, 133";
	  $this->db->order_by("FIELD(Enrollement_id,130, 134, 135, 131,132, 133)");
	  // $this->db->limit('4');
	  $SubsellerSql = $this->db->get(); 
...
/********************************************Akshay Start*******************************/
	...
	function Get_my_gift_cards($Enrollment_id,$Company_id,$Card_id)
	{
		...
		$this->db->from('igain_giftcard_tbl as T');
		$this->db->join('igain_enrollment_master as IE', 'T.Card_id = IE.Card_id','LEFT');
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id, 'T.Card_balance >' => 0)); // 'T.Card_balance >' => 0
	...
	/**************Ravi Gmap- 05-10-2020***************/
	function get_discount_vouchers($Enrollment_id,$Company_id,$Card_id)
	{
		$this->db->select("T.*");
		$this->db->from('igain_giftcard_tbl as T');
		$this->db->join('igain_enrollment_master as IE', 'T.Card_id = IE.Card_id','LEFT');
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id)); // 'T.Card_balance >' => 0
		$this->db->where('T.Discount_percentage > ', 0);
		$this->db->where_in('T.Payment_Type_id', array('99'));
		$this->db->order_by('T.id','DESC');
		$sql = $this->db->get();
		// echo $this->db->last_query(); die;
		if($sql->num_rows() > 0)
	...
	function get_value_vouchers($Enrollment_id,$Company_id,$Card_id)
	{
		$this->db->select("T.*");
		$this->db->from('igain_giftcard_tbl as T');
		$this->db->join('igain_enrollment_master as IE', 'T.Card_id = IE.Card_id','LEFT');
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id, 'T.Discount_percentage' => 0)); // 'T.Card_balance >' => 0
		// $this->db->where('T.Discount_percentage ', 0);
		$this->db->where_in('T.Payment_Type_id', array('99'));
		$this->db->order_by('T.id','DESC');
		$sql = $this->db->get();
	...
	function Fetch_brand_outlet_google_map($Company_id,$Brand_id)
	{		
		...
		/* -----Ravi---02-12-2019------ */
			...
			$this->db->select("*");
			$this->db->from('igain_enrollment_master');
			$this->db->where(array('User_id' => '2','User_activated' => '1','Super_seller' => '0','Sub_seller_admin' => '0','Sub_seller_Enrollement_id ' => $Brand_id,'Company_id' => $Company_id));
			$this->db->order_by('Enrollement_id', 'ASC');
			$SubsellerSql = $this->db->get();
			if ($SubsellerSql->num_rows() > 0)
			{
	...
	function inert_verification_code($Company_id,$ValidateCode,$Enrollement_id)
	{
		$data['Company_id'] = $Company_id;
		$data['Enrollment_id'] = $Enrollement_id;
		$data['Reference_code'] = $ValidateCode;
		$data['Active_flag'] = 1;
		$data['Create_date'] = date('Y-m-d');
		$this->db->insert('igain_reference_code', $data);
		if($this->db->affected_rows() > 0)
		{
			return true;
		}
	...
	function check_verification_code($Company_id,$Enrollement_id)
	{
		$this->db->select("*");
		$this->db->from('igain_reference_code');
