...
/********************************************Akshay Start*******************************/
	 ...
	 function get_discount_value($Itemcategory_id,$ItemCode,$Item_price,$Company_id,$delivery_outlet,$cust_enroll,$Tier_id,$grandTotal)
	{
			...
			//		echo "in seller ".$step."<br>";
				...
				//	echo "in opretorFlag ".$opretorFlag."<br>";
					...
					if( $opretorFlag > 0)
					{
						...
						{
							/********Ravi-11-04-2020*********/
								$DiscountRuleSet=$row->Discount_rule_for;
		...
		//		echo "for Discount_id  ".$row->Discount_id."for ItemCode  ".$ItemCode."--in discount_Amt  ".$discount_Amt."--<br>";
		 ...
		 // echo "<br>--in total DiscountPercentageValue  ".$DiscountPercentageValue."--<br>";
		 
			/* return	json_encode(array("DiscountAmt"=>number_format(floor($discountAmt),2),"discountsArray"=>$discountsArray)); */
			
			/********Ravi-11-04-2020*********/
				return	json_encode(array("DiscountAmt"=>number_format(floor($discountAmt),2),"discountsArray"=>$discountsArray,"DiscountPercentageValue"=>$DiscountPercentageValue,"DiscountRuleSet"=>$DiscountRuleSet));
			/********Ravi-11-04-2020*********/
	...
	public function GetStatementAndTrans($Company_id,$enroll_id,$Card_id)
	{
		...
		$this->db->from("igain_transaction as A");
		$this->db->where(array('A.Company_id'=>$Company_id, 'A.Enrollement_id'=>$enroll_id));
		$this->db->where("A.Trans_type IN('2','12')");
		$this->db->join("igain_enrollment_master as B", "A.Seller=B.Enrollement_id");
		$this->db->order_by('A.Trans_id','desc');
