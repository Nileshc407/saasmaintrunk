...
class Cust_home extends CI_Controller {
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			/*------------------Qr Code-------------------
					$params['data'] =$session_data['Card_id'];
					$params['level'] = 'H';
					$params['size'] = 5;
					$params['savename'] = FCPATH . 'qr_code_profiles/' . $data['enroll'] . 'profile.png';
					$QrCode_image = $this->ciqrcode->generate($params);
			...
			$string = $session_data['Card_id'];
			$this->set_barcode($string); 
			
			/* 24-12-2020 */
				$data['Verification_code'] = $this->Igain_model->check_verification_code($session_data['Company_id'],$session_data['enroll']);
			/* 24-12-2020 */
	...
	{		
		if($this->session->userdata('cust_logged_in')) {
			...
			$store_image = imagepng($file,FCPATH ."qr_code_profiles/".$data['enroll']."_"."{$code}.png");
			return $code.'.png';
			...
			if($_POST==null){
			 ...
			 /*  $Enrollment_id = $session_data['enroll'];
                ...
                if ($userEmailId != $Current_email) {
					...
					$result = $this->Igain_model->update_profile($post_data7, $Enrollment_id);
                ...
                if ($result == true) {
				 ...
				 // $update_profile = $this->Igain_model->update_profile($post_data,$Enrollement_id)
	...
	function Current_offers() 
	{
        if ($this->session->userdata('cust_logged_in')) {
            ...
            // $this->load->view('front/offers/offer', $data);		
            $this->load->view('front/offers/Current_offers', $data);
        } else {
            redirect('login', 'refresh');
        }
    }
    ...
    function contactus_App() {
        if ($this->session->userdata('cust_logged_in')) {
            ...
            $data["Enroll_details"] = $this->Igain_model->get_enrollment_details($data['enroll']);
            $Enroll_details12 = $this->Igain_model->get_enrollment_details($data['enroll']);
            $data["Brand_details"] = $this->Igain_model->FetchSellerdetails($data['Company_id']);
	...
	function giftcard() {	
		...
		if ($this->session->userdata('cust_logged_in')) {
            ...
            $this->load->view('front/home/gift_card', $data);
	...
	function discount_vouchers() {	
		...
		if ($this->session->userdata('cust_logged_in')) {
			...
			$data["DiscountVouchers"] = $this->Igain_model->get_discount_vouchers($data['enroll'],$data['Company_id'],$data['Card_id']);

			$data["MyGiftCard"] = $this->Igain_model->Get_my_gift_cards($data['enroll'],$data['Company_id'],$data['Card_id']);

            $this->load->view('front/home/discount_vouchers', $data);
        } else {
            redirect('login', 'refresh');
        }
		
	}
	function value_vouchers() {	
		...
		if ($this->session->userdata('cust_logged_in')) {
			...
			$data["MyValueVouchers"] = $this->Igain_model->get_value_vouchers($data['enroll'],$data['Company_id'],$data['Card_id']);

			$data["MyGiftCard"] = $this->Igain_model->Get_my_gift_cards($data['enroll'],$data['Company_id'],$data['Card_id']);

            $this->load->view('front/home/value_vouchers', $data);
        } else {
            redirect('login', 'refresh');
        }
		...
		if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if(preg_match("/^[0-9]{9}+$/", $Phone_no)) {
				...
				if(strpos($password, '#') !== false || strpos($password, '%') !== false || strpos($password, '&') !== false || strpos($password, '+') !== false || strpos($password, '=') !== false ){
					...
					echo json_encode(array("status" => "1008", "Status" => "Password should not allow #,%,&, + and = characters"));
					exit;
				}			
				if($password_len < 6 ){
					
					echo json_encode(array("status" => "1003", "Status" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_upper_char != 1 ){
					
					echo json_encode(array("status" => "1004", "Status" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_lower_char != 1 ){
					
					echo json_encode(array("status" => "1005", "Status" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_special_char != 1 ){
					
					echo json_encode(array("status" => "1006", "Status" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}
				if($password_numaric != 1 ){
					
					echo json_encode(array("status" => "1007", "Status" => "Password should be minimum of 6 characters with one number, one special, one upper and one lower case letter"));
					exit;					
				}				
				...
				if($matchepwd != 0){
					...
					echo json_encode(array("status" => "1009", "Status" => "Password and confirm password don't match"));
					exit;
					
				} else if ($flag == NULL && $CompanyID == NULL && $email == NULL && $Phone_no == NULL) {
					
					echo json_encode(array("status" => "1010", "Status" => 'Provide valid information. Please
					try again'));
					exit;
					   ...
					   // echo "---Currentyear-----".$Currentyear."---<br>";
						...
						if ($YearDiff < 18 ) {
							echo json_encode(array("status" => "1011", "Status" => 'Must be 18+ to be eligible for Loyalty'));
							exit;
						 } */                  
						...
						if ($Check_EmailID == 0) {
							...
							if ($CheckPhone_number == 0) {
								...
								if ($Last_enroll_id > 0) {
									...
									echo json_encode(array("status" => "1012", "Get_member_details" => $Return_json_response));
								} else {
									echo json_encode(array("status" => "1013", "Status" => 'Some thing went wrong. Please try again.'));
									exit;
								}
							} else {
								echo json_encode(array("status" => "1014", "Status" => 'The provided phone number already exists. Please try another phone number'));
								exit;
							}
						} else {
							echo json_encode(array("status" => "1015", "Status" => 'The provided email address already exists. Please try another email address'));
							exit;
						}
					} else {
					   
						echo json_encode(array("status" => "1016", "Status" => 'Provide valid information. Please try again'));
						exit;
				...
				echo json_encode(array("status" => "1002", "Status" => 'Enter valid phone number'));
				exit;
			...
			echo json_encode(array("status" => "1001", "Status" => 'Please enter a valid email address'));
			exit;
	   ...
	   if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if ($flag == 1 || $email == "" || $Company_id == "") {
				...
				if($result != NULL || $result > 0) {
					...
					echo json_encode(array("status" => "1022", "Status" => "Login credentials are sent to your email. Please check."));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1023", "Status" => "The provided email doesn't match our records"));
					exit;
				}
				
			} else {
				
				echo json_encode(array("status" => "1019", "Status" => "Enter valid email address"));
				exit;
			}
		} else {
	...
	/******************Nilesh c end 07-04-2020**************************/
	/*******************Nilesh start purchase gift card 15-05-2020*****************************/
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			foreach($Company_details as $Company)
			{
				...
				$data["Points_used_flag"] = $Company['Points_used_gift_card'];	
				$data["Min_amount"] = $Company['Minimum_gift_card_amount'];	
			}
			$Country_details = $this->Igain_model->get_dial_code($Country);
			...
			$data['Mpesa_auth_key'] = $Enroll_details->Mpesa_auth_key;
			$data["Company_Details"]= $this->Igain_model->get_company_details($session_data['Company_id']);
	...
	public function CheckoutGiftCardPayment()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data['goods_till_number'] = $Enroll_details->goods_till_number;
			$data['Seller_api_url2'] = $Enroll_details->Seller_api_url2;
			$Seller_api_url2 = $Enroll_details->Seller_api_url2;
			$payment_auth_key = $Enroll_details->Mpesa_auth_key;
			...
			$Current_balance = $Enroll_details1->Current_balance;
			$Total_reddems = $Enroll_details1->Total_reddems;
			...
			/**********************Nilesh VerifyToken 09-07-2020****************************/	
				...
				if($Paid_amount > 0 && ($Seller_api_url2 != "" || $Seller_api_url2 != NULL))
				{
					$Mpesa_TransID = $Mpesa_TransID;
					$Payment_type_id = 5; //Mobile Money
					$Paid_by = "Mobile Money";
					$Paid_amount = $_REQUEST['Paid_by_MPESA'];
					
					$Mpesa_Total_Paid_Amount = filter_var($Paid_amount, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);//1989.34
			
			
					$input_args3 = Array('TransID' => $Mpesa_TransID ,'BalanceDue' => number_format($Mpesa_Total_Paid_Amount,2),'PaidAmount' => number_format($Mpesa_Total_Paid_Amount,2),'BillRefNumber' => $bill,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
				...
				else if($CCDapproval != Null)
				{
					$Mpesa_TransID = $TransactionToken;
					$Payment_type_id = 3; //Credit/Debit Card
					$Paid_by = "DPOCustomerCreditType1";
					$Paid_amount = $DPOTransFinalAmount1;
				}
				else
				{
					$Payment_type_id = 4; //Redeem Points
					$Paid_by = "Points";
					$Paid_amount = 0.00;
				}
				...
				$giftData["Card_balance"] = $_REQUEST['Gift_card_amt'];
				$giftData["Card_value"] = $_REQUEST['Gift_card_amt'];
				...
				$data123 = array('Company_id' => $data['Company_id'],
									...
									'Purchase_amount' => $_REQUEST['Gift_card_amt'],
									'Paid_amount' => $Paid_amount,
									'Mpesa_Paid_Amount' => $Paid_amount,
									'Mpesa_TransID' => $Mpesa_TransID,
									'Redeem_points' => $_REQUEST['point_redeem'],
									'Redeem_amount' => $_REQUEST['redeem_amt'],
									...
									'Credit_Cheque_number' => $DPOTransactionApproval1, 
				...
				$Email_content = array(
					  ...
					  'GiftCardNo' => $gift_cardid,
					  'GiftCard_balance' => number_format($_REQUEST['Gift_card_amt'],2),
					  ...
					  'Paid_by_MPESA' => $_REQUEST['Paid_by_MPESA'],
					  'Paid_by_CreditCard' => $DPOTransFinalAmount1,
					  'Redeem_points' => $_REQUEST['point_redeem'],
					  'Redeem_amount' => $_REQUEST['redeem_amt'],
				...
				$Update_Current_balance = ($Current_balance - $_REQUEST['point_redeem']);
							
				$Update_total_reddems = $Total_reddems + $_REQUEST['point_redeem'];
				$up = array('Current_balance' => $Update_Current_balance, 'Total_reddems' => $Update_total_reddems);
												
				$this->Shopping_model->update_cust_balance($up,$Card_id,$data['Company_id']);

				$this->session->set_flashdata("error_code","Gift Card Buy successfully!!");
			}
			else	
			{
			...
			$data['gift_amt'] = number_format($_REQUEST['Gift_card_amt'],2);
	...
	public function CreateToken()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$Super_seller_flag = 1;
			$result = $this->Shopping_model->Get_Seller($Super_seller_flag,$session_data['Company_id']);				   
			$Seller_id = $result->Enrollement_id;
			
			$Order_outlet = $Seller_id;
			...
			$RedirectURL = base_url().'index.php/Cust_home/CheckoutGiftCard';
			$BackURL = base_url().'index.php/Cust_home/CheckoutGiftCard';
			...
			$ServiceDescription = "Buy Gift Card";
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$this->load->view('front/home/restaurants', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	{ 
		if($this->session->userdata('cust_logged_in'))
		{
			...
			// $data["Fetch_Sub_Seller_details"] = $this->Igain_model->Fetch_Sub_Seller_details($data['Company_id'],$Brand_id);
			
			
			// $data['Sellerdetails'] = $this->Igain_model->Fetch_Seller_outlet_details($data['Company_id']);
			$data['Sellerdetails'] = $this->Igain_model->Fetch_Sub_Seller_details($data['Company_id'],$Brand_id);
			...
			/**************Ravi Gmap- 21-11-2019*************** use only for google Map **/
				...
				$data["Seller_details"] = $this->Igain_model->Fetch_brand_outlet_google_map($session_data['Company_id'],$Brand_id);	
			/**************Ravi Gmap- 21-11-2019*************** use only for google Map **/	
			
			
			//var_dump($data["Fetch_Sub_Seller_details"]);			
			$this->load->view('front/home/restaurant-map', $data);
		}
		else
		{
			redirect('Login', 'refresh');
	...
	function Update_img() 
	{
        if ($this->session->userdata('cust_logged_in')) {
			...
			if($profile_update_flag == 1) 
			{
				$this->session->set_flashdata("error_code", "Photo not uploaded, as the Image Size exceeded the dimension 1000 X 1000 pixels!!");
				redirect('Cust_home/Update_img');
			} 
			else{
				$this->session->set_flashdata("error_code", "Profile photo updated successfully");
				redirect('Cust_home/myprofile');
			}
	...
	function verify_email_smart_phone() {
		...
		if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
				...
				//Email is valid
				/* echo json_encode(array("status" => "1002", "Status" => "The provided email valid"));
                exit; */
		
			if ($flag == 1 && !empty($email) && $Company_id != "") {

				$enc_email = App_string_encrypt($email);
				$result = $this->Igain_model->Check_EmailID($enc_email,$Company_id);
				if($result > 0) {               

					echo json_encode(array("status" => "1021", "Status" => "The provided email address already exists in the system, Please try another email"));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1020", "Status" => "The provided email is available"));
					exit;
				}
			} else {
				echo json_encode(array("status" => "1019", "Status" => "Enter valid email address"));
	...
	function verify_phone_smart_phone() {
		...
		if(preg_match("/^[0-9]{9}+$/", $Phone_no)) {
			...
			if ($is_numeric == true && $flag == 1 && !empty($Phone_no) && strlen($Phone_no) == 9 && $Company_id != "") {
				...
				if($ValidatePhone_number > 0) {               
					...
					echo json_encode(array("status" => "1014", "Status" => "The provided phone number already exists in the system, Please try another phone number"));
					exit;
					
				} else {
					
					echo json_encode(array("status" => "1017", "Status" => "The provided phone number is available"));
					exit;
				}
				
			} else {
				echo json_encode(array("status" => "1018", "Status" => "Enter valid phone number"));
				exit;
			}
	...
	function Send_gift_card() {
        if ($this->session->userdata('cust_logged_in')) {
            ...
            if ($_POST != NULL) {
                ...
                $input_email = App_string_encrypt($this->input->post('input_email'));
                $Login_Enrollement_id = $this->input->post('Login_Enrollement_id');
                $input_Enrollement_id = $this->input->post('input_Enrollement_id');
                $Send_member = $this->input->post('Send_member');
                $Phone_no = App_string_encrypt($this->input->post('Phone_no'));
                $Send_CARD = $this->input->post('Send_CARD');

              // echo "<br>Gift_card ".$Gift_card;
              // echo "<br>input_email ".$input_email;die;
                        ...
                        $logtimezone = $session_data['timezone_entry'];
                            ...
                            $result = $this->Igain_model->Update_giftcard_transaction($Company_id,$input_email,$Gift_card,$lv_date_time,$Login_Enrollement_id,$Send_member);

                            $result45 = $this->Igain_model->insert_Giftcard_transction($Gift_card,$Company_id,$lv_date_time,$Login_Enrollement_id,$input_email,$Send_member,$Phone_no,$Send_CARD);
							
							$giftcard_details = $this->Igain_model->get_giftcard_balance($Gift_card,$Company_id);
							
                               $Email_content = array(
                                    'Notification_type' => 'You have Sent Gift Card ('.$Gift_card.') to '. $Send_member,
                                    'Gift_card' => $Gift_card,
                                    'User_email_id' => App_string_decrypt($data["Enroll_details"]->User_email_id),
                                    'Enrollement_id' => $data["Enroll_details"]->Enrollement_id,
                                    'Customer_name' => $data["Enroll_details"]->First_name.' '.$data["Enroll_details"]->Last_name,
                                    'Transferred_to' => $Send_member,
                                    'Transferred_email' => $this->input->post('input_email'),
                                    'Card_value' => $giftcard_details[0]->Card_value,
                                    'Card_validity' => $giftcard_details[0]->Valid_till,
                                    'Template_type' => 'Send_gift_card'
                                );
                                $this->send_notification->send_Notification_email($Login_Enrollement_id, $Email_content, '0', $Company_id);
								
                               $Email_content2 = array(
                                    'Notification_type' => 'You have Received Gift Card ('.$Gift_card.') from '. $data["Enroll_details"]->First_name.' '.$data["Enroll_details"]->Last_name,
                                    'Customer_name' => $Send_member,
									'User_email_id' => $this->input->post('input_email'),
									'Enrollement_id' => $this->input->post('input_Enrollement_id'),
                                    'Gift_card' => $Gift_card,
                                    'Transferred_to' => $data["Enroll_details"]->First_name.' '.$data["Enroll_details"]->Last_name,
                                    'Transferred_email' => App_string_decrypt($data["Enroll_details"]->User_email_id),
                                    'Card_value' => $giftcard_details[0]->Card_value,
                                    'Card_validity' => $giftcard_details[0]->Valid_till,
                                    'Template_type' => 'Send_gift_card'
                                );
                                $this->send_notification->send_Notification_email($Login_Enrollement_id, $Email_content2, '0', $Company_id);
                                ...
                                $opval = $Gift_card;
                                $Todays_date = date("Y-m-d");
                                $firstName = $Send_member;
                                $lastName = '';
                                $Enrollment_id = $data["Enroll_details"]->Enrollement_id;
                                $LogginUserName = $data["Enroll_details"]->First_name . ' ' . $data["Enroll_details"]->Last_name;
                                $result_log_table = $this->Igain_model->Insert_log_table($session_data['Company_id'], $session_data['enroll'], $session_data['username'], $LogginUserName, $Todays_date, $what, $where, $userid, $opration, $opval, $firstName, $lastName, $Login_Enrollement_id);
                                /****************igain Log Table change 28-06-2017****************/
								
                                $data["MColor"] = "#41ad41";
                                $data["Img"] = "success";
                                $data["Success_Message"] = "Your Gift Card ($Gift_card) sent successfully !!! ";
                                $this->load->view('front/transferpoints/success', $data);
                            }
                        else {
							$this->load->view('front/home/Send_gift_card', $data);
						}	
			}
		 else {
            redirect('login', 'refresh');
	...
	public function Check_email_giftcard() {
        $session_data = $this->session->userdata('cust_logged_in');
        $data['Country_id'] = $session_data['Country_id'];
		...
		$input_email = App_string_encrypt($input_email); 
		
        if ($input_email == $enrollment_details->User_email_id) {
            $this->output->set_output("0");
        } else {
            $result = $this->Igain_model->fetch_giftcard_email_details($this->input->post("Company_id"), $input_email);
	...
	public function validate_code() {
        ...
        if ($this->session->userdata('cust_logged_in')) {
			...
			$ValidateCode= strtoupper($this->input->post("ValidateCode"));
			$TTREWARDS_code='TTREWARDS';
			$matchepwd= strcmp($ValidateCode,$TTREWARDS_code);
           if (!empty($ValidateCode)) {
			   
				if($matchepwd != 0){
					
					echo json_encode(array("status" => "1030", "Message" => "In-Valid Verification Code"));
					exit;
					
				} else {
					
					$inert_verification_code = $this->Igain_model->inert_verification_code($Company_id,$ValidateCode,$Enrollement_id);

					if($inert_verification_code) {               

						echo json_encode(array("status" => "1027", "Message" => "Verification Successful. Please wait"));
						exit;
						
					} else {
						
						echo json_encode(array("status" => "1028", "Message" => "Verification Not Successful. Please try later"));
						exit;
					}
				}
				...
				echo json_encode(array("status" => "1029", "Message" => "Enter Verification code"));
				exit;
			}
        }
    }
	...
	/* 27-01-2021 */
	 function get_current_point_balance(){
		 ...
		 if ($this->session->userdata('cust_logged_in')) {
			...
			echo json_encode(array("status" => "1012", "Current_balance" => floor($Current_point_balance)));
			
		} else {
			
			echo json_encode(array("status" => "1013", "Message" => 'Some thing went wrong. Please try again.'));
			exit;
		}
	 }
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			// echo "<br>pin-----".$pin;
			// echo "<br>Pin_number_validity-----".$Pin_number_validity;
			// echo "<br>currentTime-----".$currentTime;
			// echo "<br>cenvertedTime-----".$cenvertedTime;
			// exit;
            $post_data = array(
                        'pinno' => $pin,
            ...
            $this->load->view('front/dashboard/Show_pin', $data);
        } else {
            redirect('login', 'refresh');
        }
    }
