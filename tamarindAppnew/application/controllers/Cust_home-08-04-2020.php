...
class Cust_home extends CI_Controller {
    ...
    function __construct() {
		...
		// var_dump($_SESSION['sellerID']);
		// var_dump($_REQUEST['sellerID']);
		// echo base_url(uri_string());
		
		
		
		if($_SESSION['sellerID'] !=0 ){
			
			$sellerID=$_SESSION['sellerID'];
			
		} else {
			
			$_SESSION['sellerID'] =$_REQUEST['Seller_id'];	
			$sellerID=$_SESSION['sellerID'];
		}
		$url=$this->router->fetch_class().'/'.$this->router->fetch_method();
		
		
		if($url == 'Cust_home/front_home'){
			// echo"---url----".$url."---<br>";
			$_SESSION['sellerID']=0;
			$sellerID=0;
		} 
		
		if($_SESSION['sellerID'] != "" || $_SESSION['sellerID'] = 0 ){
			$sellerID=$_SESSION['sellerID'];
		} else {
			$sellerID=0;
		}
		
		// echo"---session ---sellerID----".$_SESSION['sellerID']."---<br>";

		
		
		
		// echo"---sellerID--111---".$sellerID."---<br>";
		
		// die;
        $General_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'General', 'value', $Company_id,$sellerID);
        $this->General_details = json_decode($General_data, true);
		
		// echo"---General_data--".$this->db->last_query()."---<br>";
      
        $Small_font = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Small_font', 'value', $Company_id,$sellerID);
        $this->Small_font_details = json_decode($Small_font, true);

        $Medium_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Medium_font', 'value', $Company_id,$sellerID);
        $this->Medium_font_details = json_decode($Medium_font_data, true);

        $Large_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Large_font', 'value', $Company_id,$sellerID);
        ...
        $Extra_large_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Extra_large_font', 'value', $Company_id,$sellerID);
        $this->Extra_large_font_details = json_decode($Extra_large_font_data, true);

        $Button_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Button_font', 'value', $Company_id,$sellerID);
        $this->Button_font_details = json_decode($Button_font_data, true);

        $Value_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Value_font', 'value', $Company_id,$sellerID);
        $this->Value_font_details = json_decode($Value_font_data, true);

        $Footer_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Footer_font', 'value', $Company_id,$sellerID);
        $this->Footer_font_details = json_decode($Footer_font_data, true);

        $Placeholder_font_data = $this->General_setting_model->get_type_name_by_id('frontend_settings', 'Placeholder_font', 'value', $Company_id,$sellerID);
	...
	public function front_home() {
        ...
        /* if ($this->session->userdata('cust_logged_in')) {
		 ...
		 if ($this->session->userdata('cust_logged_in')) {
			...
			// -----------------------------23-03-2020------------------------------------ //
				$Country_details = $this->Igain_model->get_dial_code($Company_Details->Country);
				$data['Currency_Symbol'] = $Country_details->Symbol_of_currency;
				
				$data["MyDiscountVouchers"] = $this->Igain_model->Get_my_discount_vouchers(0,0,$data['enroll'],$data['Company_id'],$data['Card_id']);
			// -----------------------------23-03-2020------------------------------------ //
			
			$discountVoucherCount=array();
			$discountVoucherWorth=array();
			
			$totalVouchers=0;
			$totalAmount=0;
			
			$i=0;
			if($data["MyDiscountVouchers"]){
				
				
				foreach($data["MyDiscountVouchers"] as $MyVouchers){
					
					
					
					if($MyVouchers->Card_balance > 0){ 
					
						// echo"--Gift_card_id--".$MyVouchers->Gift_card_id."---Card_value--".$MyVouchers->Card_value."--<br>";
						$discountVoucherWorth[]=$MyVouchers->Card_value;
						// $discountVoucherCount[]=$MyVouchers->Gift_card_id;
						$i++;
					}
			...
			$totalVouchers=$i;
			$totalAmount=array_sum($discountVoucherWorth);
			
			
			$data["totalVouchers"]=$totalVouchers;
    ...
    function merchantoffers() {
        if ($this->session->userdata('cust_logged_in')) {
            ...
            $data['MerchantCommunication'] = $this->Igain_model->FetchSellerCommunication_App($data['Company_id'], $data['enroll'],$sellerID);
    ...
    function MerchantCommunication() {
        if ($this->session->userdata('cust_logged_in')) {
			...
			{
				$sellerID=$_REQUEST["Sort_by_merchant_flag"];
			} */
			
			
			// echo"-----session---seller--1---".$_SESSION['sellerID']."---<br>";
			if($_SESSION['sellerID'] != "" || $_SESSION['sellerID'] = 0 ){
				$sellerID=$_SESSION['sellerID'];
			} else {
				$sellerID=0;
    ...
    public function changepassword() {
        ...
        $result11 = $this->Igain_model->Check_Old_Password($old_Password, $this->input->post("Company_id"), $this->input->post("Enrollment_id"));
	...
	public function seller_dashboard() {
        ...
        if ($this->session->userdata('cust_logged_in')) {
			...
			$sellerID=$_SESSION['sellerID'];
			// echo "----sellerID----222--".$sellerID."---<br>";
			// die;
			
			if($sellerID==0){
			
				// echo "----('Cust_home/front_home', 'refresh')--<br>";
				redirect('Cust_home/front_home', 'refresh');
				...
				$data["Sub_Seller_details"] = $this->Igain_model->FetchSubSellerdetails($session_data['Company_id'],$sellerID);
			
				$this->load->view('front/home/seller_dashboard', $data);
			}
	...
	public function restaurant() {
        ...
        if ($this->session->userdata('cust_logged_in')) {
			...
			if($sellerID==0){
				...
				$this->load->view('front/home/restaurant', $data);
			}
