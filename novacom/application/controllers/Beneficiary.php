...
class Beneficiary extends CI_Controller 
{
    ...
    public function Added_Beneficiary_accounts()
	{
		...
		{			
			...
			$data["Get_Beneficiary_inactive_account"] = $this->Beneficiary_model->Get_Beneficiary_members_inactive_account($data['Company_id'],$Enrollment_id);
	...
	public function Beneficiary_Points_Transfer()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			{                                                
				...
				if($Purchase_miles > 0 && $Equivalent > 0 && $Login_Current_balance >= $Equivalent )
				{
                   ...
                   $opts = array('http' =>
					...
					$this->session->set_flashdata("error_code","Purchase Done Successfully.");
				}
				else {
                                    
                    // $this->load->view('front/transferacross/transfer_failed', $data);
					
					$this->session->set_flashdata("error_code","You have entered invalid data for purchase.");
			...
			redirect('Beneficiary/Load_buy_miles','refresh');
	...
	public function activate_Beneficiary_account()
    {
		if($this->session->userdata('cust_logged_in'))
		{
			$session_data = $this->session->userdata('cust_logged_in');
			...
			$Beneficiary_account_id=$_REQUEST["Beneficiary_account_id"];
			$Delete = $this->Beneficiary_model->activate_Beneficiary_account($Beneficiary_account_id);
			$this->session->set_flashdata("success","Member Account Activated Successfully !!!");
	...
	public function get_category_publisher()
    {	
		if($this->session->userdata('cust_logged_in'))
		{
			$session_data = $this->session->userdata('cust_logged_in');
			...
			$data['Card_id']= $session_data['Card_id'];
			$data['Company_id']= $session_data['Company_id'];
			$Publishers_category=$_REQUEST["CategoryID"];
			$Company_id=$_REQUEST["Company_id"];
			if($Publishers_category != "" && $Company_id != "") {
				
				$data["Get_Beneficiary_members"] = $this->Beneficiary_model->Get_Beneficiary_members($data['Company_id'],$Enrollment_id,$Publishers_category);
				
				//$this->load->view('front/transferacross/across_transferpts_done', $data);
				$theHTMLResponse = $this->load->view('front/transferacross/category_publisher', $data, true);				
				$this->output->set_content_type('application/json');
				$this->output->set_output(json_encode(array('CategoryPublisher'=> $theHTMLResponse)));
	...
	public function add_category_publisher()
    {	
		if($this->session->userdata('cust_logged_in'))
		{
			$session_data = $this->session->userdata('cust_logged_in');
			...
			if($Publishers_category != "" && $Company_id != "") {
				...
				$theHTMLResponse = $this->load->view('front/transferacross/add_category_publisher', $data, true);				
				$this->output->set_content_type('application/json');
				$this->output->set_output(json_encode(array('CategoryPublisher'=> $theHTMLResponse)));
	...
	public function get_to_publisher_account()
    { 
        if($this->session->userdata('cust_logged_in'))
        {			
            // $this->output->enable_profiler(TRUE);
           ...
           // $data["Get_Beneficiary_members"] = $this->Beneficiary_model->Get_Beneficiary_members($data['Company_id'],$Enrollment_id);
			...
			var_dump($_POST);
			die;
            if($_POST['To_Beneficiary_company_id'] == NULL) {
			   ...
			   $this->load->view('front/transferacross/load_beneficiary', $data);
				...
				$theHTMLResponse = $this->load->view('front/transferacross/to_publisher', $data, true);				
				$this->output->set_content_type('application/json');
				$this->output->set_output(json_encode(array('TOPublisher'=> $theHTMLResponse)));
