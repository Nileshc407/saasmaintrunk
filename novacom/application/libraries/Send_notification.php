    ...
    public function send_Notification_email($Enrollment_id,$Email_content,$Seller_id,$Company_id)
    {
		...
		{  
			...
			<TR>
			 ...
			 if($total_loyalty_points != 0 && $Loyalty_enabled == 1)
			 {
				$html .= '<TR>
        ...
        {
			$Communication_id = 0;
			$Offer = "Congratulations !!! You have recieved discount voucher from ".$company_details->Company_name;
			$Symbol_of_currency = $Email_content['Symbol_of_currency'];
			$Transaction_date = date("d M Y",strtotime($Email_content['Transaction_date']));
			$subject = "Congratulations !!! You have recieved discount voucher from ".$company_details->Company_name;			
		...
		{ 
			...
			<TR>
			 ...
			 if(count($Voucher_array) > 0)
				{
					$html .= '<TR>
					<TD style="border: #dbdbdb 1px solid;PADDING-BOTTOM: 4px; PADDING-LEFT: 4px; PADDING-RIGHT: 4px; PADDING-TOP: 4px" align=left colspan="2"> 																				<b>Note<span style="color:red"> * </span> : ('.implode(",",$Voucher_array).') this discount vouchers are used </b>					 	 </TD>                                                                                                                    
				</TR>';
				}
			 if($total_loyalty_points != 0 )
			 {
        ...
        //**************************Email Fuction Code*****************************
			...
			if($Template_type == "Purchase_order")
			{				
				$this->CI->email->bcc($company_details->Company_contactus_email_id);	// send purchase order to company contactus email id in bcc
			}
