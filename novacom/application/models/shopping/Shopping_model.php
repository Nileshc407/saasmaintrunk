...
{
    public function get_all_items($limit,$start,$Company_id,$Merchandize_category_id,$Sort_by,$Sort_by_merchant,$Sort_by_brand,$Sort_by_gender,$Sort_by_name)
	{
		...
		if($Sort_by_name!='')//sort by brand
		{
			$this->db->where("A.Merchandize_item_name like '%".$Sort_by_name."%'");
		}
		...
		else //Default
		{
			...
			$this->db->order_by('A.Merchandize_category_id','asc');
		}
	...
	public function Show_product_details($Company_merchandise_item_id)
	{
		$this->db->where(array('Company_merchandise_item_id' => $Company_merchandise_item_id,'Active_flag' => 1));
		$this->db->from('igain_company_merchandise_catalogue');
		$sql = $this->db->get();
	...
	{	 
		$this->db->select("T.Trans_id,T.Trans_date,SUM(T.Purchase_amount) as Purchase_amount,T.Voucher_no,T.Voucher_status,T.Bill_no,T.Update_date,T.Item_code,T.Item_size,T.Quantity,T.Manual_billno,CONCAT(IE.First_name, ' ', IE.Last_name) as Outlet_name,Delivery_method");
	...
	public function get_order_details1($Trans_id,$Bill_no,$Voucher_no) 
	{
		...
		$this->db->from('igain_transaction as T');
		$this->db->join('igain_transaction_child3 as TC3', 'T.Trans_id = TC3.Transaction_id','LEFT');
