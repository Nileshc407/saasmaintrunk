...
class Cust_home extends CI_Controller
{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			if($_POST != "")
			{
				...
				$post_data = array(
						...
						'User_pwd' => App_string_encrypt($this->input->post('Password')),
						'Company_id' => $this->input->post('Company_id'),
	...
	public function changepassword()
	{
		...
		{
			$this->session->set_flashdata("invalid_error_code","Old Password is same with New Password");
	...
	function enroll_new_member_website()
	{
		...
		$email = App_string_encrypt($_REQUEST['userEmailId']);
		$Company_id = $_REQUEST['Company_id'];
		$Country = $_REQUEST['Country'];
		$flag = $_REQUEST['flag'];
		$Phone_no = App_string_encrypt($_REQUEST['phno']);
		if($first_name !="" || $last_name !="" || $email !="" || $Company_id !="" || $Country !=""  )
		{
				...
				if($CheckPhone_number ==0)
				{
					...
					/**********************igain Log Table change 29-06-2017 *************************/
						...
						if($Last_enroll_id > 0)
						{
							...
							$this->session->set_flashdata("error_code","You have Enroll successfully...Login credentials sent on your registered Email Id !!");

						}
						else
						{
							$this->session->set_flashdata("error_code","Provided Mobile No is already Exist...Please provide valid Mobile No!!");
						}
				}
				else
				{
			...
			{
				$this->session->set_flashdata("error_code","Provided Email ID is already Exist...Please provide valid email id!!");
