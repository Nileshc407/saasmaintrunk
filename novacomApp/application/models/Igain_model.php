...
/********************************************Akshay Start*******************************/
	...
	function FetchSellerdetails_google_map($Company_id,$Cust_lat,$Cust_long)
	{
	...
	/* 15-02-2021 Product Voucher without stamp*/
	function get_member_product_vouchers($Card_id,$Company_id,$item_code,$Quantity)
	{
		$today = date("Y-m-d");
		...
		/*---25-06-2020---*/
			$this->db->where(array('igain_giftcard_tbl.Card_id' => $Card_id,'igain_giftcard_tbl.Company_id' => $Company_id,'igain_giftcard_tbl.Payment_Type_id' => '997','B.Company_merchandize_item_code' => $item_code,'B.Voucher_id !=' =>0,'B.Offer_code' => ""));			
	...
	function get_member_product_inprecentage_vouchers($Card_id,$Company_id,$item_code,$Quantity)
	{
		...
		/*---25-06-2020---*/
			$this->db->where(array('igain_giftcard_tbl.Card_id' => $Card_id,'igain_giftcard_tbl.Company_id' => $Company_id,'igain_giftcard_tbl.Payment_Type_id' => '997','B.Company_merchandize_item_code' => $item_code,'B.Quantity <=' => $Quantity,'B.Voucher_id' =>0,'B.Offer_code !=""'));			
			// $this->db->where_in("Payment_Type_id",array('997'));
		/*---25-06-2020---*/
	...
	function Get_my_discount_vouchers($Enrollment_id,$Company_id,$Card_id)
	{
		...
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id ,'T.Valid_till >=' => $today)); // 'T.Card_balance >' => 0
		// $this->db->where_in('T.Payment_Type_id', array('99','997','998')); 997 removed for stamp 
		$this->db->where_in('T.Payment_Type_id', array('99','998'));
		$this->db->where(array('T.Card_balance >' => 0));
	...
	function Get_my_gift_cards($Enrollment_id,$Company_id,$Card_id)
	{
		$today=date('Y-m-d');
		$this->db->select("T.*");
		$this->db->where(array('T.Company_id' => $Company_id, 'T.Card_id' => $Card_id, 'T.Payment_Type_id !=' => 99,'T.Valid_till >= ' => $today)); // 'T.Card_balance >' => 0
		$this->db->from('igain_giftcard_tbl as T');
		$this->db->join('igain_enrollment_master as IE', 'T.Card_id = IE.Card_id','LEFT');
		$this->db->where_in('T.Payment_Type_id', array('3','4','5'));
		$this->db->order_by('T.id','DESC');
		$sql = $this->db->get();
		// echo $this->db->last_query(); die;
		if($sql->num_rows() > 0)
	...
	function Fetch_Seller_outlet_details($Company_id)
	{
	  $this->db->select("*");
	  $this->db->from('igain_enrollment_master');
	  $this->db->where(array('User_id' => '2','User_activated' => '1','Super_seller' => '0','Sub_seller_admin' => '0','Sub_seller_Enrollement_id !=' => '0','Company_id' => $Company_id));
	...
	function Fetch_unused_stamp_voucher($enroll,$Company_id, $Card_id)
	{
		...
		$this->db->where_in('gc.Payment_Type_id', array('997'));
