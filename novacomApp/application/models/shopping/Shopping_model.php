...
class Shopping_model extends CI_Model 
{
	...
	public function get_order_details($Trans_id,$enroll_id) 
	{
		...
		// $this->db->where(array('Trans_id' => $Trans_id));
		$this->db->where(array('T.Bill_no'=>ltrim(rtrim($Trans_id)),'T.Enrollement_id' =>$enroll_id));
		//$this->db->limit(0,1);		
		$sql = $this->db->get();
		// echo $this->db->last_query();
		if($sql->num_rows() > 0)
	...
	{  		  
		...
		$this->db->select('P.Company_merchandise_item_id,P.Merchandize_item_name,P.Thumbnail_image1,P.Billing_price,P.Item_Weight,P.Weight_unit_id,T.Merchandize_Partner_id,T.Merchandize_Partner_branch,P.Seller_id,P.Merchant_flag,P.Cost_price,P.VAT,P.Merchandize_category_id,P.Item_image1,P.Combo_meal_flag,T.Quantity,T.Purchase_amount,T.Redeem_points,T.Paid_amount,T.Company_id,T.Bill_no,T.Quantity,T.Voucher_no,T.Voucher_status,T.Item_code,T.Item_size,T.Item_size,Shipping_cost,PM.Country_id as Partner_Country,PM.State as Partner_state,T.Seller,T.Delivery_method,T.remark2,T.remark3,T.Mpesa_Paid_Amount,T.COD_Amount,T.Mpesa_TransID,T.Table_no,T.Bill_discount,T.Item_category_discount,T.Total_discount,T.Manual_billno,T.Loyalty_pts,T.Trans_date,T.Trans_type,T.Order_no,T.Enrollement_id'); 
		...
		$this->db->join('igain_company_merchandise_catalogue as P', 'T.Item_code = P.Company_merchandize_item_code','LEFT');		
		$this->db->join('igain_partner_master as PM', 'P.Partner_id = PM.Partner_id','LEFT');			
		$this->db->where(array('T.Bill_no' =>ltrim(rtrim($serial_id)), 'T.Enrollement_id' => $Enrollement_id, 'T.Company_id' => $Company_id));
		
		// $this->db->where(array('P.Active_flag'=>1,'P.show_item'=>1,'T.Bill_no' => $serial_id, 'T.Enrollement_id' => $Enrollement_id, 'T.Company_id' => $Company_id));
		
		// $this->db->limit(1);
		$this->db->group_by('P.Company_merchandize_item_code');
		 ...
		 // echo $this->db->last_query();
			...
			{ 
				...
				else
				{
					...
					$Seller_enroll_id= $delivery_outlet_details->Enrollement_id;
					...
					if($Seller_api_url != "")
					{
						...
						/*----14-10-2020---*/
							...
							// $Seller_enroll_id= $delivery_outlet_details->Enrollement_id;
							$Seller_api_url = $Seller_api_url.'?outlet_id='.$Seller_enroll_id.'&Authorization='.$this->OnlineOrderAPI_key;
						// echo "Seller_api_url --".$Seller_api_url."<br>";	
						
						/*----14-10-2020---*/
						
						
						// $Seller_api_url= "https://apps.novacom.co.ke:7073/coloop/ehp?outlet_id=13&Authorization=14504397AEA35BD9374EEA1F708013FA6472A4463B0A3982B225A5BE7322BA05";
	...
	public function get_product_offers($product_id,$Merchandize_category_id,$Company_id)
	{
		$Todays_date=date('Y-m-d');
		...
		$this->db->where(array('A.Active_flag'=>1,'A.Company_id'=>$Company_id,'A.Buy_item_category '=>$Merchandize_category_id));
	...
	public function member_sent_offers($Company_id,$enroll_id,$Offer_id)
	{
		$this->db->select('Voucher_send_id,Offer_id');
		$this->db->from('igain_company_send_voucher');
		$this->db->where(array('Company_id'=>$Company_id, 'Enrollement_id'=>$enroll_id, 'Offer_id'=>$Offer_id));
		$sql77 = $this->db->get();
		// echo $this->db->last_query();die;
		if($sql77->num_rows() > 0)
		{
	...
	public function get_item_purchase_count($ItemCode,$Company_id,$enroll_id,$From_date,$Till_date)
	{
		...
		$this->db->select('sum(Quantity) as product_qty');
		$this->db->from('igain_transaction');
