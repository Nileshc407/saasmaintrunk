...
class Shopping extends CI_Controller 
{
	...
	function index()
	{
		...
		{	
			...
			// echo"-------delivery_outlet_details-----".$data["delivery_outlet_details"]->Sub_seller_Enrollement_id."--<br>";
			$data["Merchandize_category"] = $this->Shopping_model->Get_Merchandize_Category($data['Company_id'],$Sort_by_merchant);
			// $data["Merchandize_category"] = $this->Shopping_model->Get_Merchandize_Category($data['Company_id'],$data["delivery_outlet_details"]->Sub_seller_Enrollement_id);
	...
	{ 
		...
		$data['Stamp_item_flag']= $_REQUEST['Stamp_item_flag'];  //***stamp item 06-10-2020
		
		$Item_code=$_REQUEST['Company_merchandize_item_code']; 
		
		// echo " Stamp_item_flag ".$_REQUEST['Stamp_item_flag']; die;
 	...
 	echo "<br>sides condi array--<br>";
		...
		{	
			$data2 = array(
				 ...
				 'Item_category_id' => $_REQUEST["Product_category_id"],
				 'options' => array('Company_merchandize_item_code' => $_REQUEST["Company_merchandize_item_code"],'Item_image1' => $_REQUEST["Item_image1"],'E_commerce_flag' => 1,'Redemption_method' => 29,'Branch' => $_REQUEST["Branch"],'Item_size' => $_REQUEST["Item_size"],'Item_Weight' => $_REQUEST["Item_Weight"],'Weight_unit_id' => $_REQUEST["Weight_unit_id"],'Partner_id' => $_REQUEST["Partner_id"],'Partner_Country_id' => $_REQUEST["Partner_Country_id"],'Partner_state' => $_REQUEST["Partner_state"],'Seller_id'  => $_REQUEST["Seller_id"],'Merchant_flag' => $_REQUEST["Merchant_flag"],'Cost_price' => $_REQUEST["Cost_price"],'VAT' => $_REQUEST["VAT"]),
				 'name' => $_REQUEST['name'],'Stamp_item_flag' => $_REQUEST['Stamp_item_flag']
			   );
	...
	{ 
		...
		$Stamp_item_flag = $_REQUEST['Stamp_item_flag']; // stamp item 06-10-2020
	...
	//	print_r($MainItemInfo); echo "<br>";
		...
		$data2 = array(
			 ...
			 'name' => $_REQUEST['name'],
			 'Stamp_item_flag' => $_REQUEST['Stamp_item_flag'],
			...
			echo "<br><br>";*/
					...
					$data2 = array(
					 ...
					 'options' => array('Company_merchandize_item_code' => $Order_det['Item_code'],'Item_image1' => $Order_det['Item_image1'],'E_commerce_flag' => 1,'Redemption_method' => 29,'Branch' => $Order_det['Merchandize_Partner_branch'],'Item_size' => $Order_det['Item_size'],'Item_Weight' => $Order_det['Item_Weight'],'Weight_unit_id' => $Order_det['Weight_unit_id'],'Partner_id'  => $Order_det['Merchandize_Partner_id'],'Partner_Country_id' => $Order_det['Partner_Country'],'Partner_state'  => $Order_det['Partner_state'],'Seller_id' => $Order_det['Seller_id'],'Merchant_flag' => $Order_det['Merchant_flag'],'Cost_price' => $Order_det['Cost_price'],'VAT' => $Order_det['VAT'],'Condiments_code' => $Order_det['remark3'],'Condiments_name' => $Order_det['remark2'],'RequiredCondiments' => 		$Required_CondimentsCode,
						 ...
						 'name' => $Merchandize_item_name,
						  'Stamp_item_flag' => $Order_det['Stamp_item_flag'],
						 'Main_item' => $MainItemInfo,
						 'MainItem_TotalPrice' => $MainItemTotal_Price,
						 'SideCondiments_TotalPrice' => $SideCondiments_TotalPrice,	
				   );
	...
	{ 
		...
		{	
			...
			//*************** sandeep gift card logic 28-09-2020 **************************
				$GiftCards = $this->Igain_model->get_member_gift_cards($data['Card_id'],$data['Company_id']);
			...
			*************** Revi Product Voucher logic 01-07-2020 **************************/
			...
			else
			{
				...
				$data["Redemptionratio"] = $data["delivery_outlet_details"]->Seller_Redemptionratio;
			...
			if ($cart = $this->cart->contents())
			{
				...
				/********************************Ravi 02-07-2020************************************/		
					...
					$ProductInPercentageVouchers= $this->Igain_model->get_member_product_inprecentage_vouchers($data['Card_id'],$data['Company_id'],$item["options"]['Company_merchandize_item_code'],$item['qty']);
    ...
    public function CheckoutPayment()
	{
		...
		{						
			...
			foreach($Company_details as $Company)
			{
				...
				$Stamp_voucher_validity = $Company['Stamp_voucher_validity'];	
			...
			$delivery_outlet_details = $this->Igain_model->get_enrollment_details($_SESSION['delivery_outlet']);
			$Seller_Redemptionratio = $delivery_outlet_details->Seller_Redemptionratio;
			
			
			/*-----Ravi Cahnge 28-10-2020-------
				if($this->input->post('point_redeem') > 0 && $this->input->post('redeem_amt') > 0 ) {
					...
					/* echo"---point_redeem---".$point_redeem."---<br>";
						echo"---redeem_amt---".$redeem_amt."---<br>";
						echo"---Seller_Redemptionratio---".$Seller_Redemptionratio."---<br>"; ****
				
						$new_redeem_amt = ($point_redeem/$Seller_Redemptionratio);
						
						/* echo"---new_redeem_amt---".$new_redeem_amt."---<br>";
						var_dump(strcmp($new_redeem_amt,$redeem_amt));	 ******					
						
						if($new_redeem_amt != $redeem_amt){
							
							$this->session->set_flashdata("error_code", "Provided redeem point don't match with redeem amount");
		...
		/**********************Nilesh VerifyToken 09-07-2020****************************/	
			...
			if($_REQUEST["BillRefNumber"]){
				$BillRefNumber=$_REQUEST["BillRefNumber"];
			} else {
				$BillRefNumber="";
			}			
			/* ---------05-06-2020---------- */
			...
			/**********sandeep 29-09-2020 ***gift card********/	

			//$giftCardArray = explode("_",$_REQUEST['gift_card_id']);
			$giftCardID = $_REQUEST['gift_card_voucher'];
			$giftCardValue = $_REQUEST['giftCard_amt'];
			//echo $giftCardID."---val--".$giftCardValue."<br>"; exit;
 ...
 */											$Extra_earn_points = $result->Extra_earn_points;
									...
									//****************** 28-08-2019 sandeep ***********************************			
											...
											//  echo"---subtotal-----".$item['subtotal']."---<br>";
																...
																$subtotal12 = $this->cheque_format($subtotal);

																$subtotal1 = filter_var($subtotal12, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);//1989.34

																$Purchase_amount=$item['subtotal'];
																// $transaction_amt = $grand_total;
																$transaction_amt1 = (($subtotal1 * $Purchase_amount ) / $subtotal);
																
																$transaction_amt = filter_var($transaction_amt1, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
											 ...
											 */
																...
																$grand_total12 = $this->cheque_format($grand_total);
																
																$grand_total1 = filter_var($grand_total12, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);//1989.34
																
																$Purchase_amount=$item['subtotal'];
																// $transaction_amt = $grand_total;
																 $transaction_amt1 = (($grand_total1 * $Purchase_amount ) / $subtotal);
				...
				if($TotalDiscountAmount > 0)
				{
					$Weighted_discount_amt = (($TotalDiscountAmount * $item_total_amount) / $subtotal);
											...
											//******** 18-03-2020****sandeep**********
					//******** 29-09-2020***gift card*sandeep**********
							$Weighted_giftcard_amt = 0;
							
							if($giftCardValue > 0)
							{
								$Weighted_giftcard_amt = (($giftCardValue * $item_total_amount) / $subtotal);
							}
				//******** 29-09-2020***gift card*sandeep**********						
			...
			// $RedeemAmt=$Weighted_redeem_points/$Company_Redemptionratio;
			// echo "<br><b>Weighted_redeem_points--------- </b>".$Weighted_redeem_points;			
			// echo "<br><b>Seller_Redemptionratio--------- </b>".$Seller_Redemptionratio;			
			$RedeemAmt=$Weighted_redeem_points/$Seller_Redemptionratio;
	...
	//**sandeep**$PaidAmount=( $Purchase_amount + $Split_Shipping_cost )-( $RedeemAmt + $Weighted_discount_amt);
	$PaidAmount=( $Purchase_amount + $Split_Shipping_cost )-( $RedeemAmt + $Weighted_discount_amt + $Weighted_giftcard_amt);
	...
	/*echo "<br><b>PaidAmount--------- </b>".$PaidAmount;	*/									
										...
										/*************Nilesh*************/	
											...
											$data123 = array(
													...
													'Shipping_points' => ($Split_Shipping_cost*$Seller_Redemptionratio),
													...
													//***sandeep gift card**** 29-09-2020 ************
													
													'GiftCardNo' => $giftCardID
													/***** 18-03-2020 ************				
						...
						//****** sandeep stamp item 06-10-2020 *******************
					

													// echo "id--".$item['id']."--code--".$Company_merchandize_item_code."--stamp--".$item['Stamp_item_flag']."<br>";
													if($item['Stamp_item_flag'] == 1)
													{
														
														$Product_offers = $this->Shopping_model->get_product_offers($item['id'],$Merchandize_category_id,$data['Company_id']);
														...
														if($Product_offers != null)
														{
															...
															{	
																$OfferSent = $this->Shopping_model->member_sent_offers($data['Company_id'],$data['enroll'],$offer['Offer_id']);
																// echo "OfferSent ".$OfferSent;die;
																if($OfferSent == 0)
																{
																	if($offer['Company_merchandise_item_id'] > 0)
																	{
																		$item_purchase = $this->Shopping_model->get_item_purchase_count($Company_merchandize_item_code,$data['Company_id'],$data['enroll'],$offer['From_date'],$offer['Till_date']);
																	}
																	else if($offer['Company_merchandise_item_id'] == 0)
																	{
																		$item_purchase = $this->Shopping_model->get_category_purchase_count($Merchandize_category_id,$data['Company_id'],$data['enroll'],$offer['From_date'],$offer['Till_date']);
																	}
																//	echo $item_purchase."hhhhhh";	
																	
																	if($item_purchase >= $offer['Buy_item'])
																	{
																		
																		$FreeItem = $this->Shopping_model->Get_Merchandize_Item_details($offer['Free_item_id']);
																//var_dump($FreeItem);
																		...
																		$data76['Company_id'] = $data['Company_id'];
																		$data76['Enrollement_id'] = $data['enroll'];
																		 
																		$data76['Offer_id'] = $offer['Offer_id'];
																		...
																		$data76['Creation_date'] = date("Y-m-d");
																		
																		$this->Shopping_model->insert_product_voucher($data76);
															
															
													//****insert in gift card tbl ***********
															$data77['Company_id'] = $data['Company_id'];
															...
															// $data77['Card_balance'] = $data76['Cost_price'];
															// $data77['Card_value'] = $data76['Cost_price'];
															$data77['Discount_percentage'] = 100;
													
															$data77['Card_id'] = $CardId;
															$data77['Email'] = App_string_encrypt($CustomerEmail);
															$data77['Phone_no'] = App_string_encrypt($phno);
															$data77['Payment_Type_id'] = 997;
															$data77['Seller_id'] = $delivery_outlet_details->Enrollement_id;
															$data77['Create_date'] = date("Y-m-d");
															
															$this->Shopping_model->insert_voucher_in_gift_card($data77);
													//****insert in gift card tbl ***********
													...
													//****now send product voucher notification *****************
																	...
																	$Email_content76 = array
																		( 
																			...
																			'Description' => "Show above voucher code and get ".$offer['Free_item']." ".$FreeItem->Merchandize_item_name." free",
																			'Voucher_validity' => date("Y-m-d",strtotime("+$Stamp_voucher_validity days")),
																			'Template_type' => 'Product_voucher'
																		);
																				
																	//print_r($Email_content76);
																		$PVoucherNotification=$this->send_notification->send_Notification_email($data['enroll'],$Email_content76,'0',$data['Company_id']);
			...
			//****** sandeep stamp item 06-10-2020 *******************		
								...
								if($giftCardID != null && $giftCardValue > 0)
								{
									$today = date("Y-m-d H:i:s");
									$giftData2["Update_date"] = $today;
									$giftData2["Update_user_id"] = $delivery_outlet_details->Enrollement_id;
									$giftData2["Card_balance"] = 0;
									
									
									$this->db->where(array("Gift_card_id"=>$giftCardID,"Company_id"=>$Company_id));
									$this->db->update("igain_giftcard_tbl",$giftData2);
				...
				******** payment type discount rule *****sandeep*******/		
						...
						/********** mpesa trans_id confirmation 05-10-2019 ***********************/
							...
							{								
								...
								/* $input_args3 = Array('TransID' => $Mpesa_TransID ,'BalanceDue' => number_format($Bill_Amount_Due,2),'PaidAmount' => number_format($Mpesa_Total_Paid_Amount,2),'BillRefNumber' => $Mpesa_bill_no,"PaymentMethod"=> "MOBILE MONEY","PaymentType"=> "MPESA"); */
								
								$input_args3 = Array('TransID' => $Mpesa_TransID ,'BalanceDue' => number_format($Mpesa_Total_Paid_Amount,2),'PaidAmount' => number_format($Mpesa_Total_Paid_Amount,2),'BillRefNumber' => $Mpesa_bill_no,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
...
/************************************************Akshay Start***********************************************/
	...
	public function call_mpesastkpush_api($newTransID,$Final_Grand_total,$BillRefNumber,$step1,$newHash,$Seller_mpesastkpush_api_url){
	
		
		$input_args1 = Array ('TransID' => $newTransID ,'BalanceDue' => $Final_Grand_total,"BillRefNumber" =>$BillRefNumber,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
				
		$input_args1 = json_encode($input_args1);
		
		/* $MpesaPushurl = "https://apps.novacom.co.ke:7074/api_b2b/mpesastkpush"; */
		
		$MpesaPushurl = $Seller_mpesastkpush_api_url;
	
		$ch = curl_init();
		// $timeout = 30; // Set 0 for no timeout.
		$timeout = $step1; // Set 0 for no timeout.
		curl_setopt($ch, CURLOPT_URL, $MpesaPushurl);

		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_POSTFIELDS,$input_args1);
		//set the content type to application/json
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Bearer '.$newHash,'Content-Type:application/json'));
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);		
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
	...
	public function getVoucher(){
		...
		$BillRefNumber=$Voucher_no;			
		return $BillRefNumber; 
	}
	public function Verify_mpesa()
	{
		
		/* 20-08-2020 New mpesastkpush */
		...
		$Seller_mpesastkpush_api_url = $_REQUEST['Seller_mpesastkpush_api_url'];		
		
		$newTransID=$dial_code.''.$Trans_id;
		/**************************************************************************/
		
			
			
			$BillRefNumber=$this->getVoucher();
			$validateBillRefNumber = $this->Shopping_model->validate_BillRefNumber($BillRefNumber);			
			if($validateBillRefNumber == 0 ){				
				$BillRefNumber=$BillRefNumber;				
			} else {				
				$BillRefNumber=$this->getVoucher();	
			}
			...
			for($i=0;$i<3; $i++){			
				...
				sleep($step1-(date()-$previousTime)); */
				
				$API_Result=$this->call_mpesastkpush_api($newTransID,$Final_Grand_total,$BillRefNumber,$step1,$newHash,$Seller_mpesastkpush_api_url);
								
				$API_response = json_decode($API_Result, true);
				// echo "---API_Result--API_response--".print_r($API_response)."----<br>";
				/* $API_response == null || $API_response == "" ||  */
				if($API_response['ResultCode'] != "0000")
				{					
					
					if($API_flag==1){
						$New_TransID = null;						
						$this->output->set_content_type('application/json');
						$this->output->set_output(json_encode(array('response' => $API_Result)));
						break;
					}					
					continue;
					
				} else if($API_response['ResultCode'] == "0000") {
					
					$New_TransID = $API_response['TransID'];
					$API_flag=1;			
					
					$Final_Grand_total=$_REQUEST['Final_Grand_total'];
					$goods_till_number=$_REQUEST['goods_till_number'];
					$Trans_id=$New_TransID;
					$outlet_url=$_REQUEST['Seller_api_url2'];
					$newHash = $_REQUEST['payment_auth_key'];
					
						$input_args = Array ('TransID' => $Trans_id ,'BalanceDue' => $Final_Grand_total,"BillRefNumber" =>$BillRefNumber,"PaymentMethod"=> "MOBILEMONEY","PaymentType"=> "MPESA");
						...
						$input_args = json_encode($input_args);
						// echo "<br>---input_args API 2----".print_r($input_args).'<br>';	
						...
						$response['BillRefNumber']=$BillRefNumber;
						$result=json_encode($response);
								/* For Live */
								// $response = json_decode(json_decode($result, true),true);
											
						// echo "--Verify Mpesa Call-----".json_encode($response)."<br>";die;
		...
		/**************************************************************************
		$input_args = Array ('TransID' => $Trans_id ,'BalanceDue' => $Final_Grand_total,"BillRefNumber" =>$BillRefNumber,"PaymentMethod"=> "MOBILE MONEY","PaymentType"=> "MPESA");
	...
	public function select_outlet_shopping()
	{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			// var_dump($data['Sellerdetails']);
			
				$Seller_api_url = $data['Sellerdetails'][0]['Seller_api_url'];
				// $Seller_api_url ='https://apps.novacom.co.ke:7071/coloop/ehp';
			
				// echo "------Seller_api_url--".$Seller_api_url."------<br>";
				$Coloop_status=0;
				// $url='https://apps.novacom.co.ke:7071/coloop/ehp';
				//Initiate cURL
				...
				$data["Coloop_status"]=$Coloop_status;
				
				// echo "------Coloop_status--".$Coloop_status."------<br>";
			// die;
