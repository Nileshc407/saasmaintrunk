	...
	function stamp_collection() 
	{
        if ($this->session->userdata('cust_logged_in')) {
			...
			// if($_GET['brndID']){
				...
				foreach($data['Sellerdetails'] as $SS_details1){
					
					$enroll[$SS_details1['Enrollement_id']]=$SS_details1['Enrollement_id'];
					
				}
				
				$data['Sub_Seller_details'] = $this->Igain_model->Fetch_Seller_outlet_details($data['Company_id']);
