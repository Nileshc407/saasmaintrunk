	...
	<div id="application_theme" class="section pricing-section" style="min-height: 500px;">
		...
		<?php if($Enroll_details->Current_address != "" && $Enroll_details->City != ""  && $Enroll_details->Country != ""  ) { ?>
			...
			<div class="container">
				...
				<div class="scrolling-wrapper-flexbox" style="padding:4px;min-width: 300px;" id="Mcategory">
				
						
					<?php foreach ($Merchandize_category as $MerchandizeCat) { 
							if($MerchandizeCat->Merchandize_category_id != 10) {				
								
								if($Category_filter == $MerchandizeCat->Merchandize_category_id)
								{								
									$top_img_id=$MerchandizeCat->Merchandize_category_id;
								}								
							}
						}
						?>
					...
					<?php foreach ($Merchandize_category as $MerchandizeCat) { 
						...
						?>
							...
							<span class="mr-1" style="width: 8rem;"> 
								<small>
									<ul class="pagination">
									<!--<a href="#" <?php /*if($Category_filter == $MerchandizeCat->Merchandize_category_id)
									{ 
									...
									?> class="activeMe" 
									<?php }
									?>  onclick="show_category_items('<?php echo $MerchandizeCat->Merchandize_category_id; ?>')"> 
									
									<b>
									<?php echo $MerchandizeCat->Merchandize_category_name; */?></b> 
									</a>-->
									<?php echo "<li><a 
									 onclick='show_category_items(".$MerchandizeCat->Merchandize_category_id.");'><button type='button' class='btn btn-default' style='background: #322010;padding:3px 6px;white-space: pre-wrap; height:52px;width:90px;font-size: 10px;'>".$MerchandizeCat->Merchandize_category_name."</button></a></li>"; ?>
									</ul>
								</small>
							</span> 
						<?php }
					...
					<?php
						...
						$p = 1;
							...
							foreach($Merchandize_category as $key=>$MerchandizeCat)
							{
								...
								$opt .= "<li><a 
									 onclick='show_category_items(".$MerchandizeCat->Merchandize_catsego2ry_id.");'><button type='button' class='btn btn-default' style='background: #322010;padding:3px 10px;white-space: pre-wrap; height:52px;width:90px;font-size: 10px;'>".$MerchandizeCat->Merchandize_category_name."</button></a></li>";
									 }
...
<div class="col-md-6 col-md-offset-3" id="popup">
	<div class="alert alert-success text-center" role="alert" id="popup_info" style="background-color:#ffffff; color:#322010; border-color:#322010; width:200px; !IMPORTANT"></div>
</div>
