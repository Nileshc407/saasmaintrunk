...
class Igain_model extends CI_Model
{
	...
	function Fetch_stamp_brand_details($Company_id)
	{
		$this->db->select("*");
		$this->db->from('igain_enrollment_master');
		$this->db->where(array('igain_enrollment_master.User_id' => '2','igain_enrollment_master.User_activated' => '1','igain_enrollment_master.Company_id' => $Company_id));
		$this->db->join('igain_offer_master', 'igain_offer_master.Seller_id = igain_enrollment_master.Enrollement_id');
	...
	function FetchSellerdetails($Company_id)
	{
	  $this->db->select("*");
	  $this->db->from('igain_enrollment_master');
	  // $this->db->where(array('User_id' => '2','User_activated' => '1','Super_seller' => '0','Sub_seller_admin' => '1','Sub_seller_Enrollement_id' => '0','Company_id' => $Company_id,'Business_flag' =>1,'Participating_flag' =>1,'Parent_flag' =>1));
	...
	{
		$this->db->select("*,(Current_balance-Blocked_points) AS Total_balance,igain_state_master.name as state_name,igain_city_master.name as city_name,igain_country_master.name as country_name,Photograph");
		$this->db->from('igain_enrollment_master');
	...
	public function update_promocode($post_data,$promo_code,$Company_id,$Enrollment_id,$Current_balance,$membership_id,$lv_date_time)
	{
		...
		// echo"<br>--PromocodePoints---".$PromocodePoints."";	
		// echo"<br>--Current_balance--00---".$Current_balance."";	
		$Total_Current_Balance=$Current_balance + $PromocodePoints;
		// echo"<br>--Total_Current_Balance--111---".$Total_Current_Balance."";	
	...
	function get_promocode_details($promo_code,$Company_id)
	{
		...
		$this->db->where(array('PC.Company_id' => $Company_id,'BINARY PC.Promo_code'=>$promo_code,'PC.Promo_code_status' => '0','PC.Active_flag' => '1'));
	...
	function insert_auction_bidding($Super_Seller,$custEnrollId,$Company_id,$auctionID,$bidval,$Prize)
    {
		...
		$data['Company_id'] = $Company_id;        
		$data['Enrollment_id'] = $custEnrollId;
		$Member_Enrollment_id = $custEnrollId;
		$data['Prize'] = $Prize;
		$data['Bid_value'] = $bidval;
		$data['Create_user_id'] = $custEnrollId;
	...
	function check_promocode($promocode,$Company_id)
	{
		...
		/* $query =$this->db->select('PC.Promo_code');
				...
				$this->db->where(array('BINARY PC.Promo_code = ' => $promocode,'PC.Company_id' => $Company_id,'PC.Active_flag' =>'1','PC.Promo_code_status' => '0'));
				$this->db->where(" '".$today."' BETWEEN From_date AND To_date "); */
...
/********************************************Akshay Start*******************************/
	...
	{
		$this->db->select("*,A.Trans_type as TransType,SUM(A.Loyalty_pts) as Loyalty_pts,SUM(A.Redeem_points) as Redeem_pts,SUM(A.Purchase_amount) as Purchase_amt,SUM(A.Topup_amount) as Topup_amount,CONCAT(B.First_name,' ',B.Last_name) as Outlet_name,B.Photograph as Outlet_logo");
		$this->db->from("igain_transaction as A");
		$this->db->where(array('A.Company_id'=>$Company_id, 'A.Enrollement_id'=>$enroll_id));
		$this->db->where("A.Trans_type IN('1','2','3','12','29')");
		$this->db->join("igain_enrollment_master as B", "A.Seller=B.Enrollement_id");
		$this->db->order_by('A.Trans_id','desc');
	...
	function Fetch_Seller_Stamp_details($Company_id, $Enrollement_id, $brndID)
	{
		...
		$this->db->select("of.Offer_description,of.Seller_id,of.Buy_item,of.Free_item,of.From_date,of.Till_date,of.Offer_name,of.Offer_id,of.Offer_code");
		$this->db->from('igain_offer_master as of'); 				
		...
		if($query58 != NULL)
		{
			foreach($query58 as $row58)
			{
				...
				// echo"<br>-----Offer_code---".$this->db->last_query()."---<br>"; //die;
				...
				// print_r($item_code_array);
					...
					if($item_code_array){
						...
						$this->db->select("Trans_date,SUM(Quantity-Free_item_quantity) as TotalQty ");
						$this->db->from("igain_transaction");
						...
						//$this->db->limit(1,0);
						$sql63 = $this->db->get();
						// echo"<br>--igain_transaction--".$this->db->last_query()."---<br>";
						foreach ($sql63->result() as $row63)
						{
							$row58->TotalQty = $row63->TotalQty;
							$row58->Trans_date = $row63->Trans_date;
	...
	function Fetch_Seller_Stamp_unused_voucher($enroll,$Company_id, $Card_id, $brndID,$Offer_code)
	{
		...
		if($query58 != NULL)
		{
			...
			{				
				...
				$this->db->group_by("gc.Gift_card_id",'desc');
				$query59 = $this->db->get()->result();
				...
				$this->db->select("*");
				$this->db->from('igain_company_send_voucher as SC'); 				
				// $this->db->where(array('SC.Offer_code != "" ','SC.Enrollement_id' => $enroll));				
				$this->db->where(array('SC.Offer_code'=>$Offer_code,'SC.Enrollement_id' => $enroll));				
				//$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id'); 
				$this->db->group_by("SC.Voucher_code",'desc');
				$this->db->group_by("SC.Offer_code",'desc');
				$query59 = $this->db->get()->result();
				// echo"--Count---".$this->db->last_query()."---<br>"; //die;
				foreach($query59 as $row59){
						...
						$this->db->select("COUNT(id) AS GiftCardCount");
						$this->db->from('igain_giftcard_tbl AS gc'); 
						$this->db->where(array('gc.Gift_card_id' => $row59->Voucher_code,'gc.Company_id' => $Company_id,'gc.Seller_id' =>$row58->Enrollement_id,'gc.Card_id' =>$Card_id,'gc.Card_balance > ' =>0));	
						
						
						$this->db->group_by("gc.Gift_card_id",'desc');
						
						$query60 = $this->db->get()->result();
						// echo"--GiftCardCount---".$this->db->last_query()."---<br>"; //die;
						foreach($query60 as $row60){
							
							// echo"--GiftCardCount---".$row60->GiftCardCount."---<br>"; //die;
							$GiftCardCount_array[]=$row60->GiftCardCount;
						}
	...
	function Fetch_unused_stamp_voucher($enroll,$Company_id, $Card_id)
	{
		
		// echo"--Stamp_voucher_validity---".$Stamp_voucher_validity."---<br>";
		$todayDate=date("Y-m-d");				
		$this->db->select("gc.Gift_card_id,gc.Valid_till,gc.Company_id,gc.Seller_id,OF.Offer_name");
		$this->db->from('igain_giftcard_tbl as gc'); 				
				
		$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id','LEFT'); 
		$this->db->join('igain_offer_master as OF', 'OF.Offer_code = SC.Offer_code AND OF.Company_id = SC.Company_id','LEFT'); 
		$this->db->where(array('gc.Company_id' => $Company_id,'gc.Card_id' =>$Card_id,'gc.Card_balance >' =>0,'SC.Enrollement_id' => $enroll,'gc.Valid_till >= ' => $todayDate));	
		$this->db->order_by("gc.Seller_id",'ASC');
		$this->db->order_by("gc.Create_date",'DESC');
		$this->db->group_by("gc.Gift_card_id",'DESC');
		$query59 = $this->db->get()->result();
		// echo"--Count---".$this->db->last_query()."---<br>"; die;
		 return $query59;			
	...
	/**************Ravi- 01-02-2021***************/
	/**************AMIT KAMBLE 14-09-2021--------------------------------***************/
		function Fetch_used_stamp_voucher($enroll,$Company_id, $Card_id)
	{
		
		// echo"--Stamp_voucher_validity---".$Stamp_voucher_validity."---<br>";
		$todayDate=date("Y-m-d");				
		$this->db->select("gc.Gift_card_id,gc.Valid_till,gc.Company_id,gc.Seller_id,OF.Offer_name,gc.Update_date");
		$this->db->from('igain_giftcard_tbl as gc'); 				
				
		$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id','LEFT'); 
		$this->db->join('igain_offer_master as OF', 'OF.Offer_code = SC.Offer_code AND OF.Company_id = SC.Company_id','LEFT'); 
		$this->db->where(array('gc.Company_id' => $Company_id,'gc.Card_id' =>$Card_id,'gc.Card_balance ' =>0,'SC.Enrollement_id' => $enroll));	
		$this->db->order_by("gc.Seller_id",'ASC');
		$this->db->order_by("gc.Create_date",'DESC');
		$this->db->group_by("gc.Gift_card_id",'DESC');
		$query59 = $this->db->get()->result();
	...
	function Fetch_expired_stamp_voucher($enroll,$Company_id, $Card_id)
	{
	...
	/**************AMIT KAMBLE 14-09-2021-------------END-------------------***************/
	
	/**************SMS 10-11-2021-------------Start-------------------***************/
	public function get_sms_configuration($Company_id,$SMS_configuration_id)
	{
		$this->db->where(array('Company_id' => $Enrollement_id,'SMS_configuration_id' => $SMS_configuration_id, 'Active_flag' =>1));
		$query = $this->db->get('igain_sms_configuration');	
		if($query -> num_rows() > 0)
		{
			return $query->row();
		}
	...
	public function insert_data($tableName,$InsertPara)
	{
		$this->db->insert($tableName, $InsertPara);		
		// echo"<br>-----insert_data------".$this->db->last_query();
		if($this->db->affected_rows() > 0)
		{
			return true;
		}  
	}
	public function fetch_data($where,$tableName)
	{
		$this->db->where($where);
		$query = $this->db->get($tableName);	
		// echo"<br>-----fetch_data------".$this->db->last_query();
		if($query -> num_rows() > 0)
		{
			return $query->row();
		}
	...
	public function update_data($where,$data,$tableName)
	{
		...
		// echo"<br>-----update_data------".$this->db->last_query();
		// die;
		if($this->db->affected_rows() > 0)
		{
			return true;
	...
	//*********** 05-01-2022 *********Email Template ******
	function get_brand_images($Company_id,$brndID)
	{
		$this->db->select("*");
		$this->db->from('igain_brand_offer_images');
		$this->db->where(array('Brand_id' => $brndID,'Company_id' => $Company_id));
		$this->db->order_by('Sequence', 'ASC');
		// $this->db->limit('4');
		$imagesSql = $this->db->get();	
		return $imagesSql->result_array();
