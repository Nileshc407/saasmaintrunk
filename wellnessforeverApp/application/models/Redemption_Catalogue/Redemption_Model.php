...
class Redemption_Model extends CI_Model 
{
	...
	{
		$this->db->insert('evouchar_place_order_response',$data);
		if($this->db->affected_rows() > 0)
		{
			return $this->db->insert_id();
		}
	...
	public function Insert_xoxoday_temp_items($data)
	{
		$this->db->insert('third_party_items_temp',$data);
		if($this->db->affected_rows() > 0)
		{
			return $this->db->insert_id();
		}
	...
	public function Fetch_thirdparty_temp_items($pID)
	{
		$this->db->select('*');
		$this->db->from('third_party_items_temp');
