...
/********************************************Akshay Start*******************************/
	...
	function Fetch_Seller_Stamp_details($Company_id, $Enrollement_id, $brndID)
	{
		...
		/* 		
			$this->db->select("v.Item_name,v.Quantity,v.Cost_price,v.Valid_from,v.Valid_till,v.Company_merchandize_item_code,o.Buy_item,o.Free_item");
			$this->db->from('igain_giftcard_tbl as g'); 
			$this->db->join('igain_company_send_voucher as v', 'v.Voucher_code = g.Gift_card_id AND v.Company_id = g.Company_id'); 
			$this->db->join('igain_offer_master as o', 'o.Company_merchandise_item_id = v.Company_merchandise_item_id AND o.Company_id = v.Company_id');  
		
		*/
		
		$this->db->select("MC.Company_merchandize_item_code,MC.Merchandize_item_name,of.Seller_id,of.Buy_item,of.Free_item,of.From_date,of.Till_date,of.Offer_name");
		...
		$this->db->where(array('of.Company_id' => $Company_id));
		/* ,'of.Seller_id' =>$brndID */
		...
		if($query58 != NULL)
		{
			foreach($query58 as $row58)
			{
				...
				$this->db->from("igain_enrollment_master");
				$this->db->where(array('Enrollement_id'=>$brndID,'Company_id'=>$Company_id));
				$this->db->limit(1,0);
				$sql631 = $this->db->get();
				//echo $this->db->last_query();
				...
				// $From_date=$row58->From_date.' 00:00:00';
				
				$From_date=date("Y-m-d 00:00:00", strtotime($row58->From_date));
				$Till_date=date("Y-m-d 23:59:59", strtotime($row58->Till_date));
				
				// $Till_date=$row58->Till_date.' 23:59:59';
				$this->db->select("SUM(Quantity) as TotalQty ");
				$this->db->from("igain_transaction");
				$this->db->where_in("Seller",$brndID);
				$this->db->where(array('Enrollement_id'=>$Enrollement_id,'Company_id'=>$Company_id,'Item_code'=>$row58->Company_merchandize_item_code));
				$this->db->where('Trans_date BETWEEN "'.$From_date.'" AND  "'.$Till_date.'" ');
				//$this->db->limit(1,0);
				$sql63 = $this->db->get();
				// echo"<br>--igain_transaction--".$this->db->last_query();
