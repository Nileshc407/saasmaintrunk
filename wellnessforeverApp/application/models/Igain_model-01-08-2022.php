...
/********************************************Akshay Start*******************************/
		...
		if ($sql->num_rows() > 0)
		{
			...
			//		echo "item category--".$Itemcategory_id."--";
				...
				//************** sandeep 7-1-2020 ***************
							...
							if($row->Dis$discount_Amt = $row->Maximum_limit;
							}
				//************** sandeep 7-1-2020 ***************
						}
					}
	...
	{
		$this->db->select("*,A.Trans_type as TransType,SUM(A.Loyalty_pts) as Loyalty_pts,SUM(A.Redeem_points) as Redeem_pts,SUM(A.Purchase_amount) as Purchase_amt,CONCAT(B.First_name,' ',B.Last_name) as Outlet_name,B.Photograph as Outlet_logo");
		$this->db->from("igain_transaction as A");
		$this->db->where(array('A.Company_id'=>$Company_id, 'A.Enrollement_id'=>$enroll_id));
	...
	function Fetch_unused_stamp_voucher($enroll,$Company_id, $Card_id)
	{
		...
		$this->db->join('igain_company_send_voucher as SC', 'SC.Voucher_code = gc.Gift_card_id AND SC.Company_id = gc.Company_id'); 
		$this->db->join('igain_offer_master as OF', 'OF.Offer_code = SC.Offer_code AND OF.Company_id = SC.Company_id'); 
