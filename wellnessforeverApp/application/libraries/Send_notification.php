...
class Send_notification 
{
    ...
    public function send_Notification_email($Enrollment_id,$Email_content,$Seller_id,$Company_id)
    {
		...
		$Company_name = $company_details->myloyaltyprogram;                
		$Base_url = base_url();	
		...
		if($Template_type == "Joining_Bonus") {
			...
			$subject = "Complimentary Bonus $Company_Currency Issued";		
			...
			/************************************Variable Replace Code******************************
				$search_variables = array('$Customer_name','$Merchant_name','$Membership_ID','$Joining_bonus_points','$Transaction_date','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Company_Currency','$Logo');//
				$inserts_contents = array($Customer_name,$Merchant_name,$customer_details->Card_id,$Email_content['Joining_bonus_points'],$Transaction_date,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Company_Currency,$Logo);
		...
		if($Template_type == "Enroll")
        {
			...
			ob_end_clean();	 */		
				...
				// var_dump($myData);
				$Pwddata = base64_encode(json_encode($myData));
				$Pwddata_URL = $Base_url2. "digicoffeehouse/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
				$PwddataLink = "<a href='" . $Pwddata_URL . "' target='_blank' style='color:#000;'>Click here to Set Password</a>";
				// $PwddataLink =$PwddataLink;
				
				// echo "-----PwddataLink----".$PwddataLink."----<br>";
				
				
			/************************************Variable Replace Code******************************
				$search_variables = array('$Customer_name','$Merchant_name','$Membership_ID','$Enrolled_under','$User_email_id','$Phone_no','$User_pwd','$Pwdlink','$pinno','$Pin_no_applicable','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Logo');//
				$inserts_contents = array($Customer_name,$Merchant_name,$Membership_ID,$Email_content['Enrolled_under'],$User_email_id,$Phone_no,$User_pwd,$PwddataLink,$pinno,$Pin_no_applicable,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		...
		if($Template_type == "Change_password")
        {
			...
			// var_dump($myData);
			$Pwddata = base64_encode(json_encode($myData));
			$Pwddata_URL = $Base_url2. "tamarind/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
			$PwddataLink = "<a href='" . $Pwddata_URL . "' target='_blank' style='color:#000;'>Click here to Set Password</a>";	
		...
		if($Template_type == "Forgot_password")
        {
			...
			$Pwddata = base64_encode(json_encode($myData));
			$Pwddata_URL = $Base_url2. "digicoffeehouse/index.php/Login/Setpassword?Pwd_data=" . $Pwddata;
			$PwddataLink = "<a href='" . $Pwddata_URL . "' id='OpenUrl' target='_blank' style='color:#000;'>Click here to Set Password</a>";
			// echo "-----PwddataLink----".$PwddataLink."----<br>";
			/************************************Variable Replace Code******************************
				$search_variables = array('$Customer_name','$Membership_ID','$User_email_id','$Phone_no','$User_pwd','$pinno','$Pwdlink','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Logo');//
				$inserts_contents = array($Customer_name,$Membership_ID,$User_email_id,$Phone_no,$User_pwd,$pinno,$PwddataLink,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Logo);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		}
        ...
        if($Template_type == "Transfer_points")
        {
		  ...
		  /* ob_start();       
		   ...
		   /************************************Variable Replace Code******************************
			$search_variables = array('$Customer_name','$Transferred_CardId','$Transferred_to','$Transferred_points','$Current_balance','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Company_Currency','$Logo');//
			$inserts_contents = array($Customer_name,$Email_content['Transferred_CardId'],$Email_content['Transferred_to'],$Email_content['Transferred_points'],$Current_point_balance,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Company_Currency,$Logo);
			$html = str_replace($search_variables,$inserts_contents,$body);
		   /************************************Variable Replace Code******************************/
	}
	 if($Template_type == "From_transfer_miles")
        ...
        if($Template_type == "Get_transfer_points")
        {
		   ...
		   /************************************Variable Replace Code******************************
			$search_variables = array('$Customer_name','$Received_points','$Received_CardId','$Received_from','$Current_balance','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$From_company','$Company_Currency','$Logo');//
			$inserts_contents = array($Customer_name,$Email_content['Received_points'],$Email_content['Received_CardId'],$Email_content['Received_from'],$Current_point_balance,$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$From_company,$Company_Currency,$Logo);
			$html = str_replace($search_variables,$inserts_contents,$body);
		   /************************************Variable Replace Code******************************/
		}
    ...
    { 
					...
					*/
										...
										$html ='
											...
											<div class="box purchuase-box">
												...
												<p><b>Credit Card (DPO):</b> '.$Symbol_of_currency.' '.$DPO_Card_Amount.'</p>
											</div>';
			...
			// echo"<br>---html---Purchase_order-----".$html;						
								...
								<body style="background-image:url("'.$Base_url.'assets/img/statement-bg.jpg")">
										...
										<div class="custom-body">
											<div class="box purchuase-box">
												...
												<p><b>Order No:</b> '.$Orderno. '</p>';
												if($POS_Bill_No != "") {
													$html2.='<p><b>Bill No:</b> '.$POS_Bill_No. '</p>';
												}
												$html2.='
												<hr class="hr-space"/>
												<h2 class="o_total"><b>Order Total:</b> '.$Symbol_of_currency.' '.number_format($_SESSION["Total_Shipping_Cost"]+$subtotal,2).'</h2>';
												...
												if($total_loyalty_points != 0 && $company_details->Loyalty_enabled ==1 )
												{
													$html2.='<h3>'.$Company_Currency.' Earned: <span class="text-green">'.$total_loyalty_points.'</span></h3>';
												}
														...
														$html2.='<div class="item">
															...
															<div class="dtc">
																...
																if($Condiments_name != "") {
																	...
																	$html2.='<p>+ '.$Condiments_name.'</p>';
																	
																	 if($item['MainItem_TotalPrice'] != "" && $item['SideCondiments_TotalPrice'] != "") { 
																		 $html2.='<span class="kes">'.$Symbol_of_currency.' '.number_format( ($item['MainItem_TotalPrice'] + $item['SideCondiments_TotalPrice']) * $item['qty'], 2);
																	 }
											...
											<div class="box purchuase-box">
												...
												if($vouchers != "" && $vouchers != 0)
												{
													$html2.='<p><b>Voucher:</b> '.$Symbol_of_currency.' '.$VoucherDiscountAmt.'</p>';
												}
				
												
												$html2.='<p><b>Redeem '.$Company_Currency.' ('.$Cust_wish_redeem_point.'  ):</b> '.$Symbol_of_currency.' '.number_format($EquiRedeem,2).'</p>
												<p><b>Mpesa:</b> '.$Symbol_of_currency.' '.number_format($mpesa_BillAmount,2).'</p>
												<p><b>Credit Card (DPO):</b> '.$Symbol_of_currency.' '.$DPO_Card_Amount.'</p>
											</div>';
											...
											if($delivery_type==29) {
												...
												$html2 .='<div class="box purchuase-box">
															<h2>Delivery Address</h2>
															<h3>'.$delivery_outlet_details->First_name.' '.$delivery_outlet_details->Last_name.'</h3>
															<p><b>Building / Estate:</b> '.$str_arr0.'</p>
															<p><b>Floor / House No.:</b> '.$str_arr1.'</p>
												...
												if($delivery_type ==107){
													...
													$html2 .='<div class="box purchuase-box">
														...
														if($delivery_type ==107 && $_SESSION["TableNo"] != "" ){
															...
															$html2 .='<p><b>Table No:</b>'.$Email_content['TableNo'].'</p>';
														}
																	
														$html2 .='<p><b>Building / Estate:</b> '.$str_arr0.'</p>
														<p><b>Floor / House No.:</b> '.$str_arr1.'</p>
														<p><b>Street / Road:</b> '.$str_arr2.'</p>
														<p><b>Additional:</b> '.$str_arr3.'</p>
												</div>';
    ...
    if($Template_type == "Purchase_Cancel")
    {
		...
		if($Template_type == "Call_Center_Query_Log")
        {
			...
			// $Custoee_qty','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store');//
				$inserts_contents = array($Customer_name,$Transaction_date,$Todays_date,$Email_content['Offer_name'],$Email_content['Item_name'],$Email_content['Item_code'],$Email_content['Orderno'],$Email_content['Free_qty'],$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store);
				$html = str_replace($search_variables,$inserts_contents,$body);
			/************************************Variable Replace Code******************************/
		// echo "<br>";
		...
		{  
			...
			ob_end_clean();
			/************************************Variable Replace Code***************************** */
			$search_variables = array('$Customer_name', '$Merchant_name', '$Membership_ID', '$Purchase_amount', '$Symbol_currency', '$GiftCardNo', '$GiftCard_balance', '$Valid_till','$Bill_no','$Transaction_date', '$Member_website', '$Company_name', '$Gooogle_Play', '$iOs_app_store', '$Comapany_Currency','$Logo'); //
			$inserts_contents = array($Customer_name, $Merchant_name, $customer_details->Card_id, $Email_content['Purchase_amount'], $Email_content['Symbol_currency'], $Email_content['GiftCardNo'], $Email_content['GiftCard_balance'],$Email_content['Valid_till'],$Email_content['Bill_no'], $Transaction_date, $Member_website, $Company_name, $Gooogle_Play, $iOs_app_store, $Comapany_Currency,$Logo);
			$html = str_replace($search_variables, $inserts_contents, $body);
			/************************************Variable Replace Code***************************** */
			// echo $html; die;
      }
	/**************16-05-2020 nilesh purchase gift card**********************/
		...
		{  
			$Communication_id = 0;
			$Offer = "Email Address Verification";
			...
			$pin=$Email_content['pin'];
			$Logo = $Base_url."/assets/images/logo.png";
			ob_start();
			include './application/libraries/Email_templates/Email_verification_otp.php';
			$body = ob_get_contents();
			ob_end_clean();
			/************************************Variable Replace Code***************************** */
			$search_variables = array('$First_name','$Customer_name', '$pin', '$Company_name','$Logo'); //
			$inserts_contents = array($First_name, $Customer_name, $pin,$Company_name,$Logo);
			$html = str_replace($search_variables, $inserts_contents, $body);
			/************************************Variable Replace Code***************************** */
			// echo $html; die;
		}
        ...
        { 
		   ...
		   /************************************Variable Replace Code******************************/
			$search_variables = array('$Notification_type','$Customer_name','$Gift_card','$Transferred_to','$Transferred_email','$Card_value','$Card_validity','$Member_website','$Company_name','$Gooogle_Play','$iOs_app_store','$Company_Currency','$Logo');//
			$inserts_contents = array($Offer,$Customer_name,$Email_content['Gift_card'],$Email_content['Transferred_to'],$Email_content['Transferred_email'],$Email_content['Card_value'],$Email_content['Card_validity'],$Member_website,$Company_name,$Gooogle_Play,$iOs_app_store,$Company_Currency,$Logo);
			$html = str_replace($search_variables,$inserts_contents,$body);
			// echo $html;
		   /************************************Variable Replace Code******************************/
		}
			...
			if ($Current_point_balance < 0) {
				...
				/************************************Email_body Variable Replace Code******************************/
					...
					$email_content = str_replace($search_variables,$inserts_contents,$TemplateDetails->Email_body);
						 ...
						 /* print_r($Email_content);	
						echo"Email_content--<br><br>--Email_body---";
						print_r($TemplateDetails->Email_body);	
					...
					$search_variables_sub = array('$First_name','$Last_name','$Company_name','$End_date','$Start_date','$Voucher_type','$Revenue_voucher','$Product_voucher','$Current_balance','$Membership_id','$Customer_name','$Discount_voucher','$Discount_percentage','$Discount_value','$User_email_id','$Pwdlink','$Company_Currency','$Joining_bonus_points','$Transaction_date','$Google_play_link','$Ios_application_link','$Transferred_from','$Received_from','$Received_points','$Auction_name');
					$inserts_contents_sub = array($First_name,$Last_name,$Company_name,$Email_content['End_date'],$Email_content['Start_date'],$Email_content['Voucher_type'],$Email_content['Revenue_voucher'],$Email_content['Product_voucher'],$Current_balance,$Card_id,$Customer_name,$Email_content['Discount_voucher'],$Email_content['Discount_percentage'],$Email_content['Discount_value'],$User_email_id,$Email_content['Pwdlink'],$Company_Currency,$Email_content['Joining_bonus_points'],$Email_content['Transaction_date'],$Cust_apk_link,$Cust_ios_link,$Email_content['Transferred_from'],$Email_content['Received_from'],$Email_content['Received_points'],$Email_content['Auction_name']);
					$Email_subject = str_replace($search_variables_sub,$inserts_contents_sub,$TemplateDetails->Email_subject);
					
					
					/* echo"<br><br>--TemplateDetails-Email_subject--<br><br>---";
				print_r($TemplateDetails->Email_subject);
				echo"<br><br>---Email_subject--<br><br>---";
				...
				try {
						...
						{						
							$Company_primary_email_id=$company_details->Company_primary_email_id;
							 $mail->addAddress($Partner_contact_person_email, 'Partner Person');
					...
					//**************************Email Fuction Code*****************************
						...
						$config['smtp_user'] = 'no-reply@igainspark.com';
					...
					{						
						...
						$Company_primary_email_id=$company_details->Company_primary_email_id;
						// $this->CI->email->from($User_email_id);
						// $mail->From = $User_email_id;
						// $this->CI->email->to($Partner_contact_person_email); 
						$mail->addAddress($Partner_contact_person_email, $Partner_contact_person_email);
						// $this->CI->email->cc($Admin_email);
					...
					if($Template_type == "Purchase_order"){
						...
						$mail->addBCC($company_details->Company_contactus_email_id);	
					}
					if($Template_type == "Purchase_order")
					{
						$html=$html2;
					...
					if($Template_type == "Email_verification" || $Template_type == "Forgot_password" || $Template_type == "Send_gift_card")
					{	
						if (!$mail->send())
						{  
							return $customer_notification;
