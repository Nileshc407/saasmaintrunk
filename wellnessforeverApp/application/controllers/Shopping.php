				...
				******** payment type discount rule *****sandeep*******/		
						...
						/********** mpesa trans_id confirmation 05-10-2019 ***********************/
							...
							{								
								...
								$input_args3 = Array('TransID' => $Mpesa_TransID ,'BalanceDue' => number_format($Mpesa_Total_Paid_Amount,2),'PaidAmount' => number_format($Mpesa_Total_Paid_Amount,2),'BillRefNumber' => $Mpesa_bill_no,"PaymentMethod"=> "MOBILE MONEY","PaymentType"=> "MPESA");
...
/************************************************Akshay Start***********************************************/
	...
	public function call_mpesastkpush_api($newTransID,$Final_Grand_total,$BillRefNumber,$step1,$newHash){
	
		/* echo "--in call_api---".date('H:i:s')."------<br>";	
	
		echo "---newTransID----".$newTransID."----<br>";
		echo "---Final_Grand_total----".$Final_Grand_total."----<br>"; */
		...
		// echo "---BillRefNumber----".$BillRefNumber."----<br>";
			...
			/* 
				API Input JSON {
					...
					"TransID":"254722111111",
					"BalanceDue":"1.00",
					"BillRefNumber":"ORDERNO1",
					"PaymentMethod": "MOBILEMONEY",
					"PaymentType": "MPESA"
				}
			*/
		
		$input_args = Array ('TransID' => $newTransID ,'BalanceDue' => $Final_Grand_total,"BillRefNumber" =>$BillRefNumber,"PaymentMethod"=> "MOBILE MONEY","PaymentType"=> "MPESA");
		...
		curl_close($ch);
		$response = json_decode($result, true);		
		// echo "---call--API-1 ---".json_encode($response)."<br>";
		
		/* 		
		$previousTime = date('H:i:s');
		sleep($step1-(date()-$previousTime));
		if($result == null || $result == "" || $response['ResultCode'] != "0000")
		{
			// $New_TransID = $response['ResultCode'];
			$New_TransID = $response;
		} */
		...
		/* 
			{
				...
				"PaidAmount": "0",
				"ResultCode": "0000",
				"ResultDesc": "ws_CO_200820200957284632",
				"TransID": "ws_CO_200820200957284632"
			} 
		*/
	...
	public function Verify_mpesa()
	{
		...
		/**************************************************************************/
			...
			for($i=0;$i<3; $i++){			
				...
				$API_Result=$this->call_mpesastkpush_api($newTransID,$Final_Grand_total,$BillRefNumber,$step1,$newHash);
	...
	public function select_outlet_shopping()
	{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			*/
			...
			$brndID=$_SESSION['brndID'];			
			// echo "----brndID----".$brndID."-----<br>";
			$data['Sellerdetails'] = $this->Igain_model->Fetch_Seller_outlet_details($data['Company_id'],$brndID);
			...
			/**************Ravi Gmap- 21-11-2019*************** use only for google Map **/
				...
				$Cust_long= $latlong_details->Longitude;
				$data['Page_header']= "Outlet Location";
				$data["Seller_details"] = $this->Igain_model->FetchSellerdetails_google_map($session_data['Company_id'],$Cust_lat,$Cust_long,$brndID);	
				// var_dump($data["Seller_details"]);
			/**************Ravi Gmap- 21-11-2019*************** use only for google Map **/	
				// echo "here----";
			// die;
