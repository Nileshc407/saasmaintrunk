...
class Customer_Api extends CI_Controller 
{
	...
	public function __construct()
	{
		...
		$this->load->model('api_model/Customer_Api_model');
	...
	{ 
		$this->load->view('api_view/igainspark_api');		
	}
                    ...
                    if($API_flag == 50)  //*** Transfer Beneficiary Points
                    {
						...
						if($_REQUEST['Membership_id'] != "")
                        {
                            ...
                            else
                            {
								if($Check_member_exist->User_activated==0)
                                {
                                    echo json_encode(array("status" => "2004", "status_message" => "Membership ID Has been Disabled or (\x20-\x7f)]*/s", "", $Beneficiary_Identifier1);
