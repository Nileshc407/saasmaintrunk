...
class Cust_home extends CI_Controller {
    ...
    function Create_Account_smart_phone() {
		...
		/* 13-01-2021 */
			echo json_encode(array("status" => "1009", "Status" => "Sorry! We are unable to process your enrollment now!"));
			exit;
		/* 13-01-2021 */
    ...
    public function works(){
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data['Company_id']= $session_data['Company_id'];
			 ...
			 $data["NotificationsCount"] = $this->Igain_model->Fetch_Open_Notification_Count($data['enroll'],$session_data['Company_id']);
			/*18-0-2021--- $data["NotificationsDetails"] = $this->Igain_model->FetchNotificationDetails($data['enroll'],$session_data['Company_id']); */
			
            
            
             
            /* 18-0-2021---$data["Point_balance"] = $this->Igain_model->get_current_point_balance($data['enroll']);
			...
			/* 18-0-2021--- $Company_details = $this->Igain_model->Fetch_Company_Details($data['Company_id']);
			foreach($Company_details as $Company)
			{
				$Country = $Company['Country'];
				$Company_Redemptionratio = $Company['Redemptionratio'];	
	...
	public function search_outlet()
    {
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$sql1 = $this->db->query(" Select First_name,Last_name,Current_address,Phone_no,User_email_id from igain_enrollment_master where (`First_name` LIKE '%".$query."%' OR `Last_name` LIKE '%".$query."%' OR `Current_address` LIKE '%".App_string_encrypt($query)."%'  ) AND Company_id=".$session_data['Company_id']." AND Sub_seller_Enrollement_id=".$_SESSION['brndID']." AND User_id=2 AND User_activated=1 AND Super_seller=0 AND Sub_seller_admin=0");
			$sql = $sql1->result();
			
			
			/* OR `Phone_no` LIKE '%".App_string_encrypt($query)."%' OR `User_email_id` LIKE '%".App_string_encrypt($query)."%' */
			
			/*  $this->db->select("First_name,Last_name,Current_address,Phone_no,User_email_id");
			$this->db->from('igain_enrollment_master');
			  $query = $this->input->get('query');
				$this->db->like('First_name', $query);
				$this->db->or_like('Last_name', $query);
				$this->db->or_like('Current_address', App_string_encrypt($query));
				$this->db->or_like('Phone_no',  App_string_encrypt($query));
				$this->db->or_like('User_email_id',  App_string_encrypt($query));
				$this->db->where(array('Company_id'=> $session_data['Company_id'],'Sub_seller_Enrollement_id'=> $_SESSION['brndID'],'User_id'=>2)); */
