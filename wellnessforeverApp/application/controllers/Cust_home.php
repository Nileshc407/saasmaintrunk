...
class Cust_home extends CI_Controller {
	...
	{	
        if ($this->session->userdata('cust_logged_in')) {
			...
			*/
			...
			// echo"-<br>-----Mobile_verified------".$Mobile_verified;
			if($data['Company_Details']->Sms_enabled==1)
			{
	...
	{  
        if ($this->session->userdata('cust_logged_in')) {
			...
			else 
			{
				...
				$GenderStatus = array('Male','Female','Other','');
				...
				$userEmailId = strip_tags($this->input->post('userEmailId'));
    ...
    public function update_promocode_App() {
        if ($this->session->userdata('cust_logged_in')) {
            ...
            if ($_POST == NULL) {
                ...
                $To_date = $Promocode_Details->To_date;    
                $From_date = $Promocode_Details->From_date;   
                if ($Promo_code_status != "" || $Promo_code_status != 0) {
                    $result = $this->Igain_model->update_promocode($post_data, $promo_code, $Company_id, $Enrollment_id, $Current_balance, $membership_id, $lv_date_time);
                    $Notification = $this->send_Notification_email($data['enroll'], $mail_to, $promo_code, $Company_id);
                    $SuperSeller = $this->Igain_model->get_super_seller_details($data['Company_id']);
                    ...
                    if ($result == true) {
                       ...
                       /*  $data["MColor"] = "#41ad41";
                        ...
                        $this->load->view('front/games/success', $data); */
						 ...
						 // $data["Success_Message"] = "Congrats! You have got a extra " . $PromocodePoints . " ";
						$this->session->set_flashdata("error_code", "Congrats! You have got a extra " . $PromocodePoints . " ");
                  ...
                  /*   $data["MColor"] = "#FF0000";
					...
					// $data["Success_Message"] = "In-valid Promo Code";
					$this->session->set_flashdata("error_code", "In-valid Promo Code");
                }
                // $this->load->view('front/games/promocode', $data);
				redirect('Cust_home/promocode', 'refresh');
            }
        } else {
            redirect('login', 'refresh');
        }
 ...
 public function check_promo_code() {
			...
			else
			{
				...
				if ($result11 != "") 
				{
					...
					if ($result)
					{
						...
						$this->session->set_flashdata("error_code", "Password changed successfully... Please login again..");
						redirect('Cust_home/logout', 'refresh');	 
						// $this->load->view('front/profile/success', $data);
					}
	  ...
	  /*  $old_Password = $_REQUEST['old_pin'];
		...
		if ($this->session->userdata('cust_logged_in')) {
			...
			if($_POST==null){
				...
				if (strval($newpin) !== strval(intval($newpin))) {
					$this->session->set_flashdata("error_code", "Please Enter Valid New Pin");					
					redirect('Cust_home/Load_Change_Pin', 'refresh');
					die;
				}
				
				 $result = $this->Igain_model->Check_Old_Pin($this->input->post("old_pin"), $Company_id,$session_data['enroll']);
				...
				if ($result != "") {
					...
					if ($result) {
						...
						$this->session->set_flashdata("error_code", "Pin changed successfully!");
						
					}
					
				} else {				   
					$this->session->set_flashdata("error_code", "Please Enter Correct Old Pin");					
		...
		if ($this->session->userdata('cust_logged_in')) {
			...
			$data['Phone_no']=App_string_decrypt($data["Enroll_details"]->Phone_no);
		   	...
		   	if($_POST==null){
				...
				// $get_pin = $this->Igain_model->get_customer_pin($Company_id,$session_data['enroll']);
				// if ($get_pin->pinno != "" || $get_pin->pinno != 0) {
					
					
				$Enroll_details = $this->Igain_model->get_enrollment_details($session_data['enroll']);
				if ($Enroll_details->pinno != "" || $Enroll_details->pinno != 0) {
					$SuperSeller = $this->Igain_model->get_super_seller_details($Company_id);
					$Seller_id = $SuperSeller->Enrollement_id;
					$Email_content = array(
					...
					$this->send_notification->send_Notification_email($session_data['enroll'], $Email_content, $Seller_id, $Company_id);
					/*             * *******************Nilesh igain Log Table change 27-06-207************************ */
					...
					$result_log_table = $this->Igain_model->Insert_log_table($Company_id, $Enrollment_id, $User_email_id, $LogginUserName, $Todays_date, $what, $where, $userid, $opration, $opval, $firstName, $lastName, $Enrollment_id);
					/*********************igain Log Table change 27-06-2017 ************************ */
					$this->session->set_flashdata("error_code", "Your Pin has been sent on your registered email address");
					
				} else {
					$this->session->set_flashdata("error_code", "Something went wrong. Please  try again!");
				}				
				redirect('Cust_home/Load_Resend_Pin', 'refresh');
    ...
    function transferpointsApp() {
        if ($this->session->userdata('cust_logged_in')) {
            ...
            if ($_POST != NULL) 
			{
				...
				$Membership_id = App_string_decrypt($Membership_id);
				$Transfer_Points = App_string_decrypt($Transfer_Points);
                ...
                if ($result12 != NUll) 
				{
                    if (is_numeric($Transfer_Points)) 
					{
                        ...
                        if ($Membership_id == $Enroll_details12->Card_id || $Membership_id == $Enroll_details12->Phone_no) 
						{
							...
							$this->session->set_flashdata("error_code", "Transfered " . $Company_details->Currency_name . " Fail. Please Enter Valid Membership id");							
						...
						else if ($Current_point_balance1 < $Transfer_Points) 
						{
							...
							$this->session->set_flashdata("error_code", "Transfered fail you have insufficient " . $Company_details->Currency_name . ' ' . " balance");							
							redirect('Cust_home/transferpointsApp');
                        } 
						else 
						{
						   ...
						   $Transfer_PTS = array(
                            ...
                            if ($result > 0) {
								...
								$this->session->set_flashdata("error_code", "Thanks for your Transfer " . $Company_details->Currency_name);							
								redirect('Cust_home/transferpointsApp');
                            }
                        }
                    } else {
						...
						$this->session->set_flashdata("error_code", "Transfer " . $Company_details->Currency_name . " fail please enter valid data");							
						redirect('Cust_home/transferpointsApp');
					...
					$this->session->set_flashdata("error_code", "Transfer " . $Company_details->Currency_name . " fail please enter valid membership id or phone no.");							
					redirect('Cust_home/transferpointsApp');
			...
			$Login_Membership_id = $Enroll_details12->Card_id;
			 ...
			 else 
			 {
				...
				if ($result12 != NUll) 
				{
					if (is_numeric($Transfer_Points)) 
					{
                        ...
                        if ($Membership_id == $Enroll_details12->Card_id || $Membership_id == $Enroll_details12->Phone_no) 
						{
							...
							$this->session->set_flashdata("error_code", "Transfered " . $data['Company_Details']->Currency_name . " Fail. Please Enter Valid Membership id");							
							redirect('Cust_home/transferpointsApp');
                        } 
						else if ($Current_point_balance1 < $Transfer_Points) 
						{
							...
							$this->session->set_flashdata("error_code", "Transfered fail you have insufficient " . $data['Company_Details']->Currency_name . ' ' . " balance");							
							redirect('Cust_home/transferpointsApp');
                        }
						
						$data['Member_Enrollement_id'] = $result12->Enrollement_id;
						$data['Member_Membership_id'] = App_string_encrypt($result12->Card_id);
						...
						$data['Points_transfer'] = App_string_encrypt($Transfer_Points);
					...
					else 
					{
						$data["MColor"] = "#FF0000";
						$data["Img"] = "Fail";
						$data["Success_Message"] = "Transfer " . $data['Company_Details']->Currency_name . " fail please enter valid data";
						
						$this->session->set_flashdata("error_code", "Transfer " . $data['Company_Details']->Currency_name . " fail please enter valid data");	
						redirect('Cust_home/transferpointsApp');
					}
				} 
				else
				{
					...
					$this->session->set_flashdata("error_code", "Transfer " . $data['Company_Details']->Currency_name . " fail please enter valid membership id or phone no.");							
					redirect('Cust_home/transferpointsApp');
				} 
			 }       
        } else {
	...
	function getsurveyquestionApp() { //from mail or Notification
        error_reporting(0);
        $session_data = $this->session->userdata('cust_logged_in');
		$data['username'] = $session_data['username'];
			$data['First_name'] = $session_data['First_name'];
		...
		if($Walking_customer == 1)
		{
			...
			$survey_id = App_string_decrypt($_REQUEST['Survey_data']);
			if(is_numeric($survey_id)!=1)
			{
				redirect('Cust_home/getsurvey');
			}
			$survey_id = strip_tags($survey_id);
    ...
    function getsurveyquestion2() {
      ...
      function enroll_new_member_website() {
		...
		{
			$this->session->set_flashdata("login_success","Please Enter all the fields !!!");
			redirect('login', 'refresh');
        ...
        if($last_name){
			...
			//--------------License Expiry Check-----------------
			   ...
			   //---------License Expiry Check----------------------
				...
				if((date('Y-m-d') >= $_SESSION['Expiry_license']) || ($Member_count >= $Member_limit))
				{
					
					echo json_encode(array("status" => false, "errorcode" => "1028", "message" => "Company work in progress. Will be up soon. Sorry for the inconvenience!"));
					exit;
											...
											if($Sms_enabled == 1 ){
												...
												if($sms_configuration){
													...
													$message="Mobile Verification OTP is: ".$OTP." for ".$company_details->Company_name.".";
														// $message="Mobile Verification OTP is: $OTP for ";
