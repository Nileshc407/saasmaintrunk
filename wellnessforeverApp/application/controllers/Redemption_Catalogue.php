...
/***************************Nilesh Filter***************************************/	
	...
	public function Insert_Redeem_Items()
	{
		...
		if($Sub_Total <= $Current_balance )	
		{
			...
			$insert_data = array(
				...
				'Voucher_status' => strip_tags($Voucher_status),
				'Shipping_points' => 0,
				'Shipping_cost' => 0,
				'Cost_payable_partner' =>strip_tags(number_format($Cost_payable_partner,2)),
	...
	function eVoucher_catalogue()
	{
		...
		//	echo "------total_row------".$total_row."----<br>";
			...
			/*********************************XOXODAY Configuration*************************************************/
				...
				// echo"<br>---Gifting_enviornment_flag---".$data['Company_Details']->Gifting_enviornment_flag;
				...
				// die;
					...
					$curl = curl_init();
						...
						curl_close($curl);
						 ...
						 $this->db->truncate('third_party_items_temp');
						
						foreach($data['voucher_result'] as $key => $value){
								...
								foreach($value as $key2 => $value2)
								{
									foreach($value2 as $key2 => $value22)
									{
											...
											foreach($value22 as $key1 => $value1) {												
												// echo"--valueDenominations--".$value1['valueDenominations']."------<br>";
												// echo"--productId--".$value1['productId']."------<br>";
												
													// $first_price= json_encode($value1['valueDenominations']); 
													$first_price= explode(",",$value1['valueDenominations']); 
													// print_r($first_price);	
													// $smallest_price=$first_price[0];
													// echo"---smallest_price----".$smallest_price."---<br>";
													// echo"<br><br><br>---".$value1['productId']."---<br>";
													
													foreach($first_price as $price){
														
														
														$insertXoxoDayItemTemp=array(
						
															"productId"=>strip_tags($value1['productId']),
															"productPrice"=>strip_tags($price),
															"creation_date"=>strip_tags($lv_date_time)
														
														);
														$resultplaceOrder = $this->Redemption_Model->Insert_xoxoday_temp_items($insertXoxoDayItemTemp);
													}
	...
	public function oxoidpi(){
		...
		/* var_dump($_SESSION['pIDXX']);
		var_dump($_SESSION['VoucherPriceXX']);
		var_dump($_SESSION['qtyXX']); */
	}
	public function oxoidpirwesopj(){
		...
		// var_dump($_SESSION['PriceValueXX']);
		$data['token'] = $this->security->get_csrf_hash();		
		echo json_encode($data);
	}
	public function Redemption_done()
	{
		...
		$data['token'] = $this->security->get_csrf_hash();
			...
			$temp_items_details = $this->Redemption_Model->Fetch_thirdparty_temp_items($voucher_id);	
			...
			// echo"<br>----pIDXX---".$_SESSION['pIDXX']."---<br>";
			// echo"<br>----VoucherPriceXX---".$_SESSION['VoucherPriceXX']."---<br>";
			
			if($temp_items_details){			
				
				
				
				if($_SESSION['pIDXX'] == $temp_items_details->productId && $_SESSION['pIDXX'] != 0 && $_SESSION['VoucherPriceXX'] != 0 && $Purchase_amount > 0){
					
					/* echo"<br>----Current_redeem_points---".$Current_redeem_points."---<br>";
					echo"<br>----Total_balance---".$Total_balance."---<br>";
					echo"<br>----Gift_payment_balance---".$Gift_payment_balance."---<br>";
					echo"<br>----Purchase_amount---".$Purchase_amount."---<br>"; */
					...
					if(($Current_redeem_points<=$Total_balance) && ($Gift_payment_balance >= $Purchase_amount))
					{
							...
							$variablesarray["data"]= array('productId'=>$productId,'quantity'=>$quantity,'denomination'=>$denomination,'email'=>$email,'contact'=>"",'tag'=>"",'poNumber'=>$poNumber,'notifyAdminEmail'=>1);  
							$arr1 = array('query' =>'plumProAPI.mutation.placeOrder', 'tag' =>'plumProAPI', 'variables' =>$variablesarray);
															...
															foreach($value1 as $value2){									
																...
																if($Tier_redemption_ratio > 0){
																	...
																	$Insert_trasaction_data = array(
																		...
																		// 'Weight' => $this->input->post('Item_Weight'),
																		// 'Weight_unit_id' => $this->input->post('Weight_unit_id'),			
																		'Cost_payable_partner' =>number_format($Paid_amount,2),// $orderTotal,			
