	...
	public function index()
	{
		...
		foreach($Company_details as $Company_Records)
		{
			if($Company_Records["Cron_points_expiry_flag"]==1)
			{
				...
				$Trans_Records = $this->Auto_process_model->get_cust_last_points_used($Company_Records["Company_id"]);
				...
				if($Trans_Records !=NULL && $Company_Records["Deduct_points_expiry"]!=0.00 && $Company_Records["Points_expiry_period"]!=0)
				{
					...
					echo "<br><br><b>***************Inside**********************</b><br>Last date->".$Trans_Records->Trans_date."*--*Trans_id-->".$Trans_Records->Trans_id."*-*Redeem_points-->".$Trans_Records->Redeem_points."*-*Trans_type-->".$Trans_Records->Trans_type."*-*Card_id-->".$Trans_Records->Card_id."*--*num_days-->".$num_days."*--*Current_balance-->".$Current_balance."*--*Total_Current_balance-->".$Total_Current_balance."<br><br>";
					...
					if(($num_days==$Company_Records["Points_expiry_period"]-30) || ($num_days==$Company_Records["Points_expiry_period"]-60) || ($num_days==$Company_Records["Points_expiry_period"]-90))
					{
						$Email_content = array(
							...
							'Template_type' => 'Points_Expiry_Reminder'
						);
					...
					if($num_days>=$Company_Records["Points_expiry_period"] && $Total_Current_balance > 0) 
					{
						...
						/******************Insert in transaction*******************************/
							$result21 = $this->Auto_process_model->Insert_points_expiry_trans($Company_id,$Enrollement_id,$Deduct_balance,$Trans_Records->Card_id);
						/****************************************************************/
