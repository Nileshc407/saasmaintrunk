		...
		if($this->session->userdata('logged_in'))
		{
			...
			{	
				...
				/*-----------------------------------Image1-----------------------------*/
					...
					if($this->upload->display_errors())
					{
						if($data22['file_name']!='')
						{
							...
							redirect('Voucher_creation/', 'refresh');	
						}
					}
				...
				/*-----------------------------------Image4-----------------------------*/	
					...
					if(   is_numeric($_REQUEST['Voucher_type']) != 1  || is_numeric($_REQUEST['show_item']) != 1 )
					{
						$this->session->set_flashdata("error_code","Invalid data");
						redirect('Voucher_creation/', 'refresh');	
					}
					if($this->input->post('Merchandize_Partner_ID')){
						$Merchandize_Partner_ID=$this->input->post('Merchandize_Partner_ID');
						if(is_numeric($Merchandize_Partner_ID) != 1)
						{
							$this->session->set_flashdata("error_code","Invalid Merchandize Partner");
							redirect('Voucher_creation/', 'refresh');	
						}
					}
					if($this->input->post('Cost_price')){
						$Cost_price=$this->input->post('Cost_price');
						if(is_numeric($Cost_price) != 1)
						{
							$this->session->set_flashdata("error_code","Invalid Cost_price");
							redirect('Voucher_creation/', 'refresh');	
						}
					} else {
						$Cost_price=0;
					...
					if($this->input->post('Points')){
						...
						{
							$this->session->set_flashdata("error_code","Invalid Points");
							redirect('Voucher_creation/', 'refresh');	
						}
					} else {
						$Points=0;
					...
					if($Cost_in_points > 0){
						...
						if(is_numeric($Cost_in_points) != 1)
						{
							$this->session->set_flashdata("error_code","Invalid Cost_in_points");
							redirect('Voucher_creation/', 'refresh');	
						}
					} else {
						$Cost_in_points=0;
					...
					if($this->input->post('Discount_percentage')){
						...
						if(is_numeric($Discount_percentage) != 1)
						{
							$this->session->set_flashdata("error_code","Invalid Discount_percentage");
							redirect('Voucher_creation/', 'refresh');	
						}
					} else {
						$Discount_percentage=0;
					}
					if($this->input->post('Voucher_issuance_type')){
						$Voucher_issuance_type=$this->input->post('Voucher_issuance_type');
						if(is_numeric($Voucher_issuance_type) != 1)
						{
							$this->session->set_flashdata("error_code","Invalid Voucher_issuance_type");
							redirect('Voucher_creation/', 'refresh');	
						}
					} else {
						$Voucher_issuance_type=0;
					}
					...
					echo"---Discount_percentage-----".$Discount_percentage."---<br>"; 
					echo"---Voucher_issuance_type-----".$Voucher_issuance_type."---<br>"; 
					die; */
					...
					$_REQUEST['Voucher_code'] = preg_replace('/[^A-Za-z0-9\-]/', ' ', $_REQUEST['Voucher_code']);
					// $_REQUEST['Voucher_name'] = preg_replace('/[^A-Za-z0-9\-]/', ' ', $_REQUEST['Voucher_name']);
					// $_REQUEST['Voucher_description'] = preg_replace('/[^A-Za-z0-9\-]/', ' ', $_REQUEST['Voucher_description']);	
					...
					$Post_data=array(
						...
						'Partner_id'=>$this->input->post('Merchandize_Partner_ID'),
						'Voucher_issuance_type'=>$Voucher_issuance_type,
						'Voucher_name'=>$_REQUEST['Voucher_name'],
						'Voucher_description'=>$_REQUEST['Voucher_description'],
					...
					if($result)
					{
						...
						if($POS_item) {
							...
							for($i=0; $i<count($POS_item); $i++){
								...
								$today=date("Y-m-d H:i:s");
								if(is_numeric($POS_item[$i]) != 1 )
								{
									$this->session->set_flashdata("error_code","Invalid POS Item");
									redirect('Voucher_creation/', 'refresh');	
								}
						...
						/*******************Insert igain Log Table*********************/
							...
							$opval = $_REQUEST['Voucher_name'].' ( '.$_REQUEST['Voucher_code'].' )';
						...
						$this->session->set_flashdata("success_code"," ".$_REQUEST['Voucher_name']." Voucher Created Successfuly!!");
					}
					else
					{
						$this->session->set_flashdata("error_code","Error creating ".$_REQUEST['Voucher_name']." voucher");
					}			
					redirect("Voucher_creation");
			}
		}
		...
		if($this->session->userdata('logged_in'))
		{
			...
			{	
				...
				if($data["VoucherDetails"]){
					...
					$Partner_id=$data["VoucherDetails"]->Partner_id;
					if($Partner_id!=0)
					{
						$get_partner_detail = $this->Catelogue_model->Get_Company_Partners_details($Partner_id);
						$data['Partner_id'] = $get_partner_detail->Partner_id;
						$data['Partner_name'] = $get_partner_detail->Partner_name;
					}
					else
					{
						$data['Partner_id'] = 0;
			...
			/***********************************/
				...
				/*-----------------------------------Image1-----------------------------*/
					...
					else
					{
						...
						{
							$Item_image1 =$_SESSION['Item_image1'];								
						}
					}
					if($this->upload->display_errors())
					{
					...
					{							
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Thumbnail_image1 =$_SESSION['Thumbnail_image1'];								
						}
					}
					if($this->upload->display_errors())
					{
				...
				/*-----------------------------------Image2-----------------------------*/
					...
					else
					{
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Item_image2 =$_SESSION['Item_image2'];								
						}
					}
					if($this->upload->display_errors())
					{
					...
					{							
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Thumbnail_image2 =$_SESSION['Thumbnail_image2'];								
						}
					}
					if($this->upload->display_errors())
					{
				...
				/*-----------------------------------Image3-----------------------------*/
					...
					else
					{
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Item_image3 =$_SESSION['Item_image3'];								
						}
					}
					if($this->upload->display_errors())
					{
					...
					{							
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Thumbnail_image3 =$_SESSION['Thumbnail_image3'];								
						}
					}
					if($this->upload->display_errors())
					{
				...
				/*-----------------------------------Image4-----------------------------*/
					...
					else
					{
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Item_image4 =$_SESSION['Item_image4'];								
						}
					}
					if($this->upload->display_errors())
					{
					...
					{							
						...
						if(isset($_SESSION["Voucher_id"]))
						{
							$Thumbnail_image4 =$_SESSION['Thumbnail_image4'];								
						}
					}
					if($this->upload->display_errors())
					{
				...
				/*-----------------------------------Image Upload Code-----------------------------*/
					...
					// echo"---Company_Redemptionratio-----".$Company_Redemptionratio."---<br>";
					if( is_numeric($_REQUEST['show_item']) != 1 )
					{
						$this->session->set_flashdata("error_code","Invalid data");
						redirect('Voucher_creation/', 'refresh');	
					}
					...
					$Update_data=array(
						...
						'Valid_till'=>$Valid_till,						
						'Voucher_name'=>$_REQUEST['Voucher_name'],
						'Voucher_description'=>$_REQUEST['Voucher_description'],'Voucher_issuance_type'=>$Voucher_issuance_type,					
						'Item_image1'=>$Item_image1,
						'Item_image2'=>$Item_image2,
						'Item_image3'=>$Item_image3,
						'Item_image4'=>$Item_image4,
					...
					{						
						...
						if($POS_item){
							...
							$DeleteProductVoucherItemChild = $this->Voucher_model->delete_product_voucher_item_child($Company_id,$POS_item,$Voucher_id,$_SESSION['Voucher_type'],$_SESSION['Voucher_code']);
							// die;
							for($i=0; $i<count($POS_item); $i++){
								...
								$Merchandize_Item_details = $this->Voucher_model->Get_Merchandize_Item_details($POS_item[$i],$today);
								$CheckProductVoucherItemChild = $this->Voucher_model->Check_product_voucher_item_child($Company_id,$POS_item[$i],$Voucher_id,$_SESSION['Voucher_type'],$_SESSION['Voucher_code']);
								...
								if($CheckProductVoucherItemChild){									
									...
									$update_child_data=array(
										...
										'Voucher_code'=>$_SESSION['Voucher_code'],				'Voucher_issuance_type'=>$Voucher_issuance_type,
										'Company_merchandise_item_id'=>$Merchandize_Item_details->Company_merchandise_item_id,
										'Company_merchandize_item_code'=>$Merchandize_Item_details->Company_merchandize_item_code,
										'Valid_from'=>$Valid_from,
										'Valid_till'=>$Valid_till,
								...
								$CheckRevenueVoucher= $this->Voucher_model->Check_revenue_voucher_child($Company_id,$Voucher_id,$_SESSION['Voucher_type'],$_SESSION['Voucher_code']);
						...
						/*******************Insert igain Log Table*********************/
							...
							$lastName = '';
							$Seller_name = $session_data['Full_name'];
							$opval = $_REQUEST['Voucher_name'];
							$result_log_table = $this->Igain_model->Insert_log_table($Company_id,$enroll,$username,$Seller_name,$Todays_date,$what,$where,$userid,$opration,$opval,$firstName,$lastName,$enroll);
						/*******************Insert igain Log Table*********************/
						
						$this->session->set_flashdata("data_code",$this->upload->display_errors());
						$this->session->set_flashdata("success_code"," ".$_REQUEST['Voucher_name']." Voucher Updated Successfuly!!");
					}
					else
					{
						$this->session->set_flashdata("error_code","Error updating ".$_REQUEST['Voucher_name']." voucher");
					}			
					redirect("Voucher_creation");
