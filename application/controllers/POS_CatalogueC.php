...
class POS_CatalogueC extends CI_Controller
{
	...
	public function Create_POS_items()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$data["Allergies_Code_decode"] = $this->POS_catalogue_model->Get_company_Code_decode_master(18,$Company_id);
			$data["Req_cond_group"] = $this->POS_catalogue_model->Get_req_opt_cond_group(14,$Company_id,0);
			$data["Opt_cond_group"] = $this->POS_catalogue_model->Get_req_opt_cond_group(15,$Company_id,0);
			...
			if(!is_dir('Merchandize_images'))
			{
				...
				/*-----------------------File Upload---------------------*/
			$data["Merchandize_Items_Records"] = $this->Catelogue_model->Get_Merchandize_Items('', '',$Company_id,1);
			$data["InActive_Merchandize_Items_Records"] = $this->Catelogue_model->Get_Merchandize_Items('', '',$Company_id,0);
			if($_POST == NULL)
			{
				/* $get_sellers15 = $this->Igain_model->get_seller_details($session_data['enroll']);
				...
				if($get_sellers15 != NULL)
				{
					...
					{
						$superSellerFlag = $seller_val->Super_seller;
						$Sub_seller_adminFlag = $seller_val->Sub_seller_admin;
						$Sub_seller_Enrollement_idFlag = $seller_val->Sub_seller_Enrollement_id;
					}
				} */			
				// echo "<br>Logged_user_id ".$Logged_user_id;
				// echo "<br>SuperSellerFlag ".$SuperSellerFlag;
				/* if($Logged_user_id > 2 || $SuperSellerFlag == 1)
				{
					$get_sellers = $this->Igain_model->get_company_sellers($Company_id);
				}
			...
			{	
				...
				/*-----------------------------------Image4-----------------------------*/
					...
					{							
						...
						/**********Insert linked Outlets*****************************/
						$LINKED_Outlet_id=$_REQUEST['Outlet_id'];
						if($LINKED_Outlet_id != NULL)
						{
							foreach($LINKED_Outlet_id as $outlet)
							{
								$Post_data29=array(
								'Company_id'=>$Company_id,
								'Merchandize_item_code'=>$this->input->post('Company_merchandize_item_code'),
								'Brand_id'=>$_REQUEST['seller_id'],
								'Outlet_id'=>$outlet,
								'Create_user_id'=> $data['enroll'] ,
								'Create_date'=> date('Y-m-d H:i:s'));
								$result29 = $this->POS_catalogue_model->Insert_pos_linked_outlets($Post_data29);
							}
						}
				...
				/********Insert Allergies ,Required Condiment Group,Optional Condiment Group***********/
					...
					// echo 'Item_type '.$_REQUEST['Item_type'];
				...
				'Partner_id'=>$Partner_records->Partner_id,
				'Seller_id'=>$this->input->post('seller_id'),//$Seller_id,
	...
	public function Edit_POS_Items()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			//******** sandeep *************
			$seller_id=$data["Merchandize_Item_Row"]->Seller_id;// echo $seller_id."<br>";
			...
			//echo "inserted id--".$data["Merchandize_Item_Row"]->Merchandize_category_id;
			//print_r($data["Merchandize_Category_Records"]);
			...
			$data['Get_Brand_outlets'] = $this->POS_catalogue_model->Get_pos_linked_Brand_outlets($Company_id,$seller_id,$Company_merchandize_item_code);
		//	 print_r($data['Get_pos_main_items']);//die;
			if($_POST == NULL)
			{
				//*********** SANDEEP 13-03-2020 **********			
			...
			else
			{
				...
				/*-----------------------------------Image4-----------------------------*/		
					...
					/**********Insert linked Outlets*****************************/
						$LINKED_Outlet_id=$_REQUEST['Outlet_id'];
						$del_outlets = $this->POS_catalogue_model->Delete_pos_linked_outlets($_REQUEST['seller_id'],$_REQUEST['Company_merchandize_item_code']);
	...
	public function get_seller_opt_cond_groups()
	{
		$seller_id = $this->input->post('seller_id');
		$Company_id = $this->input->post('Company_id');
		...
		$data["Opt_cond_group"] = $this->POS_catalogue_model->Get_req_opt_cond_group(15,$Company_id,$seller_id);

		
			$opText3 = $this->load->view("POS_Catalogue/get_seller_opt_cond_group",$data, true);
		}
		...
		{
			$this->output->set_content_type('text/html');
			$this->output->set_output($opText3);
		}
	}
	
			
	public function get_seller_req_cond_groups()
	{
		$seller_id = $this->input->post('seller_id');
		$Company_id = $this->input->post('Company_id');
		...
		if($seller_id > 0)
		{
		$data['Req_cond_group'] = $this->POS_catalogue_model->Get_req_opt_cond_group(14,$Company_id,$seller_id);
		
			$opText4 = $this->load->view("POS_Catalogue/get_seller_req_cond_group",$data, true);
	...
	public function get_outlet_by_subadmin()
	{
		...
		if($seller_id > 0)
		{
			$data['Outlets'] = $this->POS_catalogue_model->get_outlet_by_subadmin($Company_id,$seller_id);
		
			$opText4 = $this->load->view("POS_Catalogue/get_outlet_by_subadmin",$data, true);
