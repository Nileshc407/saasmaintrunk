	...
	public function index()
	{
		...
		foreach($Company_details as $Company_Records)
		{
			...
			if($Company_Records["Cron_tier_flag"]==1 )
			{
				...
				$Tiers_Record = $this->Auto_process_model->Tier_list($Company_Records["Company_id"]);
			
				if($Tiers_Record !=NULL)
				{
					foreach($Tiers_Record as $Tiers_Record)
					{
						/*
						...
						*/
						...
						$Tier_Business_flag = $Tiers_Record->Tier_Business_flag; 
						
						echo "<br><br>Tier_id : $Tier_id --- Tier_Business_flag : $Tier_Business_flag --- Tier_name : $Tier_name ---  Tier_level_id : $New_Tier_level_id ---  Tier_criteria : $Tier_criteria ---   Tier_criteria : $Tier_criteria ---  Criteria_value : $Criteria_value --- ";
						...
						{
							$Trans_Records31 = $this->Auto_process_model->Get_Cumulative_spend_transactions($Company_id,$From_date,$Todays_date,$Tier_Business_flag);
							
						}
						if($Tier_criteria==2)//Cumulative Number of Transactions
						{
							$Trans_Records31 = $this->Auto_process_model->Get_Cumulative_Total_transactions($Company_id,$From_date,$Todays_date,$Tier_Business_flag);
						}
						if($Tier_criteria==3)//Cumulative Points Accumlated
						{
							$Trans_Records31 = $this->Auto_process_model->Get_Cumulative_Points_Accumlated($Company_id,$From_date,$Todays_date,$Tier_Business_flag);
						}
						if($Tier_criteria==4)//Tenor - No. of Days
						{
							$From_date = strtotime($Todays_date .' -'.$Criteria_value.' days');
							$From_date=date("Y-m-d",$From_date);
							$Trans_Records2 = $this->Auto_process_model->Get_Cumulative_spend_transactions($Company_id,$From_date,$Todays_date,$Tier_Business_flag);
							$Criteria=$Trans_Records2->Total_Spend;
							$Enrollement_id=$Trans_Records2->Enrollement_id;
							// echo "<br><b>Total_Spend -->".$Criteria."</b>";
						}
						// print_r($Trans_Records31);
					// die;
						
						if($Trans_Records31 != NULL)
						{
						...
						foreach($lv_Get_Customer_Tier as $Rec_tier)
						{
							if($Rec_tier->Process!="Retention")
							{
								...
								/*********************************************************************/
									...
									echo "<br><b>Tier_notification Tier_id -->".$Rec_tier->Tier_id."<-->Old_Tier_name -->".$Rec_tier->Old_Tier_name."<-->New Tier_name -->".$Rec_tier->Tier_name."<-->Enrollement_id -->".$Rec_tier->Enrollement_id."<-->Process -->".$Rec_tier->Process."</b>";
