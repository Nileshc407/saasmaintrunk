...
class Enrollement_data_transform extends CI_Controller 
{
	...
	public function Enrollement_data_mapping()
	{
		...
		{			
			...
			$data["results"] = $this->Enrollement_data_map->data_map_list($Company_id,$data['Super_seller'],$Logged_user_enrollid,$data['Sub_seller_Enrollement_id']);
	...
	public function insert_data_map()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$exist_merchant_array=array();
				...
				// $result_count = $this->Enrollement_data_map->check_exist_data_map_seller($Column_outlet_id,$Company_id);
				...
				if(is_numeric($_REQUEST['Column_Seller_id']) != 1  || is_numeric($_REQUEST['Column_header_rows']) != 1  || is_numeric($_REQUEST['Column_First_Name']) != 1  || is_numeric($_REQUEST['Column_Last_Name']) != 1  ||  is_numeric($_REQUEST['Column_Date']) != 1   || is_numeric($_REQUEST['Column_Address']) != 1  || is_numeric($_REQUEST['Column_Phone_No']) != 1  || is_numeric($_REQUEST['Column_User_Email_ID']) != 1 || is_numeric($_REQUEST['Column_Membership_ID']) != 1)
				{
					$this->session->set_flashdata("error_code","Invalid data");
					redirect('Enrollement_data_transform/Enrollement_data_mapping', 'refresh');	
				}
				...
				if($Outlets!=NULL && $Outlets[0] > 0)
				{
					foreach($Outlets as $outlet_id)
					{
						...
						$result_count2 = $this->Enrollement_data_map->check_exist_data_map_seller($outlet_id, $Company_id);
	...
	public function update_data_map()
	{
		if($this->session->userdata('logged_in'))
		{
			if(is_numeric($_REQUEST['Column_header_rows']) != 1  || is_numeric($_REQUEST['Column_First_Name']) != 1  || is_numeric($_REQUEST['Column_Last_Name']) != 1  ||  is_numeric($_REQUEST['Column_Date']) != 1   || is_numeric($_REQUEST['Column_Address']) != 1  || is_numeric($_REQUEST['Column_Phone_No']) != 1  || is_numeric($_REQUEST['Column_User_Email_ID']) != 1 || is_numeric($_REQUEST['Column_Membership_ID']) != 1)
				{
					$this->session->set_flashdata("error_code","Invalid data");
					redirect('Enrollement_data_transform/Enrollement_data_mapping', 'refresh');	
				}
	...
	public function upload_data()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			{	
				...
				$file_status = $this->Enrollement_data_map->upload_data_map_file($filepath,$filename,$Company_id,$_REQUEST['outlet_id']);
				...
				$data['UploadData'] = $this->Enrollement_data_map->get_upload_file_data($_REQUEST['outlet_id']);
				...
				// $data['lp_array'] = $ARRAY_OF OUTLET_SDJ$this->Igain_model->get_loyalty_rule($Company_id,$Logged_user_enrollid,$Logged_user_id);
				...
				/*********************************************************************/	
				 ...
				 $data['enroll'] = $session_data['enroll'];	
   				 ...
   				 // redirect('Enrollement_data_transform/verify_data_file');
				 // echo"--Data_uploads-----";
	...
	public function verify_data_file()
	{
		...
		if($this->session->userdata('logged_in'))
		{
			...
			// echo"hrer......";
				...
				$filename=$file_enroll_session['filename'];
				$data['file_status']=$file_enroll_session['file_status'];
				$data['outlet_id']=$file_enroll_session['outlet_id'];
				// echo"----filename----".$filename."---<br>";
			
			
			if($data['Super_seller']==1)
			{
				$seller_id=$file_enroll_session['outlet_id'];
			}
			else
			{
			...
			if($_POST == NULL)
			{
				...
				$data['Upload_errors'] = $this->Enrollement_data_map->get_upload_errors($filename,$Company_id);
				$data['UploadData'] = $this->Enrollement_data_map->get_upload_file_data($data['outlet_id']);					
				$this->load->view("Enrollement_data_transform/verify_enrollement_data_file",$data);
			}
			else
			{	
				$data["DataMaping"] = $this->Enrollement_data_map->get_datamaping_details($Company_id,$Logged_user_id,$file_enroll_session['outlet_id']);				
				...
				$UploadData = $this->Enrollement_data_map->get_upload_file_data_for_transaction($file_enroll_session['outlet_id']);		
				...
				foreach($UploadData as $row)
				{
						...
						$PhoneNo = strip_tags($row->Phone_No);
						$UserEmailID = $row->User_Email_ID;
						$MembershipID = strip_tags($row->Membership_ID);
						// $RefreeMembershipID = $row->Refree_Membership_ID;
						$JoiningDate  = $row->Date;
						
					if($PhoneNo != 0 && $UserEmailID !="" && $MembershipID != '' )
					{
						...
						{	
							...
							if($check_PhoneNo_exists == 0)
							{
								...
								{					
										...
										$user_details = $this->Igain_model->get_enrollment_details($file_enroll_session['outlet_id']);
										...
										$Seller_details = $this->Igain_model->get_enrollment_details($file_enroll_session['outlet_id']);
										...
										$Enrolled_Card_id=$cardid;
											...
											// echo"---Joining_bonus_flag------".$Joining_bonus_flag--."<br><br>";
											// echo"---cardid------".$cardid--."<br><br>";
										if($Joining_bonus_flag == 1 && $cardid != "")
										{								
											if($Coalition == 1 )
											{
				...
				$Drop_upload_file = $this->Enrollement_data_map->Drop_upload_file_data($file_enroll_session['outlet_id']);
