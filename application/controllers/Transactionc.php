...
error_reporting(0);
  ...
  class Transactionc extends CI_Controller {
      ...
      if ($this->session->userdata('logged_in')) {
        ...
        if ($_POST == NULL) {
          ...
          $this->load->view('transactions/issue_bonus', $data);
            ...
            $cardis = strip_tags($this->input->post("cardId"));
            ...
            $phnumber = $dial_code . strip_tags($this->input->post("cardId"));
	...
	public function issue_wallet_credit() 
	{
      if ($this->session->userdata('logged_in')) {
        $session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
        ...
        /* -----------------------Pagination--------------------- */
        $config = array();
        $config["base_url"] = base_url() . "/index.php/Transactionc/issue_wallet_credit";
        $total_row = $this->Transactions_model->wallet_credit_trans_count($Logged_user_id, $data['enroll'], $data['Company_id']);
        $config["total_rows"] = $total_row;
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config['next_link'] = 'Next';
        ...
        if ($_POST == NULL) {
          $data["results"] = $this->Transactions_model->wallet_credit_trans_list($config["per_page"], $page, $Logged_user_id, $data['enroll'], $data['Company_id']);
          $data["pagination"] = $this->pagination->create_links();
          $this->load->view('transactions/issue_credit', $data);
        } else {
              ...
              if ($cardis != '0') {
                if ($cardId == $cardis || $Phone_no == $phnumber) {
                  ...
                  foreach ($cust_details as $row25) {
                    ...
                    $Tier_name = $row25['Tier_name'];
                    $MembershipID = $row25['Card_id'];
					$Wallet_balance = $row25['Wallet_balance'];
                    $Current_point_balance = $bal - ($Blocked_points + $Debit_points);
                  ...
                  $this->load->view('transactions/issue_credit_transaction', $data);
                } else {
                  $this->session->set_flashdata("error_code", "Sorry, Cannot proceed with the Transaction!! Your Membership ID is not registered with us...! !");
                  redirect(current_url());
                }
      ...
      if ($this->session->userdata('logged_in')) {
		...
		else 
		{
			$Required_filed = array(
				array(
						'field' => 'manual_bill_no',
						'label' => 'manual_bill_no',
						'rules' => 'required'
				),
				array(
						'field' => 'topup_amt',
			...
			$topup_amt = strip_tags($this->input->post("topup_amt"));
			$check_topup_amt = is_numeric($topup_amt);
			if($check_topup_amt !=1)
			{
				$this->session->set_flashdata("error_code", "Please Enter Valid Data");
                redirect('Transactionc/issue_bonus');
			}
          ...
          if ($this->input->post("topup_amt") == "" || $this->input->post("topup_amt") <= 0 || $this->input->post("topup_amt") == " ") {
           if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("error_code", "Sorry, Meal Topup Failed. Invalid Data Provided!!");}else{ $this->session->set_flashdata("error_code", "Sorry, Issue Bonus Transaction Failed. Invalid Data Provided!!");}
            redirect('Transactionc/issue_bonus');
				...
				$result = $this->Transactions_model->check_bill_no(strip_tags($this->input->post("manual_bill_no")),$Company_id);				
				if ($result > 0) {
            ...
            $manual_bill_no = strip_tags($this->input->post("manual_bill_no"));
            $logtimezone = $data['timezone_entry'];

            $dial_code = $this->Enroll_model->get_dial_code($data['Country_id']);
            $phnumber = $dial_code . strip_tags($this->input->post("cardId"));

            $member_details = $this->Transactions_model->issue_bonus_member_details($data['Company_id'], strip_tags($this->input->post("cardId")), $phnumber);
            foreach ($member_details as $rowis) {
              $cardId = $rowis['Card_id'];
            }
            if ($data['userId'] == 3) {
            ...
            $post_data = array(
                ...
                'Remarks' => strip_tags($this->input->post('remark')),
                'Card_id' => $cardId,
            ...
            $toname = "";
            $opval = 'Bill No.:'.$this->input->post('manual_bill_no').', Remarks:'.strip_tags($this->input->post('remark')).','.strip_tags($this->input->post("topup_amt")) . ' Points.'; // topup_amt
	...
	public function issue_credit_transaction() 
	{
      if ($this->session->userdata('logged_in')) {
        $session_data = $this->session->userdata('logged_in');
        $data['username'] = $session_data['username'];
        ...
        if ($_POST == NULL) {
          ...
          redirect('transactions/issue_wallet_credit');
        } else {
          if ($this->input->post("topup_amt") == "" || $this->input->post("topup_amt") <= 0 || $this->input->post("topup_amt") == " ") {
           if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("error_code", "Sorry, Meal Topup Failed. Invalid Data Provided!!");}else{ $this->session->set_flashdata("error_code", "Sorry, Issue Bonus Transaction Failed. Invalid Data Provided!!");}
            redirect('Transactionc/issue_wallet_credit');
          } else {
            ...
            foreach ($cust_details as $row25) {
              $card_bal = $row25['Current_balance'];
              $wall_bal = $row25['Wallet_balance'];
              $enroll_id = $row25['Enrollement_id'];
              $Total_wallet_topup = $row25['Total_wallet_topup'];
              $topup = $row25['Total_topup_amt'];
              $purchase_amt = $row25['total_purchase'];
              $reddem_amt = $row25['Total_reddems'];
            }
			...
			$available_wallet_bal = $wall_bal + $this->input->post("topup_amt");
			
            $post_data = array(
                'Trans_type' => '30',
                'Company_id' => $data['Company_id'],
                // 'Topup_amount' => $this->input->post("topup_amt"),
                'Wallet_amount' => $this->input->post("topup_amt"),
                'Trans_date' => $lv_date_time,
                ...
                'remark2' => $remark_by,
                'Loyalty_pts' => '0',
				'Available_wallet_balance' => $available_wallet_bal
            );
            $result = $this->Transactions_model->insert_topup_details($post_data);

            $curr_bal = $wall_bal + $this->input->post("topup_amt");
            $topup_amt = $topup + $this->input->post("topup_amt");
            $purchase_amount = $purchase_amt;
            $reddem_amount = $reddem_amt;
			
			$Wallet_topup_amt = $Total_wallet_topup + $this->input->post("topup_amt");
           
            $result2 = $this->Transactions_model->update_customer_wallet_balance($data['Company_id'],$enroll_id,$available_wallet_bal,$Wallet_topup_amt);
            ...
            $Email_content = array(
                'Todays_date' => $lv_date_time,
                'topup_amt' => number_format($this->input->post("topup_amt"),2),
                'manual_bill_no' => $this->input->post("manual_bill_no"),
                'Symbol_currency' => $Symbol_currency,
                // 'Notification_type' => 'Bonus ' . $data['Company_details']->Currency_name,
                'Notification_type' => 'Wallet Credit Transaction at '.$data["Company_details"]->Company_name,
				'Remarks' => $this->input->post('remark'),
                'Template_type' => 'Add_wallet_balance'
            );
            $this->send_notification->send_Notification_email($enroll_id, $Email_content, $seller_id, $data['Company_id']);
            if (($result == true) && ($result2 == true) && ($result4 == true)) {
				if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("success_code", "Meal Topup Transaction done Successfully!!");}else{ $this->session->set_flashdata("success_code", "Wallet Topup Transaction done Successfully!!");}
              
            } else {
					if($Daily_points_consumption_flag==1 || $Enable_company_meal_flag==1){ $this->session->set_flashdata("error_code", "Sorry, Meal Topup Transaction Failed!!");}else{ $this->session->set_flashdata("error_code", "Sorry, Wallet Topup Transaction Failed!!");}
    ...
    public function transaction_receipt() {
      ...
      foreach ($transaction_details as $transaction) {
        ...
        if ($transtype == 2) {
          ...
          $data['Voucher_discount'] = $transaction['Voucher_discount'];
          $data['Giftcard_amt'] = $transaction['Giftcard_amt'];
          /*           * *********** */
		  $data['remark3'] = $transaction['remark3'];
	...
	public function check_redeem_bill_no() {
      $result = $this->Transactions_model->check_redeem_bill_no($this->input->post("Bill_no"), $this->input->post("Company_id"));
	...
	public function check_waller_credit_bill_no() {
      $result = $this->Transactions_model->check_waller_credit_bill_no($this->input->post("Bill_no"), $this->input->post("Company_id"));
	...
	public function check_waller_debit_bill_no() {
      $result = $this->Transactions_model->check_waller_debit_bill_no($this->input->post("Bill_no"), $this->input->post("Company_id"));
    ...
    public function transact_giftcard() {
      ...
      if ($this->session->userdata('logged_in')) {
		...
		else 
		{
		   ...
		   /*------Manual_bill_no Checking---12-08-2019--*/
				$result = $this->Transactions_model->check_bill_no(strip_tags($this->input->post("Billno")),$Company_id);				
				if ($result > 0) {
          ...
          $manualbillno = strip_tags($this->input->post("Billno"));
          $Purchase = strip_tags($this->input->post("Purchase"));
          $GiftCardNo = strip_tags($this->input->post("GiftCardNo"));
          $Balance = strip_tags($this->input->post("Balance"));
          $Redeem = strip_tags($this->input->post("Redeem"));
          $current_balance = $Balance - $Redeem;
          $BalanceToPay = ($Purchase - $Redeem);
          $payment_by = strip_tags($this->input->post("payment_by"));
          // $companyid = $this->input->post("Company_id");
          $remark = strip_tags($this->input->post("Cheque"));
		 ...
		 $giftcard_details = $this->Transactions_model->get_giftcard_details($GiftCardNo, $Company_id);
		  ...
		  if($Balance != $Card_balance)
		  {
			  $this->session->set_flashdata("error_code", "Transaction failed due to Invalid Gift Card Balance.");
			  redirect('Transactionc/transact_giftcard');
		  }
          ...
          if ($GiftCardNo != '' && $Purchase != '') {
            ...
            if ($giftcard_details_count > 0) {
              ...
              $result2 = $this->Transactions_model->update_giftcard_balance($GiftCardNo, $current_balance, $companyid,$data['enroll']);
    ...
    function validate_card_id() {
      $result = $this->Transactions_model->check_card_id(strip_tags($this->input->post("cardid")), strip_tags($this->input->post("Company_id")));
    ...
    function validate_gift_card_id() {
      $result = $this->Transactions_model->check_gift_card_id(strip_tags($this->input->post("gift_card")), strip_tags($this->input->post("Company_id")));
      if ($result > 0) {
        $this->output->set_output("InValid");
      } else {
        $this->output->set_output("Valid");
    ...
    public function assign_giftcard() {
      if ($this->session->userdata('logged_in')) {
		...
		else 
		{
		  ...
		  $result = $this->Transactions_model->check_gift_card_id(strip_tags($this->input->post("gif_number")), $Company_id);
		  if ($result > 0) {
          ...
          $Gift_card_id = strip_tags($this->input->post("gif_number"));
          ...
          if ($result == true) {
            ...
            if ($Membership_id == '0') {
              ...
              $lastName = strip_tags($this->input->post("user_name"));
            } else {
              $get_cust_detail = $this->Igain_model->get_enrollment_details($Enrollment_id);
    ...
    public function loyalty_transaction() 
	{
      if ($this->session->userdata('logged_in')) {
		...
		{
			$seller_details = $this->Igain_model->get_enrollment_details($Sub_seller_Enrollement_id);
			$Logged_user_enrollid = $seller_details->Sub_seller_Enrollement_id;
		}
		
		$data['Logged_user_enrollid'] = $Logged_user_enrollid;
		...
		else 
		{
          ...
          if ($Lp_count > 0) {
            // if ($categoryexist > 0) {
              $cardis = strip_tags($this->input->post("cardId"));
              $_SESSION["cardId"] = strip_tags($this->input->post("cardId"));
              if (substr($cardis, 0, 1) == "%") {
                $get_card = substr($cardis, 2); //*******read card id from magnetic card***********///
              } else {
                $get_card = substr($cardis, 0, 16); //*******read card id from other magnetic card***********///
                ...
                if ($cardis != '0') {
                  if ($cardId == $cardis || $Phone_no == $phnumber) {
                    ...
                    foreach ($cust_details as $row25) {
					  ...
					  $data['Member_Tier_id'] = $Member_Tier_id;
                    }
                    ...
                    if ($lp_array != "") {
						...
						{
							$this->load->view('transactions/loyalty_purchase_transaction_VITS', $data);
						}
						else
						{
	...
	{ VoucherNumber
      if ($this->session->userdata('logged_in')) {
		...
		{
			$Promo_redeem_by = strip_tags($this->input->post("Promo_redeem_by"));
			$Gift_redeem_by = strip_tags($this->input->post("Gift_redeem_by"));
			$Redeem_voucher_flag = strip_tags($this->input->post("Redeem_voucher_flag"));
			$gift_reedem = 0;
			$PromoRedeem = 0;
			$VoucherRedeem = 0;
			$PromoRedeem_amount = 0;
		  ...
		  else 
		  {
			...
			if ($Gift_redeem_by == 1) {$gift_reedem = strip_tags($_REQUEST['gift_points_redeemed']);}
			if ($Promo_redeem_by == 1) {$PromoRedeem = strip_tags($_REQUEST['promo_points_redeemed']);
			$PromoRedeem_amount = ($PromoRedeem / $redemptionratio);}
			if($Redeem_voucher_flag==1){ $VoucherRedeem = strip_tags($_REQUEST['VoucherRedeemAmount']); }	 
			
            $loyalty_points = 0;
			if($Logged_user_id==7)//Finanace
			{
				$user_details = $this->Igain_model->get_enrollment_details($Sub_seller_Enrollement_id);
				$redemptionratio = $user_details->Seller_Redemptionratio;
			}
            $reedem_points = strip_tags($this->input->post("points_redeemed"));
            
            $redeem_amount = ($reedem_points / $redemptionratio);
			
            $Discount_amt = strip_tags($this->input->post("Discount_amt"));
            // $Discount_amt = strip_tags($this->input->post("Discount_amt1"));
            // $Discount_amt = 0;
			$purchase_amtXX = strip_tags($this->input->post("purchase_amt"));
            $bal_pay = ($purchase_amtXX - $Discount_amt - $redeem_amount-$VoucherRedeem-$gift_reedem-$PromoRedeem_amount);
			...
			/*------Manual_bill_no Checking---12-08-2019--*/
				$manual_bill_noXX = strip_tags($this->input->post("manual_bill_no"));
				$result = $this->Transactions_model->check_bill_no($manual_bill_noXX,$Company_id);				
				if ($result > 0) 
				{
					$this->session->set_flashdata("error_code", "Transaction failed due to Invalid Bill No.");
					redirect('Transactionc/loyalty_transaction');
              ...
              if ($data['userId'] == 3) 
			{
              ...
              } else 
			  {
                ...
                if ($user_details->Sub_seller_admin == 1) {
				  ...
				  $Sub_seller_Enrollement_id =  $seller_id;
                } else {
                  $remark_by = 'By Seller';
				  $Sub_seller_Enrollement_id = $user_details->Sub_seller_Enrollement_id;
                }
              ...
              foreach ($loyalty_prog as $prog) {
                ...
                $lp_details = $this->Transactions_model->get_loyalty_program_details($Company_id, $Sub_seller_Enrollement_id, $prog, $Todays_date);
              ...
              if ($Promo_redeem_by == 1) { //******** Promo Code Redeem *********/
                ...
                $PromoCode = strip_tags($this->input->post('Promo_code'));
                $PointsRedeem = strip_tags($this->input->post('promo_points_redeemed'));
			...
			if($Logged_user_id==7)//Finanace
			{
              ...
              if ($Gift_redeem_by == 1) { //******** Gift Card Redeem *********/
                ...
                $gift_reedem = strip_tags($this->input->post('gift_points_redeemed'));

                $GiftCardNo = strip_tags($this->input->post('GiftCardNo'));
                $GiftBal = strip_tags($this->input->post('Balance'));
			...
			if($Logged_user_id==7)//Finanace
			{
                ...
                $result32 = $this->Transactions_model->update_giftcard_balance($GiftCardNo, $current_gift_balance, $Company_id,$data['enroll']);

                $TotalRedeemPoints = $TotalRedeemPoints + $gift_reedem;

                $bill = $bill + 1;
              }
			  if(strip_tags($_REQUEST['Redeem_voucher_flag'])==1)
			  {
				  $VoucherNumber=strip_tags($_REQUEST['VoucherNumber']);
				  if($VoucherNumber!='')
				  {
					 $result32x = $this->Transactions_model->update_giftcard_balance(strip_tags($_REQUEST['VoucherNumber']), 0, $Company_id,$data['enroll']); 
				  }
			  }
			  else
			  {
              ...
              $bill_no = $bill + 1;
              if (strip_tags($this->input->post('Promo_code')) != "") {
                $Promo_code = strip_tags($this->input->post('Promo_code')).'-'.strip_tags($this->input->post('promo_points_redeemed'));
              } else {
                $Promo_code = "";
              }
              if (strip_tags($this->input->post('GiftCardNo')) != "") {
                $GiftCardNo = strip_tags($this->input->post("GiftCardNo"));
              } else {
                $GiftCardNo = 0;
              }
              if (strip_tags($this->input->post('points_redeemed')) != "") {
                $Redeem_points = strip_tags($this->input->post("points_redeemed"));
              } else {
                $Redeem_points = 0;
              }
			  $Seller = $data['enroll'];
			if($Logged_user_id==7)//Finanace
			{
              ...
              $post_data = array(
                  'Trans_type' => '2',
                  'Company_id' => $Company_id,
                  'Purchase_amount' => strip_tags($this->input->post("purchase_amt")),
                  'Trans_date' => $lv_date_time,
                  'Card_id' => $cardId,
                  'Seller_name' => $Seller_name,
                  // 'Seller' => $seller_id,
                  ...
                  'Payment_type_id' => strip_tags($this->input->post("Payment_type")),
                  'balance_to_pay' => $bal_pay,
                  'Bank_name' => strip_tags($this->input->post("Bank_name")),
                  'Branch_name' => strip_tags($this->input->post("Branch_name")),
                  'Credit_Cheque_number' => strip_tags($this->input->post("Credit_Cheque_number")),
                  'Source' => 0,
                  'GiftCardNo' => $GiftCardNo,
                  'remark3' => $Promo_code,
                  'Voucher_no' => $VoucherNumber,
                  'Voucher_discount' => $VoucherRedeem,
                  'Giftcard_amt' => $gift_reedem,
              ...
              $toname = "";
              $opval = strip_tags($this->input->post("purchase_amt")). ' Amount.';  // Transaction amt
              $firstName = $First_name;
              $lastName = $Last_name;
              ...
              $transaction_purchase_amt = strip_tags($this->input->post("purchase_amt"));

              $purchase_amount = $purchase_amt + $transaction_purchase_amt;

              $transaction_redeem_amt = strip_tags($this->input->post("redeem_amt"));
              ...
              if ($Refree_enroll_id > 0) {
                ...
                $Ref_rule = $this->Transactions_model->select_seller_refrencerule($Seller, $Company_id, $Referral_rule_for);
                $total_ref_topup = 0;
                ...
                if ($total_ref_topup > 0) {
                  ...
                  $result7 = $this->Transactions_model->update_topup_billno($Seller, $billno_withyear_ref);
			...
			if($Logged_user_id==7)//Finanace
			{
              ...
              $result4 = $this->Transactions_model->update_billno($Seller, $billno_withyear);

              $Notification_type = "Loyalty Transaction";
              $purchase_amt = strip_tags($this->input->post("purchase_amt"));
              $reedem = strip_tags($this->input->post("points_redeemed")); //$this->input->post("points_redeemed");
              $payment_by = strip_tags($this->input->post("Payment_type"));
              $balance_to_pay = $bal_pay;
              ...
              $Email_content = array(
                  ...
                  'PromoRedeem_amount' => $PromoRedeem_amount,
			...
			$DiscountResultVal = $this->Transactions_model->get_voucher_discount_value($Company_id,$Sub_seller_Enrollement_id,$lv_member_Tier_id,$purchase_amt);
    ...
    public function Fetch_member_details() { //Fetch_Member_info
      if ($this->session->userdata('logged_in')) {
        ...
        $_SESSION["cardId"] = strip_tags($_REQUEST['Membership_id']);
          ...
          if ($this->input->post("purchase_amt") == "" || $this->input->post("purchase_amt") <= 0 || $this->input->post("purchase_amt") == " ") {
            ...
            if (!(is_null($bal_pay))) {
              ...
              foreach ($loyalty_prog as $prog) {
                ...
                if ($lp_type == 'PA') {
                  $transaction_amt = strip_tags($this->input->post("purchase_amt"));
                }
			...
			if($Logged_user_id==7)//Finanace
			{
				$Seller = $Sub_seller_Enrollement_id;
			}
              $result4 = $this->Transactions_model->update_billno($Seller, $billno_withyear);
            ...
            if ($Coalition == 0) {
              ...
              if (!(is_null($Cancelle_amt))) {
               ...
               /*  $CustomerData = array(
				...
				$CustomerData = array('Debit_points' => $Update_debit);
    ...
    public function redeem_done() 
	{
		if ($this->session->userdata('logged_in')) {
			...
			$checkRedeem_points = is_numeric(strip_tags($this->input->post("Redeem_points")));
			if($checkRedeem_points !=1)
			{
				...
				$member_details = $this->Transactions_model->issue_bonus_member_details($Company_id, strip_tags($this->input->post("cardId")), $phnumber);
				...
				$Redeem_type = strip_tags($this->input->post("Redeem_type"));
				...
				if ($Redeem_type == 1) { //Normal
				  ...
				  $Redeem_points = strip_tags($this->input->post("Redeem_points"));
				 ...
				 /* if ($Redeem_points > $Current_point_balance) {
				  ...
				  $post_data = array(
					  ...
					  'Company_id' => $Company_id,
					  'Redeem_points' => strip_tags($this->input->post("Redeem_points")),
					  'Redeem_amount' => strip_tags($this->input->post("Redeem_points")),
					  'Manual_billno' => strip_tags($this->input->post("manual_bill_no")),
					  'Trans_date' => $lv_date_time,
					  ...
					  'Create_user_id' => $data['enroll'],
					  'Bill_no' => $tp_bill,
					  'Available_balance' => $card_bal-$this->input->post("Redeem_points")
				  );
				  ...
				  $where = "Just Redeem";
				  $toname = "";
				  $opval = strip_tags($this->input->post("Redeem_points")) . ' Points.';  // Redeem_points
				  $firstName = $First_name;
				  $lastName = $Last_name;
				...
				if ($Redeem_type == 1) {//Normal
				  $Email_content = array(
					  ...
					  'Redeem_points' => strip_tags($this->input->post("Redeem_points")),
					  'Remarks' => strip_tags($this->input->post('Remarks')),
					  'Notification_type' => 'Adjustment/Debit Transaction',
...
/**************************wallet debit*****************************/
	 public function wallet_debit() 
	 {
      if ($this->session->userdata('logged_in')) 
	  {
        ...
        /* -----------------------Pagination--------------------- */
        $config = array();
        $config["base_url"] = base_url() . "/index.php/Transactionc/wallet_debit";
        $total_row = $this->Transactions_model->wallet_debit_trans_count($Logged_user_id, $data['enroll'], $data['Company_id']);
        $config["total_rows"] = $total_row;
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config['next_link'] = 'Next';
        ...
        if ($_REQUEST == NULL) 
		{
          $data["results"] = $this->Transactions_model->wallet_debit_trans_list($config["per_page"], $page, $Logged_user_id, $data['enroll'], $data['Company_id']);
          $data["pagination"] = $this->pagination->create_links();
          $this->load->view('transactions/wallet_debit_view', $data);
        } 
		else 
		{
          /***********Changed AMIT 17-*06-2016************************ */
            ...
            if ($cardis != '0') {
              if ($cardId == $cardis || $Phone_no == $phnumber) {
                ...
                foreach ($cust_details as $row25) {
                  ...
                  $address = App_string_decrypt($row25['Current_address']);
                  $bal = $row25['Current_balance'];
                  $Wallet_bal = $row25['Wallet_balance'];
                  $Blocked_points = $row25['Blocked_points'];
                  $Debit_points = $row25['Debit_points'];
                  $phno = App_string_decrypt($row25['Phone_no']);
                  $companyid = $row25['Company_id'];
                ...
                $this->load->view('transactions/wallet_debit_transaction', $data);
              } 
			  else 
			  {
                $this->session->set_flashdata("error_code", "Sorry, Cannot proceed with the Transaction!! Your Membership ID is not registered with us...! !");
    ...
    public function wallet_debit_done() 
	{
		if ($this->session->userdata('logged_in')) {
			...
			$logtimezone = $session_data['timezone_entry'];
			  ...
			  if ($_POST == NULL) 
			  {
				$this->session->set_flashdata("error_code", "Sorry, Wallet Debit Transaction Failed. Invalid Data Provided!!");
				redirect('transactions/wallet_debit');
			  } 
			  else 
			  {
				$seller_details = $this->Igain_model->get_enrollment_details($data['enroll']);
				...
				{
				  $this->session->set_flashdata("error_code", "Inavalid Membership ID or Phone Number !!!");
				  redirect('Transactionc/wallet_debit');
				}
				...
				$currency_details = $this->Igain_model->Get_Country_master($Cust_country);
				$Symbol_currency = $currency_details->Symbol_of_currency;
				...
				$Debit_amount = $this->input->post("Debit_amount");
				 ...
				 if ($Debit_amount > $Wallet_bal) 
				 {
					$this->session->set_flashdata("error_code", "Insufficient Wallet Balance !!!");
					redirect('Transactionc/wallet_debit');
				 }
				
				$Available_wallet_bal = $Wallet_bal - $Debit_amount;
				 
				  $post_data = array(
					  'Trans_type' => '31',
					  'Company_id' => $Company_id,
					  // 'Redeem_amount' => $this->input->post("Debit_amount"),
					  'Wallet_amount' => $this->input->post("Debit_amount"),
					  'Trans_date' => $lv_date_time,
				  ...
				  $userid = $data['userId'];
				  $what = "Wallet Debit";
				  $where = "Wallet Debit Transaction";
				  $toname = "";
				  $opval = $this->input->post("Debit_amount") . ' Amount.';  
				  $firstName = $First_name;
				  $lastName = $Last_name;
				...
				$up_wallet = array('Wallet_balance' => $Available_wallet_bal);
												
				$this->Transactions_model->update_wallet($up_wallet,$Customer_enroll_id,$Company_id);
											
				$Email_content = array(
					  'Todays_date' => $Todays_date,
					  'Debit_amount' => number_format($Debit_amount,2),
					  'Symbol_currency' => $Symbol_currency,
					  'Notification_type' => 'Wallet Debit Transaction at '.$data["Company_details"]->Company_name,
					  'Remarks' => $this->input->post('Remarks'),
					  'Template_type' => 'Wallet_debit'
				  );
				
				$this->send_notification->send_Notification_email($Customer_enroll_id, $Email_content, $seller_id, $Company_id);
				
				
				$this->session->set_flashdata("success_code", " Wallet Debit transaction done successfully !!!");
...
/**************************wallet debit*****************************/
	...
	//---------------------------AMIT KAMBLE 26-11-2021 Include Redeem Voucher
	  public function Get_voucher_info() {
      $result = $this->Transactions_model->Get_voucher_info($this->input->post("VoucherNumber"), $this->input->post("MemberCardID"), $this->input->post("Company_id"));
	...
	{ 
		...
		$TotalDiscountAmt = 0;
		$DiscountResult12 = $this->Transactions_model->get_discount_value($Company_id,$Outlet_id,$Tier_id,$grandTotal);
		...
		$DiscountAmt = $DisOpt12["DiscountAmt"];
		
		if($grandTotal < $DisOpt12["DiscountAmt"])
		{
		...
		$Balance_due = $grandTotal - $DiscountAmt;
