...
class Data_transform_model extends CI_Model {
      ...
      /*   if ($limit != "" || $start != "") {
        ...
        if ($Super_seller == 0) {
			if ($Sub_seller_Enrollement_id > 0) {
				 $query = $this->db->where(array("dm.Column_Company_id" => $Company_id, "em.Company_id" => $Company_id, "dm.Column_outlet_id" => $Logged_user_enrollid, "Data_map_for" => 2));
			}
			else
			{
    ...
    public function check_exist_data_map_seller($Column_outlet_id, $Company_id) {

        $query = $this->db->where(array("Column_Company_id" => $Company_id, "Column_outlet_id" => $Column_outlet_id, "Data_map_for" => 2));
    ...
    public function upload_data_map_file($filepath, $filename, $Company_id, $Logged_user_enrollid) 
	{
        ...
        // echo $this->db->last_query();
        $this->db->from("igain_data_upload_map_tbl");
        $this->db->where(array("Column_Company_id" => $Company_id, "Column_outlet_id" => $Logged_user_enrollid, "Data_map_for" => 2));
        ...
        if ($extension == "csv") {
            ...
            for ($lines = 1; $csv_data = fgetcsv($handle, 1000, ",", '"'); $lines++) {
               ...
               // echo " TrDate2-----". $TrDate2."---<br>";
                ...
                if ($MembershipID == "" || $MembershipID == null) {
					...
					$Phone_no = App_string_encrypt($phonecode."".$csv_data[$col_cust]);
                    $Qresult = $this->db->query("Select Enrollement_id,Card_id from igain_enrollment_master where Company_id='" . $Company_id . "'  AND User_id='1' AND (Card_id='" . $MembershipID . "' OR Phone_no='" . $Phone_no . "')");
                ...
                if ($access == 1) {
                    ...
                    foreach ($errors_collection as $err) {
                       ...
                       // $transDate = $values_collection[$m]['Transaction_date']; echo " - transDate --".$trans_date."<br>";
                        ...
                        $Quantity = strip_tags($values_collection[$m]["Quantity"]); // echo " Quantity--".$Quantity."<br>";
                    ...
                    $Quantity = strip_tags($values_collection[$m]["Quantity"]);  //echo " Quantity--".$Quantity."<br>";
                    $PaymentType = strip_tags($values_collection[$m]["PaymentType"]);  //echo " PaymentType--".$PaymentType."<br>";
                    $Item_Code = strip_tags($values_collection[$m]["Item_Code"]);  //echo " Item_Code--".$Item_Code."<br>";
            ...
            for ($x = 2; $x <= count($data->sheets[0]["cells"]); $x++) {
                ...
                $PaymentType = strip_tags($data->sheets[0]["cells"][$x][$PaymentType11]);

                $product_code12 = strip_tags($data->sheets[0]["cells"][$x][$product_code11]);
                $trans_amount12 = strip_tags($data->sheets[0]["cells"][$x][$trans_amount11]);

                $quantity12 = strip_tags($data->sheets[0]["cells"][$x][$quantity11]);
                ...
                if ($MembershipId12 == "") {
                    ...
                    $Error_transaction_date = $trans_date;
                     ...
                     // $Qresult1 = $this->db->query("Select Enrollement_id from igain_enrollment_master where Company_id='" . $Company_id . "' AND Card_id='" . $MembershipId12 . "' AND User_id='1' ");
					  $Phone_no = App_string_encrypt($phonecode."".$MembershipId12);
                    $Qresult = $this->db->query("Select Enrollement_id,Card_id from igain_enrollment_master where Company_id='" . $Company_id . "'  AND User_id='1' AND (Card_id='" . $MembershipId12 . "' OR Phone_no='" . $Phone_no . "')");
					foreach ($Qresult->result() as $row)
					{
						$Enrollement_id = $row->Enrollement_id;
						$MembershipId12 = $row->Card_id;
