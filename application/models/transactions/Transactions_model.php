...
//***********************************************Akshay Starts************************************
	...
	public function wallet_credit_trans_count($Logged_user_id,$Seller_id,$Company_id)
	{
		if($Logged_user_id == 7)
		{
			$this->db->select('');
			$this->db->from('igain_enrollment_master as A');
			$this->db->join('igain_transaction as B', 'A.Card_id = B.Card_id');
			$this->db->where(array('B.Trans_type' => '30', 'A.Company_id' => $Company_id, 'B.Seller' => $Seller_id));	
		}
		else if($Logged_user_id == 2)
		{
			$this->db->select('');
			$this->db->from('igain_enrollment_master as A');
			$this->db->join('igain_transaction as B', 'A.Card_id = B.Card_id');
			$this->db->where(array('B.Trans_type' => '30', 'A.Company_id' => $Company_id));	
		}

		return $this->db->count_all_results();
	}
	public function wallet_debit_trans_count($Logged_user_id,$Seller_id,$Company_id)
	{
		if($Logged_user_id == 7)
		{
			$this->db->select('');
			$this->db->from('igain_enrollment_master as A');
			$this->db->join('igain_transaction as B', 'A.Card_id = B.Card_id');
			$this->db->where(array('B.Trans_type' => '31', 'A.Company_id' => $Company_id, 'B.Seller' => $Seller_id));	
		}
		else if($Logged_user_id == 2)
		{
			$this->db->select('');
			$this->db->from('igain_enrollment_master as A');
			$this->db->join('igain_transaction as B', 'A.Card_id = B.Card_id');
			$this->db->where(array('B.Trans_type' => '31', 'A.Company_id' => $Company_id));	
		}
   ...
   public function wallet_credit_trans_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	{
		//$this->db->limit($limit,$start);
   ...
   public function wallet_debit_trans_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	{
		//$this->db->limit($limit,$start);
	...
	public function check_redeem_bill_no($Bill_no,$Company_id)
	{		
		$query =  $this->db->select('Manual_billno')
				  ->from('igain_transaction')
				  ->where(array('Trans_type' => 3,'Manual_billno' => $Bill_no, 'Company_id' => $Company_id))->get();
		return $query->num_rows();
	}	
	public function check_waller_credit_bill_no($Bill_no,$Company_id)
	{		
		$query =  $this->db->select('Manual_billno')
				  ->from('igain_transaction')
				  ->where(array('Trans_type' => 30,'Manual_billno' => $Bill_no, 'Company_id' => $Company_id))->get();
		return $query->num_rows();
	}
	public function check_waller_debit_bill_no($Bill_no,$Company_id)
	{		
		$query =  $this->db->select('Manual_billno')
				  ->from('igain_transaction')
				  ->where(array('Trans_type' => 31,'Manual_billno' => $Bill_no, 'Company_id' => $Company_id))->get();
		return $query->num_rows();
	}
	...
	function get_bills_info($Bill_no,$Trans_type,$Seller_id)
	{
		...
		if($Trans_type == 2)
		{
			$this->db->select('B.*,A.First_name,A.Middle_name,A.Last_name,A.Phone_no,A.User_email_id,A.Current_address,A.City,C.Trans_type,E.Payment_description');
			$this->db->from('igain_enrollment_master as A');
			$this->db->join('igain_transaction as B', 'A.Enrollement_id = B.Enrollement_id');
...
// echo $this->db->last_query();
	...
	public function update_wallet($update,$Enrollement_id,$Company_id)
	{ 
		$this->db->where('Enrollement_id', $Enrollement_id, 'Company_id', $Company_id); 
		$this->db->update('igain_enrollment_master', $update); 
		return true;
	}
	public function update_customer_wallet_balance($Company_id,$Enrollment_id,$Wallet_balance,$Wallet_topup_amt)
	{
		$CustomerData = array(
					'Wallet_balance' => $Wallet_balance,
					'Total_wallet_topup' => $Wallet_topup_amt
				);
	...
	public function update_customer_blockpoints($Membership_id,$Company_id,$Blocked_points)
	{
		$CustomerData = array(
   ...
   function enrollment_details_frmemail_phn_card($Email,$Card_id,$Phone_no,$Company_id)
	...
	public function update_giftcard_balance($Gift_card_id,$Card_balance,$Company_id,$Update_user_id)
	{
		$data = array(
...
//*****************************************Sandeep Work Starts *************************************************
	...
	{
		$Payment_Type_id = array('1','3','4','5');
		$this->db->select('*');
		$this->db->from('igain_giftcard_tbl');
		$this->db->where(array('Company_id' => $Company_id));
		$this->db->where_in('Payment_Type_id', $Payment_Type_id);
   ...
   public function insert_giftcard($Company_id,$days,$Logged_user_id)
   { 
	   $data["Gift_card_id"] = strip_tags($this->input->post("gif_number"));
	   $data["Card_balance"] = strip_tags($this->input->post("gif_balance"));
	   $data["Card_value"] = strip_tags($this->input->post("gif_balance"));
	   $data["Company_id"] = $Company_id;
	   $data["Seller_id"] = $Logged_user_id;
	   $data["Valid_till"] = date("Y-m-d",strtotime("+$days days"));

	   $Enroll_flag = strip_tags($this->input->post("Enroll_flag"));
	   $Enrollement_id = 0;
	   
		if($Enroll_flag == 1)
		{ 
			$data["Card_id"] = strip_tags($this->input->post("CardID"));
			$da_Card_id = strip_tags($this->input->post("CardID"));
		...
		else
		{
			...
			$data["Email"] = App_string_encrypt(strip_tags($this->input->post("user_email")));
			$data["Phone_no"] = App_string_encrypt(strip_tags($this->input->post("user_phno")));
		}
		$Cheque_no=strip_tags($this->input->post("Cheque_no"));
		if($Cheque_no!=NULL)
		{
			$Cheque_no=strip_tags($this->input->post("Cheque_no"));
		}
	   ...
	   $data["Payment_Type_id"] = strip_tags($this->input->post("Payment_type"));
	   ...
	   if($data["Card_id"] > 0)
	   {
		   ...
		   $data77['Trans_type'] = 4;
		   $data77['Purchase_amount'] = strip_tags($this->input->post("gif_balance"));
		   $data77['Mpesa_Paid_Amount'] = strip_tags($this->input->post("gif_balance"));
		   $data77['Mpesa_TransID'] = strip_tags($this->input->post("Mpesa_code"));
		   $data77['Payment_type_id'] = strip_tags($this->input->post("Payment_type"));
		   $data77['Trans_date'] = date("Y-m-d");
		   $data77['Enrollement_id'] = $Enrollement_id;
		   $data77['Card_id'] = $data["Card_id"];
		   $data77['GiftCardNo'] = strip_tags($this->input->post("gif_number"));
		   $data77['remark2'] = $Cheque_no;
		   $data77['Quantity'] = 1;
		   
		   $this->db->insert("igain_transaction",$data77);
   ...
   public function loyalty_transaction_count($Logged_user_id,$Seller_id,$Company_id)
	...
	public function loyalty_transactions_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	{
		...
		$this->db->where(array('B.Trans_type' => '2', 'A.Company_id' => $Company_id, 'B.Create_user_id' => $Seller_id));
...
//*****************************************AMIT Work START *************************************************
	...
	public function redeem_transactions_list($limit,$start,$Logged_user_id,$Seller_id,$Company_id)
	{
		...
		$this->db->select('*,Redeem_points as Total_Redeem_Points');
		/****************************************/
		$this->db->from('igain_enrollment_master as A');
		$this->db->join('igain_transaction as B', 'A.Card_id = B.Card_id');
...
/********************AMIT KAMBLE 26-11-2021******Gift Card Transaction************/
	function Get_voucher_info($VoucherNumber,$MemberCardID,$Company_id)
	{
		$Todays_date = date('Y-m-d');
		$this->db->select('*');
		$this->db->from('igain_giftcard_tbl as g');
		$this->db->where(array('Gift_card_id' => $VoucherNumber, 'Card_id' => $MemberCardID, 'Company_id' => $Company_id, 'Valid_till >= ' => $Todays_date));
		$this->db->where_in('Payment_Type_id', array('998','99'));
		//998-REVENUE VOUCHER
		//99-DISCOUNT VOUCHER
		$query = $this->db->get();
		 // echo $this->db->last_query();die;
		...
		{			
			foreach ($query->result() as $row)
			{
               $result[] = array("User_name" => $row->User_name, "Email" => App_string_decrypt($row->Email), "Phone_no" => App_string_decrypt($row->Phone_no), "Card_balance" => $row->Card_balance, "Gift_card_id" => $row->Gift_card_id, "MembershipID" => $row->Card_id, "Discount_percentage" => $row->Discount_percentage, "Card_value" => $row->Card_value, "Update_user_id" => $row->Update_user_id);
            }			
			return json_encode($result);
		}
		else
	...
	function getGiftCard($Company_id)
	{
		...
		$gift_card_exist = $this->check_gift_card_id1($gift_cardid,$Company_id);
	...
	function check_gift_card_id1($gift_cardid,$Company_id)
	{
		$query =  $this->db->select('id')
				   ->from('igain_giftcard_tbl')
				   ->where(array('Gift_card_id' => $gift_cardid, 'Company_id' => $Company_id))->get();
	...
	function get_discount_value($Company_id,$Outlet_id,$Tier_id,$grandTotal)
	{
		$flag_call = 2;
		$channel = 2;
		$today = date("Y-m-d");
		...
		$this->db->where(array('A.Company_id' => $Company_id,'A.Seller_id' => $Outlet_id,'A.Active_flag' => 1,'A.Discount_rule_for' => 1,'A.Discount_voucher_applicable' => 0));
		...
		if ($sql->num_rows() > 0)
		{
			foreach ($sql->result() as $row)
			{
				...
				if(in_array($row->Seller_id,array(0,$Outlet_id)))
				{
					$step++;
				}
				...
				if($row->Discount_rule_for == 1 && $step == 2 && $grandTotal > 0) // on billing amt
				{
					...
					if( $opretorFlag > 0)
					{
						if($row->Discount_rule_set == 2)
						{
							...
							$disacount_value = $row->Discount_Percentage_Value;;
						}	
						...
						$disacount_offer = $disacount_value.' Discount on Purchase Amount';
					}
					$step++;
				}
				if($row->Discount_voucher_applicable == 0 )
				{
					...
					if($discount_Amt > 0)
					{
						$discountsArray[] = array("Discount_code"=>$row->Discount_code,"Discount_amt"=>number_format(floor($discount_Amt),2),"disacount_offer_details"=>$disacount_offer);
							
						$discountsArray2[] = array("Discount_code"=>$row->Discount_code,"Discount_amt"=>number_format(floor($discount_Amt),2),"disacount_offer_details"=>$disacount_offer);
					}
				}
	...
	function get_voucher_discount_value($Company_id,$Outlet_id,$Tier_id,$grandTotal)
	{
		$flag_call = 2;
		$channel = 2;
		$today = date("Y-m-d");
		...
		$this->db->from('igain_new_discount_rule_master as A');
		// $this->db->join('igain_new_discount_rule_child as B','A.Discount_code = B.Discount_code','LEFT');
		$this->db->where(array('A.Company_id' => $Company_id,'A.Seller_id' => $Outlet_id,'A.Active_flag' => 1,'A.Discount_rule_for' => 1,'A.Discount_voucher_applicable' => 1));
		$this->db->where('"'.$today.'" BETWEEN Valid_from AND Valid_till');
