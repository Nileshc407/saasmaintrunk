...
class Coal_Report_model extends CI_model
{
	...
	function get_merchant_report($start_date,$end_date,$Company_id,$seller_id,$transaction_type_id,$report_type,$login_enroll)
    {
		...
		if($report_type == 1)
		{
			...
			$fields = array(
						...
						'Voucher_redeemed' => array('type' => 'INT','constraint' => '11','DEFAULT' => 0),
						'Voucher_amount' => array('type' => 'DECIMAL','constraint' => '16,2','DEFAULT' => 0),
					);
			...
			$this->dbforge->create_table($temp_table);		
				...
				$this->db->where_in('A.Seller',$seller_id);
				$this->db->where('A.Trans_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
			...
			//SELECT Seller,sum(`Voucher_discount`) FROM `igain_transaction` WHERE `Voucher_discount`>0 and `Company_id`=8 group by Seller
			
			if ($query->num_rows() > 0)
			{
				foreach ($query->result() as $row)
				{
					if($row->Trans_type =='3rd Party Online Order')
					{
						$Channel_name = $this->Coal_Report_model->Get_beneficiary_company($row->Channel_id);
						$row->Trans_type = $Channel_name->Beneficiary_company_name;
					}
					...
					$this->db->select("Seller, Bill_no, Voucher_no, Voucher_discount");
					$this->db->from("igain_transaction");
					$this->db->where(array('Company_id' => $Company_id,'Voucher_discount!=' => '0.00'));
					$this->db->where('Trans_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
					
					if($row->Transaction_type > 0)
					{
						$this->db->where(array('Trans_type' => $row->Transaction_type));
					}
			
					$this->db->where(array('Seller' => $row->Seller));
					
					$this->db->group_by("Bill_no, Seller");
					//$this->db->order_by("Seller");
					$query888 = $this->db->get();
				//	echo $this->db->last_query();
					$data['Voucher_redeemed'] = 0;
					$totalVoucherAmt = 0;
					
					if ($query888->num_rows() > 0)
					{
						$data['Voucher_redeemed'] = $query888->num_rows();
						
						foreach ($query888->result() as $row888)
						{
							$totalVoucherAmt = $totalVoucherAmt + $row888->Voucher_discount;
						}
					}
					
				
					$data['Voucher_amount'] = $totalVoucherAmt;
					...
					$data['loyalty_pts_gain'] = floor($row->Loyalty_pts);
		...
		if($report_type == 0)
		{
			...
			$fields = array(
						...
						'loyalty_pts_gain' => array('type' => 'DECIMAL','constraint' => '25,0','DEFAULT' => 0),
						'Voucher_no' => array('type' => 'VARCHAR','constraint' => '25','null' => TRUE),
						'Voucher_amount' => array('type' => 'DECIMAL','constraint' => '16,2','DEFAULT' => 0),
						'coalition_Loyalty_pts' => array('type' => 'DECIMAL','constraint' => '25,0','DEFAULT' => 0),
						'seller_enrollid' => array('type' => 'INT','constraint' => '11','DEFAULT' => 0),
			...
			/* if($seller_id == 0 && $transaction_type_id == 0)
			{
				$this->db->select('A.Remarks,A.Trans_id, A.Manual_billno, A.Enrollement_id, A.Seller_name, A.Topup_amount, A.GiftCardNo, A.Purchase_amount, A.Redeem_points, A.balance_to_pay, A.Card_id, A.Bill_no, A.Loyalty_pts,A.Voucher_no,A.Voucher_discount,A.Coalition_Loyalty_pts, A.Seller, A.Trans_date, B.Trans_type, B.Trans_type_id, C.Company_name, D.First_name, D.Middle_name, D.Last_name,Quantity,A.Channel_id');//, E.Payment_type
				$this->db->from('igain_transaction as A');
				$this->db->join('igain_transaction_type as B', 'B.Trans_type_id = A.Trans_type');
				...
				$this->db->where_in('A.Trans_type', array('1','2','3','10','12','29'));				
				...
				$this->db->select('AA.Trans_id, AA.Manual_billno, AA.Enrollement_id, AA.Seller_name, AA.Topup_amount, AA.GiftCardNo, AA.Purchase_amount, AA.Redeem_points, AA.balance_to_pay, AA.Card_id, AA.Bill_no, AA.Loyalty_pts,AA.Voucher_no,AA.Voucher_discount,AA.Coalition_Loyalty_pts, AA.Seller, AA.Trans_date, BB.Trans_type, BB.Trans_type_id, CC.Company_name,Quantity,AA.Channel_id');
			...
			if($transaction_type_id == 0)
			{
				$this->db->select('A.Remarks,A.Trans_id, A.Manual_billno, A.Enrollement_id, A.Seller_name, A.Topup_amount, A.GiftCardNo, A.Purchase_amount, A.Redeem_points, A.balance_to_pay, A.Card_id, A.Bill_no, A.Loyalty_pts,A.Voucher_no,A.Voucher_discount,A.Coalition_Loyalty_pts, A.Seller, A.Trans_date, B.Trans_type, B.Trans_type_id, C.Company_name, D.First_name, D.Middle_name, D.Last_name,Quantity,A.Channel_id');
				$this->db->from('igain_transaction as A');
				$this->db->join('igain_transaction_type as B', 'B.Trans_type_id = A.Trans_type');
				$this->db->join('igain_enrollment_master as D', 'D.Enrollement_id = A.Enrollement_id');
				$this->db->join('igain_company_master as C', 'C.Company_id = D.Company_id');
				...
				$this->db->select('AA.Remarks,AA.Trans_id, AA.Manual_billno, AA.Enrollement_id, AA.Seller_name, AA.Topup_amount, AA.GiftCardNo, AA.Purchase_amount, AA.Redeem_points, AA.balance_to_pay, AA.Card_id, AA.Bill_no, AA.Loyalty_pts,AA.Voucher_no,AA.Voucher_discount,AA.Coalition_Loyalty_pts, AA.Seller, AA.Trans_date, BB.Trans_type, BB.Trans_type_id, CC.Company_name,Quantity,AA.Channel_id');
				$this->db->from('igain_transaction as AA');
				$this->db->join('igain_transaction_type as BB', 'BB.Trans_type_id = AA.Trans_type');
				$this->db->join('igain_company_master as CC', 'CC.Company_id = AA.Company_id');
				$this->db->where(array('AA.Company_id' => $Company_id, 'AA.Trans_type' => '4'));
				$this->db->where('AA.Trans_date BETWEEN "'.$start_date.'" AND  "'.$end_date.'" ');
				$this->db->where_in('AA.Seller',$seller_id);
				$query2 = $this->db->get()->result();
			}			
			if($transaction_type_id != 0)
			{
				...
				{
					$this->db->select('AA.Remarks,AA.Trans_id, AA.Manual_billno, AA.Enrollement_id, AA.Seller_name, AA.Topup_amount, AA.GiftCardNo, AA.Purchase_amount, AA.Redeem_points, AA.balance_to_pay, AA.Card_id, AA.Bill_no, AA.Loyalty_pts,AA.Voucher_no,AA.Voucher_discount, AA.Coalition_Loyalty_pts,AA.Seller, AA.Trans_date, BB.Trans_type, BB.Trans_type_id, CC.Company_name,Quantity,A.Channel_id');
					$this->db->from('igain_transaction as AA');
					$this->db->join('igain_transaction_type as BB', 'BB.Trans_type_id = AA.Trans_type');
					$this->db->join('igain_company_master as CC', 'CC.Company_id = AA.Company_id');
					$this->db->where(array('AA.Company_id' => $Company_id, 'AA.Trans_type' => '4'));
			...
			if ($query3 != NULL)
			{
				...
				foreach ($query3 as $row)
				{
					...
					$data['Voucher_amount'] = $row->Voucher_discount;
					$data['coalition_Loyalty_pts'] = $row->Coalition_Loyalty_pts;
					$data['seller_enrollid'] = $row->Seller;
	...
	public function get_seller_report_details($temp_table,$Report_type,$start,$limit)	//,$limit,$start
	{
		if($Report_type == 1)
		{
			...
			$this->db->select('sellerName as Merchant_name,Trans_type,purchase_amt,top_up as Bonus_points,reedem_pt,Voucher_redeemed,Voucher_amount,seller_enrollid as Merchant_id');
		}
		else
		{
			...
			$this->db->select("Trans_date,sellerName as Merchant_name,Bill_no as EHP_Order_no,Manual_billno as POS_bill_no,cardId as Membership_ID,CONCAT(First_name, ' ', Last_name) as Member_name,purchase_amt,top_up as Bonus_points,reedem_pt,balance_to_pay,Trans_type, loyalty_pts_gain,Voucher_no,Voucher_amount,coalition_Loyalty_pts,seller_enrollid as Merchant_id,Trans_id,Trans_type_id,Walkin_customer,Quantity,Remarks");
		}
		/* if($limit != NULL || $start != NULL)
		{
			$this->db->limit($limit,$start);
	...
	public function get_seller_excel_report_details($temp_table,$Report_type)	//,$limit,$start
	{
		if($Report_type == 1)
		{
			...
			$this->db->select('sellerName as Outlet,Trans_type,purchase_amt as Purchase_amt,top_up as Bonus_points,reedem_pt as Reedem_pts,Voucher_redeemed,Voucher_amount');
		...
		else
		{
			...
			$this->db->select("Trans_date,sellerName as Outlet,Bill_no as EHP_Order_no,Manual_billno as POS_bill_no,cardId as Membership_ID,CONCAT(First_name, ' ', Last_name) as Member_name,purchase_amt as Purchase_amt,top_up as Bonus_points,reedem_pt as Reedem_pts,Voucher_no,Voucher_amount,balance_to_pay,Trans_type, loyalty_pts_gain,Walkin_customer,Quantity");
	...
	/*
		...
		*/
		...
		$this->db->select('First_name,Middle_name,Last_name,IT.Enrollement_id,IT.Trans_id,IT.Trans_date,Purchase_amount,Redeem_points,Paid_amount,Topup_amount,Loyalty_pts,Manual_billno as Bill_no,TT.Trans_type_id,Seller,IT.Card_id as Membership_ID,IT.Company_id,Card_id2 as Transfer_to,Transfer_points,Expired_points,TM.Tier_name,IT.Remarks as Remarks,TT.Trans_type ');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
		...
		$this->db->where_not_in('IT.Trans_type', array('9','10','12','17','20','22','21','24','25','27','28'));
		if($transaction_type_id!=0)//Single Transaction Type
		{
			$this->db->where('IT.Trans_type' , $transaction_type_id);
		}
	...
	/* 	if($limit != NULL || $start != NULL)
		...
		if($sql51 -> num_rows() > 0)
		{
			...
			foreach ($sql51->result() as $row)
			{
				 if($row->Bill_no == '' || $row->Bill_no == NULL)
				{
					$row->Bill_no="-";
				}
				else
	...
	function get_cust_trans_summary_all($Company_id,$Enrollement_id,$start_date,$end_date,$transaction_type_id,$Tier_id,$start,$limit,$Enable_company_meal_flag)
	{
		...
		$this->db->select('IE.First_name,IE.Middle_name,IE.Last_name,IT.Card_id AS Membership_ID,TT.Trans_type,SUM(Purchase_amount) as Total_Purchase_Amount,SUM(Loyalty_pts) as Total_Gained_Points,SUM(Redeem_points) AS Total_Redeem,SUM(Paid_amount) AS Total_Paid_amount,SUM(Topup_amount) as Total_Credit_topup_Points,SUM(Transfer_points) AS Total_Transfer_Points,SUM(Expired_points) AS Total_Expired_points,Tier_name,IT.Enrollement_id');
		...
		if($sql51 -> num_rows() > 0)
		{
			...
			foreach ($sql51->result() as $row)
			{
				...
				if($row->Total_Credit_topup_Points==0)
				{
					$row->Total_Credit_topup_Points="-";
				}	
	...
	function get_cust_trans_details_reports($Company_id,$From_date,$To_date,$Enrollement_id,$transaction_type_id,$Tier_id,$Enable_company_meal_flag)
	{
		$From_date=date("Y-m-d 00:00:00",strtotime($From_date));
		$To_date=date("Y-m-d 23:59:59",strtotime($To_date));
		...
		$this->db->select('IT.Trans_date,First_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,TT.Trans_type,Manual_billno as  Bill_no,Purchase_amount,Loyalty_pts,Redeem_points,Paid_amount,Topup_amount,Card_id2 as Transfer_to,Transfer_points,Expired_points,Remarks,Seller_name as Transaction_by');
		$this->db->from('igain_transaction as IT');
		$this->db->join('igain_enrollment_master as IE','IT.Enrollement_id=IE.Enrollement_id');
		$this->db->join('igain_transaction_type as TT','IT.Trans_type=TT.Trans_type_id');
		$this->db->join('igain_tier_master as TM','IE.Tier_id=TM.Tier_id');
		...
		if($sql51 -> num_rows() > 0)
		{
			...
			foreach ($sql51->result() as $row)
			{
				...
				$this->db->select('IT.Trans_date,First_name,Middle_name,Last_name,IT.Card_id AS Membership_ID,Tier_name,TT.Trans_type,Bill_no,Purchase_amount,Loyalty_pts,Redeem_points,Paid_amount,Topup_amount,Card_id2 as Transfer_to,Transfer_points,Remarks');
