...
/*-------------------------------Nilesh Work Start 10-07-2017----------------------------------*/
	...
	public function Assign_query($Company_id,$cc_user,$Log_ticket,$querypriority,$Closure_date,$Sub_seller_admin)
	{
		$sts = 'Forward';
	...
	public function Forward_querys($Company_id,$Query_type_id,$cc_user,$Log_ticket,$querypriority,$Closure_date,$Sub_seller_admin)
	{
		if($limit!=NULL || $start !=NULL)
		{
			$this->db->limit($limit,$start);
		...
		// {	
			...
			$this->db->from('igain_callcenter_querylog_child'); 	
			$this->db->join('igain_callcenter_querylog_master', ' igain_callcenter_querylog_master.Querylog_ticket = igain_callcenter_querylog_child.Querylog_ticket');
			if($Sub_seller_admin == 0) {
				$this->db->where(array('igain_callcenter_querylog_child.Company_id' => $Company_id, 'igain_callcenter_querylog_child.Enrollment_id' => $cc_user, 'igain_callcenter_querylog_child.Query_assign' => $cc_user, 'igain_callcenter_querylog_child.Query_status' => $sts, ' igain_callcenter_querylog_master.Query_status' => $sts));
			}
			else{
				$this->db->where(array('igain_callcenter_querylog_child.Company_id' => $Company_id, 'igain_callcenter_querylog_child.Query_status' => $sts, ' igain_callcenter_querylog_master.Query_status' => $sts)); 
			...
			{
				$this->db->group_by('igain_callcenter_querylog_child.Querylog_ticket');
			}			
			$company_sql1= $this->db->get(); 
			if($company_sql1->num_rows() > 0)
			{
	...
	public function Closed_querys($Company_id,$Query_type_id,$Log_ticket,$querypriority,$Closure_date,$SuperSellerFlag,$login_userId,$Sub_seller_admin)
	{ 
		...
		// { 
			...
			if($querypriority != NULL)
			{
				$this->db->where(array('Resolution_priority_levels' => $querypriority));
			}
			if($Closure_date != NULL)
			{
				$this->db->where(array('Closure_date' => $Closure_date));
		...
		if($SuperSellerFlag == 1 || $Sub_seller_admin == 1)
		{
			$sts = 'Closed';
			
			$post_data=$this->db->select('*');
			$this->db->order_by('igain_callcenter_querylog_master.Update_date','DESC');
			$this->db->from('igain_callcenter_querylog_master');
			
			// $this->db->where(array('Company_id' => $Company_id, 'Query_status' => $sts, 'Update_User_id' => $login_userId));
			$this->db->where(array('Company_id' => $Company_id, 'Query_status' => $sts));
