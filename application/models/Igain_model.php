...
//**************** sandeep work start ***********************************	
	 ...
	 function get_loyalty_rule($Company_id,$Logged_user_enrollid,$Logged_user_id)
	{
		...
		{
			$this->db->select('distinct(Loyalty_name),Loyalty_id,Till_date,From_date,Flat_file_flag,Loyalty_at_transaction,Loyalty_at_value,discount');
...
/********************************************Akshay start *******************************/
	...
	function get_cust_enrollment_details($Enrollement_id)
	{
		$this->db->select('*');
		$this->db->from('igain_enrollment_master as e');
		$this->db->join('igain_tier_master as t','e.Company_id=t.Company_id and e.Tier_id=t.Tier_id');
		$this->db->where('e.Enrollement_id', $Enrollement_id);
...
//********************************************Amit Start*******************************
	...
	function validate_Card_id($Input_data,$Company_id)
	{
		...
		$phnumber = App_string_encrypt($dial_code.$Input_data);
	...
	function get_cust_trans_summary($Enrollement_id)
	{
		...
		$this->db->where('IT.Trans_type !=' , 26);
...
/*************amit get code decode master 13-09-2017*********end*******/
	...
	function Company_city_state_country($Company_id)
	{
		...
		$this->db->select("*,igain_country_master.name as Country_name,igain_country_master.id as Country_id");
		$this->db->from('igain_company_master');	
	...
	public function Segment_applicable_function($compid,$Segment_code,$Enrollement_id,$Sex,$Country_id,$District,$State,$City,$Zipcode,$total_purchase,$Total_reddems,$Date_of_birth,$joined_date,$Tier_id)
	{
		...
		foreach($Get_segments2 as $Get_segments)
		{
			...
			if($Get_segments->Segment_type_id==10)//Cumulative Points Accumulated
			{
				...
				$Trans_Records = $this->Igain_model->get_cust_trans_details_segment($compid,$Enrollement_id,$start_date,$end_date,$transaction_type_id,$Tier_id,'','');
				...
				// echo "<br>Enrollement_id $Enrollement_id-- lv_Cust_value ".$lv_Cust_value;die;
			}
	...
	function Get_birthday_communication($Company_id)
	{
		...
		$this->db->select('description,Link_to_voucher,Voucher_type,Voucher_id,communication_plan');
	...
	//*********** 06-12-2021 *********Email Template ******
	function get_cust_trans_details_segment($Company_id,$Enrollement_id,$start_date,$end_date,$transaction_type_id,$Tier_id,$start,$limit)
	{
	...
	public function Fetch_thirdparty_details()
	{
		$this->db->select('*');
		$this->db->from('igain_thirdparty_evouchar_configuration');
		$this->db->where(array('active_flag' =>1));
		$sql = $this->db->get();
		// echo $this->db->last_query();
	...
	public function update_thirdparty_details($id,$type,$updateData)
	{
		$this->db->where(array('type'=>$type,'id'=>$id));
		$this->db->update('igain_thirdparty_evouchar_configuration', $updateData);
			// echo $this->db->last_query(); 
		if ($this->db->affected_rows() > 0)
		{
			return true;
