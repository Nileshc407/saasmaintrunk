...
class POS_catalogue_model extends CI_Model
{
	...
	public function Get_company_Code_decode_master($Code_decode_type_id,$Company_id)
	{
		$this->db->select('*');
   ...
   public function Get_Specific_Code_decode_master($Code_decode_id)
	...
	public function Get_POS_Item_prices($Item_code,$Company_id)
	{
		...
		$this->db->order_by('Price_Active_flag','DESC');
   ...
   function Delete_pos_item_cond($Company_id,$Menu_Item_code,$Require_optional)
	...
	function Insert_pos_linked_outlets($Post_data)
	{
		$this->db->insert('igain_pos_item_linked_outlets',$Post_data);
		if($this->db->affected_rows()>0)
		{
			return true;
		}
	 ...
	 function Delete_pos_linked_outlets($Brand_id,$Merchandize_item_code)
	{
		$this->db->where(array('Brand_id'=>$Brand_id,'Merchandize_item_code'=>$Merchandize_item_code));
		$this->db->delete("igain_pos_item_linked_outlets");
		...
		function Get_pos_linked_Brand_outlets($Company_id,$Brand_id,$Merchandize_item_code)
	{
		$this->db->select("DISTINCT(Enrollement_id) as Enrollement_id,First_name,Middle_name,Last_name,Super_seller");
		$this->db->from('igain_pos_item_linked_outlets as o');
		$this->db->join('igain_enrollment_master as e','o.Outlet_id=e.Enrollement_id');
		$this->db->where(array('o.Company_id' => $Company_id,'Brand_id' => $Brand_id,'Merchandize_item_code' => $Merchandize_item_code));
		$sql = $this->db->get();
		// echo $this->db->last_query();die;
		if ($sql->num_rows() > 0)
		{
