...
/******************************Nilesh end partner merchandize ctelogue report******************************/	
	...
	public function Failed_login_Report($Company_id,$start_date,$end_date,$enter_user,$User_type, $Report_type,$Source, $start,$limit)
	{ 	
		...
		if($Report_type == 0){
			...
			$this->db->select('IE.First_name As FirstName,IE.Last_name As LastName,IF.User_email_id AS EmailAddress,IF.User_id AS UserType,COUNT(IF.Fail_flag) AS FailedAttempts');
	   
			$this->db->from('igain_failed_login as IF');
			$this->db->join('igain_enrollment_master as IE','IE.User_email_id=IF.User_email_id','LEFT');
			$this->db->where('IF.Company_id' , $Company_id);	
			$this->db->where("Login_date BETWEEN '".$start_date."' AND '".$end_date."'");
			
			if($User_type != "0")
			{
				$this->db->where('IF.User_id' ,$User_type);
			}
			if($Source != "0")
			{
				$this->db->where('IF.Source' ,$Source);
			}		
			if($enter_user != "")
			{
				// $this->db->like('IE.First_name',$enter_user);
				$this->db->like('IE.First_name', $enter_user);
				$this->db->or_like('IE.Last_name', $enter_user);
			} 
			$this->db->order_by('IF.Id','desc');
			// $this->db->group_by('DATE(IF.Login_date)');
			$this->db->group_by('IF.User_email_id');
			
			
		} else {
			
			$this->db->select('Login_date AS Date,IE.First_name As FirstName,IE.Last_name As LastName,IF.User_email_id AS EmailAddress,Password AS AttemptedPassword,IF.User_id AS UserType,IF.Source,Fail_flag AS FailedAttempts,Location AS Location');
		...
		if($sql51 -> num_rows() > 0)
		{
			foreach ($sql51->result() as $row)
			{
				...
				if($row->UserType == 4)
				{
				   $row->UserType = "Partner admin";
				}
				if($row->UserType == 5)
				{
				   $row->UserType = "Merchandize Partner User";
				}
				if($row->UserType == 6)
				{
				   $row->UserType = "Call Center User";
				}
				if($row->UserType == 7)
				{
				   $row->UserType = "Staff Users";
				}		
			   if($row->Source == 2)
				{ 
				  $row->Source = 'iOS APP';
				...
				if($row->Source == 1)
				{ 
				  $row->Source = 'Android APP';
				}
				if($row->Source == 3)
				{ 
				  $row->Source = 'Member Website';
				}
				if($row->Source == 4)
				{ 
				  $row->Source = 'Backend Application';
				}
			   
			   $row->EmailAddress = App_string_decrypt($row->EmailAddress);
			   if($Report_type != 0){
				   $row->AttemptedPassword = App_string_decrypt($row->AttemptedPassword);
			   }
