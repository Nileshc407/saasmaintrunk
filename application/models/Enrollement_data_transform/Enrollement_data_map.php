			...
			if($extension == "csv") 
			{
				...
				{	
					...
					if ($csv_data[0]) 
					{
							...
							$colphoneno = str_replace(' ', '', $colphoneno);
							...
							$coluseremailid = str_replace(' ', '', $coluseremailid);
							//echo "*****************CardID ".$CardID;
							if($coluseremailid == "")
							{
							...
							$Qresult_coluseremailid2 = $this->db->query("select upload_id from  ".$temp_tbl." where User_Email_ID ='".$coluseremailid."'  ");	
							if($Qresult_coluseremailid2->num_rows() > 0)
							{
								$get_error[] = "User Email ID Duplicate";
								$get_trans_date[] = date("Y-m-d",strtotime($TrDate));
								$get_error_row[] =  $lines ;
								$access = 1;								
							}
							$Qresult_coluseremailid3 = $this->db->query("select upload_id from  ".$temp_tbl." where Phone_No ='".$Phone_no."'  ");	
							if($Qresult_coluseremailid3->num_rows() > 0)
							{
								$get_error[] = "Phone No. Duplicate";
								$get_trans_date[] = date("Y-m-d",strtotime($TrDate));
								$get_error_row[] =  $lines ;
								$access = 1;								
							}
							$Qresult_coluseremailid4 = $this->db->query("select upload_id from  ".$temp_tbl." where Membership_ID ='".$CardID."'  ");	
							if($Qresult_coluseremailid4->num_rows() > 0)
							{
								$get_error[] = "MembershipID Duplicate";
								$get_trans_date[] = date("Y-m-d",strtotime($TrDate));
								$get_error_row[] =  $lines ;
								$access = 1;								
							}							
							if($access == 0 )
							{
								$sql_out2 = $this->db->query("select upload_id from  ".$temp_tbl." where User_Email_ID ='".$coluseremailid."'  ");
									...
									$tbl_data = array(
										...
										'First_Name' => strip_tags($colfirstname),
										'Last_Name' => strip_tags($collastname),
										'Address' => strip_tags($coladdress),
										'Phone_No' => strip_tags($Phone_no),
										'User_Email_ID' => $coluseremailid,
										'Membership_ID' => strip_tags($CardID),
