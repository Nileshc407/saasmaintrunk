...
// echo $this->db->last_query();
	...
	public function get_company_assigned_level_0_menu($Company_id)
	{
		...
		$this->db->where('Menu_id !=1');//dashboard
        $query = $this->db->get("igain_company_menu_master");
		// echo $this->db->last_query();
        if ($query->num_rows() > 0)
		{
   ...
   public function delete_company_user_menus($Company_id,$Menu_id)
	...
	//****************** sandeep 16-08-2019 ******************************
	//-----------------AMIT KAMBLE PRIVILEGES ---------------------------
	function delete_assigned_menu_privileges($post_data)
    {
		$this->db->delete('igain_menu_assign_privileges', $post_data);
		if($this->db->affected_rows() > 0)
		{
			return $this->db->affected_rows();
		}
    }
	function assign_menu_privileges($post_data)
    {
	...
	public function get_current_menu($Menu_href)
	{	   
		$this->db->where('Menu_href', $Menu_href);
		$query = $this->db->get("igain_menu_master");
	...
	public function Get_member_assign_privileges($Company_id,$Menu_id,$Enrollment_id)
	{	   
		$this->db->where('Company_id', $Company_id);
		$this->db->where('Menu_id', $Menu_id);
		$this->db->where('Enrollment_id', $Enrollment_id);
		$query = $this->db->get("igain_menu_assign_privileges");
		// echo $this->db->last_query();	 			
		if($query -> num_rows() > 0)
		{
			return $query->row();
	...
	public function Check_member_assign_menues($Company_id,$Menu_id,$Enrollment_id)
	{	   
		$this->db->where('Company_id', $Company_id);
		$this->db->where('Menu_id', $Menu_id);
		$this->db->where('Enrollment_id', $Enrollment_id);
	...
	public function get_user_assigned_level_0_menu($Company_id,$Enrollment_id)
	{
		$this->db->select("A.Menu_id,A.Menu_level,A.Parent_menu_id,A.Menu_name,A.Menu_href,A.Company_id");
		$this->db->from('igain_company_menu_master as A');
		$this->db->join('igain_menu_assign as B','A.Menu_id=B.Menu_id AND A.Company_id=B.Company_id');
		
		$this->db->where(array('A.Company_id' => $Company_id, 'Enrollment_id' => $Enrollment_id, 'A.Menu_level' => '0', 'Active_flag' => '1'));
		$this->db->order_by('A.Menu_id','ASC');
        $query = $this->db->get();
		// echo $this->db->last_query();
        if ($query->num_rows() > 0)
	...
	public function get_user_assigned_level_1_menu($Company_id,$Enrollment_id,$Menu_id)
	{
		$this->db->select("A.Menu_id,A.Menu_level,A.Parent_menu_id,A.Menu_name,A.Menu_href,A.Company_id");
		$this->db->from('igain_company_menu_master as A');
		$this->db->join('igain_menu_assign as B','A.Menu_id=B.Menu_id AND A.Company_id=B.Company_id');
		
		$this->db->where(array('A.Company_id' => $Company_id, 'A.Parent_menu_id' => $Menu_id, 'Enrollment_id' => $Enrollment_id, 'A.Menu_level' => '1', 'Active_flag' => '1'));
		$this->db->order_by('A.Menu_id','ASC');
        $query = $this->db->get();
		// echo $this->db->last_query();
        if ($query->num_rows() > 0)
	...
	public function get_user_assigned_level_2_menu($Company_id,$Enrollment_id,$Menu_id)
	{
		$this->db->select("A.Menu_id,A.Menu_level,A.Parent_menu_id,A.Menu_name,A.Menu_href,A.Company_id");
		$this->db->from('igain_company_menu_master as A');
		$this->db->join('igain_menu_assign as B','A.Menu_id=B.Menu_id AND A.Company_id=B.Company_id');
		
		$this->db->where(array('A.Company_id' => $Company_id, 'A.Parent_menu_id' => $Menu_id, 'Enrollment_id' => $Enrollment_id, 'A.Menu_level' => '2', 'Active_flag' => '1'));
		$this->db->order_by('A.Menu_id','ASC');
        $query = $this->db->get();
		// echo $this->db->last_query();
        if ($query->num_rows() > 0)
