...
class Survey_model extends CI_Model 
{
	...
	public function insert_survey_multiple_choices($Company_id,$create_user)
	{
		...
		$data['Active_flag'] = '1';
		$data['Name_of_option'] = strip_tags($this->input->post('multiple_choice_name'));
		$data['Number_of_option'] = strip_tags($this->input->post('no_of_options'));
    ...
    public function survey_multiple_choices_list($Company_id)
	{
	...
	public function survey_structure_list($Company_id)
	{
		// $this->db->limit($limit,$start);
...
/************************Nilesh Work End*************************/
	...
	{        
		$this->db->select("First_name,Last_name,Phone_no");
		...
		if($query->num_rows() > 0)
		{
			foreach ($query->result_array() as $row)
			{
				$label = htmlentities(stripslashes($row['First_name']))." ".htmlentities(stripslashes($row['Last_name']))." - ".App_string_decrypt($row['Phone_no']);
				$row_set[] = $label;
			}
	...
	function get_survey_nps_type($company_id)
	{
		...
		$this->db->from('igain_nps_type_master');
		// $this->db->where(array('NPS_company_id' =>$company_id,'Active_flag' =>1));
		$this->db->where_in("NPS_type_id",array('1','2','3'));
		$sql = $this->db->get();
		if($sql->num_rows() > 0)
		{
			return $sql->result_array();		
