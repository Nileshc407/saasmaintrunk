...
class Home_model extends CI_model
{
	...
	//	$arrmonth=array('Jan '.$login_year,'Feb '.$login_year,'Mar '.$login_year,'Apr '.$login_year,'May '.$login_year,'Jun '.$login_year,'Jul '.$login_year,'Aug '.$login_year,'Sep '.$login_year,'Oct '.$login_year,'Nov '.$login_year,'Dec '.$login_year);	
		...
		{
			$start_date = date("Y-m",strtotime($month))."-01 00:00:00";
			$end_date = date("Y-m",strtotime($month))."-31 23:59:59";

			// echo "---start date is--".$start_date."---<br>";
			// echo "---end_date is--".$end_date."---<br>";
			$curr_date = date("M-Y");
			...
			if($result1->num_rows() > 0)
			{
				...
				*/
				...
				$Total_redeem_points = $Total_just_redeem_points + $Total_merchandize_redeem_points;
					...
					$this->db->from("igain_transaction");
					$this->db->where(array("Company_id" => $company_id, "Trans_type" => '2'));
					$this->db->where("Trans_date BETWEEN '".$start_date."' AND '".$end_date."' ");
	...
	{ 
		...
		$this->db->where(array("IT.Company_id" => $company_id, "Trans_type !=" => '4', "Trans_type !=" => '26'));
	...
	public function get_company_summary_cancel_transactions($company_id)
	{ 
		
		$this->db->select('SUM(Redeem_points) AS Total_Credited_Redeem_points,(SUM(Loyalty_pts)) as Total_Debited_Points,SUM(Purchase_amount) as Total_Cancellation_Amount');
		//,IE.Current_balance,Card_id2 as Transfer_to
		$this->db->from('igain_transaction as IT');
		$this->db->where(array("IT.Company_id" => $company_id,  "Trans_type =" => '26'));
		$this->db->group_by('IT.Company_id');
		// $this->db->where('IT.Company_id' , $company_id);
