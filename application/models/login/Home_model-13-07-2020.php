...
class Home_model extends CI_model
{
	...
	{	
		...
		$Trans_type = array('2','3','7','8','9','10','12','13','15');
			...
			$this->db->select("B.First_name, B.Last_name, B.User_email_id, B.Phone_no,B.Card_id,MAX(A.Trans_date) AS Last_visit,B.Photograph");
	...
	{	
		...
		$this->db->select('First_name,Last_name,User_email_id,Phone_no,B.Card_id,Photograph,Trans_date');
		...
		if($query1->num_rows() > 0)
		{
			foreach ($query1->result_array() as $row11)
			{
				...
				$Customer_name[$i] = $row11['First_name']." ".$row11['Last_name'];		
	...
	public function get_company_summary_transactions($company_id)
	{
		...
		$this->db->select('(SUM(Redeem_points)+SUM(Transfer_points)) AS Total_Used,(SUM(Loyalty_pts)+SUM(Topup_amount)) as Total_Gained_Points,SUM(Topup_amount) as Total_Bonus_Points,SUM(Purchase_amount) as Total_Purchase_Amount,SUM(Transfer_points) AS Total_Transfer_Points,SUM(Expired_points) AS Total_Expired_points');
	...
	public function get_popular_category($company_id)
	{
		// $this->db->limit(6);
	...
	/*----------------------------------13-11-2019 Transaction  Order Types  Graph ------------------------------*/	
		...
		{
			$this->db->select("DISTINCT(smry_month) AS smry_month,Trans_count_pos,Trans_count_online,Total_purchase_pos,Total_purchase_online");
			$this->db->from("igain_pos_online_distribution_graph");
			$this->db->where(array("Company_id" => $company_id));
			// $this->db->order_by("smry_id DESC");
			$result1 = $this->db->get();
	...
	/******************Nilesh start 19-06-2020***************************************/
	public function Count_issued_vouchers($company_id)
	{
		$this->db->select('*');
		$this->db->from("igain_giftcard_tbl");
		$this->db->where(array("Company_id" => $company_id,"Payment_Type_id" => 99 ));
		$result2 = $this->db->get();
		return $result2->num_rows();
	}	
	public function Count_used_vouchers($company_id)
	{
		$this->db->select('*');
		$this->db->from("igain_giftcard_tbl");
		$this->db->where(array("Company_id" => $company_id,"Card_balance" => 0,"Payment_Type_id" => 99 ));
		$result3 = $this->db->get();
		return $result3->num_rows();
	}	
	
	public function get_total_vouchers_issued_value($company_id)
	{	
	...
	public function get_total_vouchers_used_value($company_id)
	{	
