...
Class Tier_model extends CI_Model
{
	...
	public function insert_tier($Company_id)
	{
		...
		if($this->input->post("Tier_Business_flag")==1){$Tierlevel=$this->input->post("BTierlevel");}
		if($this->input->post("Tier_Business_flag")==0){$Tierlevel=$this->input->post("ITierlevel");}
		$tier_data = array(
		'Company_id' => $Company_id,
		'Tier_name' => $this->input->post("Tiername"),
		'Tier_level_id' => $Tierlevel,
		...
		'Tier_Business_flag' => $this->input->post("Tier_Business_flag"),
		'Tier_redemption_ratio' => $this->input->post("Tier_redemption_ratio"),
		'Create_date' => $today,
		'Active_flag' => 1
		);
	...
	public function get_tier_levels($Company_id,$Tier_Business_flag)
	{
		$this->db->select('Tier_id,Tier_level_id,Tier_Business_flag');
		$this->db->from('igain_tier_master');
		$this->db->where(array('Company_id'=>$Company_id,'Tier_Business_flag'=>$Tier_Business_flag));
		$query13 = $this->db->get();
