...
{	 
	...
	{
		// $Todays_date1=date("Y-m-d 00:00:00");
		// $Todays_date2=date("Y-m-d 23:59:59");
	...
	function Get_voucher_type_code_decode_master($Company_id)
	{
		...
		$this->db->where(array('Company_id'=>1,'Code_decode_type_id'=>21));
		$sql=$this->db->get();
		//echo $this->db->last_query();
		if($sql->num_rows()>0)
		{
	...
	public function insert_send_giftcard_vouchers($Post_vouchers_data)
	{
		$this->load->model('Igain_model');
		/* echo"<br>----insert_send_giftcard_vouchers-------<br>";
		var_dump($Post_vouchers_data); */
		
		$Post_vouchers_data["Enrollement_id"];
		$Payment_Type_id='0';
		
		/* Revenue Voucher */
		if($Post_vouchers_data["Voucher_type"] ==1 && $Post_vouchers_data["Voucher_issuance_type"] == 0 ){
			/* Value */
			$Payment_Type_id='998';
			$Card_value= $Post_vouchers_data["Cost_price"];
			$Card_balance = $Post_vouchers_data["Cost_price"];
			
		}		
		/* Product Voucher */
		if($Post_vouchers_data["Voucher_type"] ==2 && $Post_vouchers_data["Voucher_issuance_type"] == 0 ){
			/* Percentage */
			/*  $Payment_Type_id='996'; */
			$Payment_Type_id='997';
			
			/* $Cost_price= $Post_vouchers_data["Discount_percentage"];
			$Card_balance = $Post_vouchers_data["Discount_percentage"];  */
		...
		} else if($Post_vouchers_data["Voucher_type"] ==2 && $Post_vouchers_data["Voucher_issuance_type"] == 1 ){
			/* Percentage */
			/*  $Payment_Type_id='996'; */
			$Payment_Type_id='997';
		...
		} else if($Post_vouchers_data["Voucher_type"] ==2 && $Post_vouchers_data["Voucher_issuance_type"] == 2 ){
		...
		/* Discount Voucher */
		if($Post_vouchers_data["Voucher_type"] ==3 && $Post_vouchers_data["Voucher_issuance_type"] == 1 ){
			/* Percentage */
			/* $Payment_Type_id='999'; */
			$Payment_Type_id='99';
		...
		} else if($Post_vouchers_data["Voucher_type"] ==3 && $Post_vouchers_data["Voucher_issuance_type"] == 2){
		...
		/* echo"----Card_balance-------".$Card_balance."----<br>";
		echo"----Card_value-------".$Card_value."----<br>"; */
		
		// die;
		
		
		
		$get_cust_detail = $this->Igain_model->get_enrollment_details($Post_vouchers_data["Enrollement_id"]);
		$Enrollement_id = $get_cust_detail->Enrollement_id;	
		$Email = $get_cust_detail->User_email_id;	
		$Phone_no = $get_cust_detail->Phone_no;	
		$Card_id = $get_cust_detail->Card_id;	
		
		$User_name=$get_cust_detail->First_name.' '.$get_cust_detail->Last_name;
			
		$data['Company_id'] = $Post_vouchers_data["Company_id"];
		$data['Gift_card_id'] = $Post_vouchers_data["Voucher_code"];
		$data['Valid_till'] = $Post_vouchers_data["Valid_till"];
		$data['Card_value'] = $Card_value;
		$data['Card_balance'] = $Card_balance;
		$data['Discount_percentage'] = $Post_vouchers_data["Discount_percentage"];
		...
		$data['Cheque_no'] = 0;
		$data['Seller_id'] = $Post_vouchers_data["Create_User_id"];
		...
		$this->db->insert('igain_giftcard_tbl',$data);
		 // echo"----igain_giftcard_tbl-------".$this->db->last_query()."----<br>";
		 // die;
		if($this->db->affected_rows()>0)
		{
			return $this->db->insert_id();
