...
class Administration_model extends CI_Model {
   ...
   public function loyalty_rule_count($Company_id,$Logged_user_enrollid,$Logged_user_id)
	...
	public function insert_loyalty_rule($Company_id)
	{
		...
		$data['Tier_id'] = strip_tags($this->input->post("member_tier_id"));
		$Rule_for = strip_tags($this->input->post("Rule_for"));
		...
		{
			$data['Channel_flag']=1;
			$data['Trans_Channel']=strip_tags($this->input->post("Trans_Channel"));
		}
		if($Rule_for == 5)
		{
			$data['Payment_flag']=1;
			$data['Payment_Type_id']=strip_tags($this->input->post("Payment_Type_id"));
		}
			
		if($this->input->post("seller_id") == 0)
		{
			$get_sellers = $this->Igain_model->get_company_sellers_details($Company_id);
			...
			{
				$data['Loyalty_at_transaction'] = strip_tags($this->input->post("gained_points"));
				...
				foreach($get_sellers as $sellers)
				{
					...
					if($Rule_for ==1)
					{
						...
						{
							$CategoryDetails = $this->Administration_model->Get_Category_Name($Category,$Company_id);
							...
							'Active_flag' => 1,
							'Loyalty_name' => $loyalty_rule_setup."-".strip_tags($this->input->post("LPName"))."(".$Category_name.")",
							...
							'Loyalty_at_transaction' => strip_tags($this->input->post("gained_points")),
			...
			{
				$Purchase_value = strip_tags($this->input->post("Purchase_value")); 
				$Issue_points = strip_tags($this->input->post("Issue_points"));				
		...
		{			
			$data['Seller'] = strip_tags($this->input->post("seller_id"));
			if($Rule_for == 1)
			{
				$data['Category_flag']=1;
				$data['Category_id']=strip_tags($this->input->post("Category_id")); 
			}
			if($Rule_for == 2)
			{
				$data['Segment_flag']=1;
				$data['Segment_id']=strip_tags($this->input->post("Segment_id"));
			}
			...
			/**************Create multiple category rule*************/
				if($Rule_for ==1)
				{
					foreach($Category_id as $Category)
					{
						...
						$data121 = array(
						'Company_id' => $Company_id,
						'Seller' => strip_tags($this->input->post("seller_id")),
						'Active_flag' => 1,
						'Loyalty_name' => $loyalty_rule_setup."-".strip_tags($this->input->post("LPName"))."(".$Category_name.")",
						'Tier_id' => $this->input->post("member_tier_id"),
						'From_date' => date("Y-m-d",strtotime($this->input->post("start_date"))),
	...
	public function update_loyalty_rule($Company_id)
	{
		$Purchase_value = array();
		$Issue_points = array();
		
		$Loyalty_id = App_string_decrypt($this->input->post("Loyalty_id"));
		...
		$loyalty_rule_setup = strip_tags($this->input->post("loyalty_rule_setup"));
		$data['Loyalty_name'] = $loyalty_rule_setup."-".strip_tags($this->input->post("LPName"));
		$LP_name = strip_tags($this->input->post("LPName"));
		$customer_gain = strip_tags($this->input->post("customer_gain"));
		$data['From_date'] = date("Y-m-d",strtotime($this->input->post("start_date")));
		$data['Till_date'] = date("Y-m-d",strtotime($this->input->post("end_date")));
		$data['Tier_id'] = strip_tags($this->input->post("member_tier_id"));
   ...
   public function insert_promo_campaign($Company_id)   
	 ...
	 public function upload_promo_campaign_file($filepath,$filename,$Company_id)
	{
		...
		$data['Company_id'] = $Company_id;
		$data['Active_flag'] = 1;
		$data['Pomo_campaign_name'] = strip_tags($this->input->post("CMPName"));
		$campaign_name = strip_tags($this->input->post("CMPName"));
  ...
  /*************************************sandeep END *******************************/
//***********************************************Akshay Starts************************************
	function insert_communication($filepath,$Company_id) 
    {  
		$entry_date = date('Y-m-d');
		$sellerID = strip_tags($this->input->post('sellerId'));
		$data['communication_plan'] = strip_tags($this->input->post('offer'));
		$Hobbie_id=strip_tags($this->input->post('Offer_related_to'));
		if($Hobbie_id!=NULL)
		{
			$Hobbie_id=$Hobbie_id;
		}
		...
		$data['description'] = strip_tags($this->input->post('offerdetails'));
		...
		/**************Voucher Link Comminucation 26-02-2020--Ravi***************/
			$data['Link_to_voucher'] = strip_tags($this->input->post('Link_to_voucher'));
			if($data['Link_to_voucher'] != 0){
				$data['Voucher_type'] = strip_tags($this->input->post('Voucher_type'));
		...
		/**************Birthday Communication 09-03-2020--Ravi***************/
			$data['Birthday_flag'] = strip_tags($this->input->post('Birthday_flag'));
		...
		/**************Birthday Communication 09-03-2020--Ravi***************/
			...
			if($this->input->post('Schedule_flag')==1) {
				...
				$data['Schedule_flag'] = strip_tags($this->input->post('Schedule_flag'));
				if($this->input->post('Schedule_flag')=='1')
				{
					$data['Link_to_lbs']='0';
				}
				...
				$data['Comm_type'] = $this->input->post('Member_radio');
				$data['Employee_flag'] = $this->input->post('Employee_flag');
				$data['Parent_id'] = $this->input->post('Parent_id');
				// $data['Tier_id'] = $this->input->post('mailtotier');
				$data['Segment_code'] = $this->input->post('Segment_code');
				
				if($_REQUEST['mailtotier']){$data['Tier_id'] = $_REQUEST['mailtotier']; }
				if($_REQUEST['IndTier']){$data['Tier_id'] = $_REQUEST['IndTier']; }
				
			}
				// echo $this->input->post('Member_radio');die;
				...
				if($this->input->post('Member_radio') == 5) // Tier Based Members
				{
					$data['Schedule_comm_value'] = $this->input->post('mailtotier');
				}
				if($this->input->post('Member_radio') == 7) // Segment Based Members
				{
			...
			/* Ravi-----01-06-2022 */
				...
				$radio2 = $this->input->post("r2");			//****send Communication for.	r2
				...
				else
				{
					...
					{	
						...
						if($Schedule_criteria==1) //Weekly
						{
							...
							{
								// echo '<br>schedule_date'.count($schedule_date).'<br>';print_r($Tier_Members);die;
								foreach($Tier_Members as $Tier_Members)
								{
									for($k=0;$k<count($schedule_date);$k++)
									{
										// echo '<br>schedule_date'.$schedule_date[$k].'----Tier_Members'.$Tier_Members;										
											$data1['Comm_id']=$comm_id;
											$data1['Company_id']=$compid;												
											$data1['Enrollment_id']=$Tier_Members;
											// $data1['User_email_id']=$customer_details->User_email_id;
											$data1['Schedule_date']=$schedule_date[$k];
											$data1['Sent_flag']='0';
											$this->db->insert('igain_communication_schedule', $data1);
						...
						if($Schedule_criteria==2) //Monthly
						{
							$startDate=$from_date;
							$endDate=$to_date;
							while(strtotime($startDate) <= strtotime($endDate)) 
					...
					if($this->input->post('Member_radio') == 7) //**segments
					{
						...
						if($Schedule_criteria==1) //Weekly
						{
							...
							{					
								...
								// $all_customers = $this->Igain_model->get_all_customers($compid);	
								$all_customers = $this->Get_dmcc_enrollment_list($User_id=1, $Business_flag,$Employee_flag,$compid);	
								foreach ($all_customers as $row)
								{
									$Applicable_array[]=0;
									...
									foreach($Get_segments2 as $Get_segments)
									{
										...
										if($Get_segments->Segment_type_id==10)//Cumulative Points Accumulated
										{
											...
											$Trans_Records = $this->Igain_model->get_cust_trans_details_segment($compid,$row["Enrollement_id"],$start_date,$end_date,$transaction_type_id,$Tier_id,'','');
					...
					if($this->input->post('Member_radio') == 3 || $this->input->post('Member_radio') == 4) //****for key/worry cust
					{
						...
						if($Schedule_criteria==1) //Weekly
						{
							...
							if($_REQUEST['key_worry_cust'] != NULL)
							{
								// echo '<br>schedule_date'.count($schedule_date).'<br>';print_r($Tier_Members);die;
								foreach($_REQUEST['key_worry_cust'] as $key_worry_cust)
								{
									for($k=0;$k<count($schedule_date);$k++)
									{
										// echo '<br>schedule_date'.$schedule_date[$k].'----Tier_Members'.$Tier_Members;										
											$data1['Comm_id']=$comm_id;
											$data1['Company_id']=$compid;												
											$data1['Enrollment_id']=$key_worry_cust;
											// $data1['User_email_id']=$customer_details->User_email_id;
											$data1['Schedule_date']=$schedule_date[$k];
											$data1['Sent_flag']='0';
											$this->db->insert('igain_communication_schedule', $data1);
	...
	public function update_communication_offer($filepath)
	{
		...
		$sellerID = $this->input->post('sellerId');
		$communication_id = App_string_decrypt($this->input->post('communication_id'));
		...
		/***************Update Comminucation--05-06-2017-Ravi***************/		
			...
			if($data['Schedule_flag'] != '0')
			{
				...
				else
				{
					...
					$offerid = $this->input->post("activeoffer");
					$comm_id= App_string_decrypt($this->input->post('communication_id'));
					$entry_date=date("Y-m-d");	
					$acitvate = '0';					
					if($this->input->post('Member_radio') == 1) //**single customer
					{
					...
					// echo '<br>';print_r($Tier_Members);die;
					// $Employee_flag=0;
					
					/* if($_REQUEST['Member_radio']==3){$Business_flag=1;$data['Tier_id'] = $_REQUEST['mailtotier'];
					if($_REQUEST['Business_Users']!=NULL){$Tier_Members=$_REQUEST['Business_Users'];}//print_r($Tier_Members);
					}//Business
					if($_REQUEST['Member_radio']==4){$Business_flag=0;$Employee_flag=$_REQUEST['Employee_flag'];$data['Tier_id'] = $_REQUEST['IndTier'];
					if($_REQUEST['Employee']!=NULL){$Tier_Members=$_REQUEST['Employee'];}//echo '<br>';print_r($Tier_Members);
					if($_REQUEST['Residents']!=NULL){$Tier_Members=$_REQUEST['Residents'];}//echo '<br>';print_r($Tier_Members);
					} */
	...
	function get_communication_offers($Seller_id,$Company_id,$Comm_type)
	{
		...
		if($Seller_id == 0)
		{
			...
			$this->db->where(array('activate' => 'yes' ));
		...
		else
		{
			$this->db->select("id,communication_plan");
			$this->db->from('igain_seller_communication');
			$this->db->where(array('seller_id' => $Seller_id,'activate' => 'yes'));
			$this->db->where('Till_date > ',$dateIS);
			$sql = $this->db->get();
		}
	...
	function get_key_worry_customers($Seller_id,$Company_id,$r2Value,$mailtokey,$Business_flag,$Parent_id)
	{
		$this->load->model('igain_model');
		// $seller_details = $this->igain_model->get_enrollment_details($Seller_id);
		if($r2Value == 3)
		{
			$key_worry_customers = $this->get_key_customers($mailtokey,$Company_id,$Business_flag,$Parent_id);
		}
		else
		{
			$key_worry_customers = $this->get_worry_customers($mailtokey,$Company_id,$Business_flag,$Parent_id);
		}
		return	$key_worry_customers;
	}
	
	function get_key_customers($mailtokey,$Company_id,$Business_flag,$Parent_id)
	{
		...
		$thismonth = date("Y-m-d");
				...
				$this->db->select('Enrollement_id');
				$this->db->where(array('User_id' => '1', 'Company_id' => $Company_id,'Business_flag' => $Business_flag, 'User_activated' => '1'));
				if($Parent_id!=0){$this->db->where(array('Parent_id' => $Parent_id));}
				$query = $this->db->get('igain_enrollment_master');
				// echo $this->db->last_query()."<br>";
				if($query->num_rows() > 0)
				{			
	...
	function get_worry_customers($mailtokey,$Company_id,$Business_flag,$Parent_id)
	{
		$i = 0;  $custenroll_ids = array();  $cust_array1 = array();
		...
		$this->db->select('Enrollement_id');
		$this->db->where(array('User_id' => '1','Business_flag' => $Business_flag, 'Company_id' => $Company_id, 'User_activated' => '1', 'joined_date <' => $lastmonth));
		if($Parent_id!=0){$this->db->where(array('Parent_id' => $Parent_id));}
		$query = $this->db->get('igain_enrollment_master');
		 // echo $this->db->last_query()."<br>";
		if($query->num_rows() > 0)
	...
	function get_all_tiers($compID,$Tier_Business_flag)
	{
		$this->db->select('Tier_id,Tier_name,Tier_Business_flag');
		$this->db->from('igain_tier_master');
		$this->db->where(array('Company_id'=> $compID,'Tier_Business_flag'=> $Tier_Business_flag,'Active_flag'=> 1));
	...
	function get_tiers_customers($Tier_id,$Company_id,$Business_flag,$Employee_flag)
	{
		
			
		$this->db->select('Enrollement_id,First_name,Last_name');
		$this->db->from('igain_enrollment_master as e');
		$this->db->join('igain_tier_master as TM','TM.Tier_id = e.Tier_id');
		$this->db->where(array('User_id' => '1', 'e.Company_id' => $Company_id,  'Business_flag' => $Business_flag,  'Employee_flag' => $Employee_flag,   'User_activated' => '1', 'TM.Active_flag' => '1'));
		if($Tier_id!=0){$this->db->where(array('e.Tier_id' => $Tier_id));}
		$query = $this->db->get();
		// echo $this->db->last_query()."<br>";
		 if ($query->num_rows() > 0)
		{
	...
	/***********************************Auction Starts******************************/
	function insert_auction($filepath,$Company_id,$Logged_user_enrollid)
    {
		...
		$data['Auction_name'] = strip_tags($this->input->post('auction_name'));
		$data['Prize'] = strip_tags($this->input->post('prize'));
		$data['Min_bid_value'] = strip_tags($this->input->post('minpointstobid'));
		$data['Min_increment'] = strip_tags($this->input->post('minincrement'));
		$data['Prize_description'] = strip_tags($this->input->post('description'));
		$data['Trigger_notification_start_days'] = strip_tags($this->input->post('Trigger_notification_start_days'));
		$data['Trigger_notification_end_days'] = strip_tags($this->input->post('Trigger_notification_end_days'));
	...
	public function update_auction($filepath,$Auction_id)
	{
		$data['From_date'] = date("Y-m-d",strtotime($this->input->post("startDate")));
		$data['To_date'] = date("Y-m-d",strtotime($this->input->post("endDate")));		
		$data['End_time'] = strip_tags($this->input->post('THH')).":".strip_tags($this->input->post('TMM')).":00";		
		$data['Auction_name'] = strip_tags($this->input->post('auction_name'));
		$data['Prize'] = strip_tags($this->input->post('prize'));
		$data['Min_bid_value'] = strip_tags($this->input->post('minpointstobid'));
		$data['Min_increment'] = strip_tags($this->input->post('minincrement'));
		...
		$data['Trigger_notification_start_days'] = strip_tags($this->input->post('Trigger_notification_start_days'));
		$data['Trigger_notification_end_days'] = strip_tags($this->input->post('Trigger_notification_end_days'));
		$data['Update_user_id'] = App_string_decrypt($this->input->post('Update_user_id'));
	...
	public function auction_winners_list($Company_id,$Max_Bid_value)
	{
		...
		$this->db->where(array('B.Winner_flag' => '0', 'B.Active_flag' => '0', 'A.Active_flag' => '1', 'A.To_date <=' => $today,'A.End_time <' => $todayTime,'B.Company_id' => $Company_id, 'B.Bid_value' => $Max_Bid_value));
		$query = $this->db->get();
		// echo $this->db->last_query()."<br><br>";
...
/************************************************AMIT STARTUP*****************************************/	
   ...
   public function referral_rule_count($Company_id)
	...
	function update_referral_rule($Company_id,$refid,$data1)
    {		
		...
		$this->db->update('igain_seller_refrencerule', $data1);
		if($this->db->affected_rows() > 0)
		{
			return true;
		}
...
/********************************************************AMIT 03-05-2017 *******************/	
	...
	public function get_seller_balance($Seller_id,$Company_id)
  {
	...
	function insert_special_offer_images($Post_data)
	{
		$this->db->insert('igain_brand_offer_images',$Post_data);
		if($this->db->affected_rows()>0)
		{
			return $this->db->insert_id();
		}
	...
	function Get_special_offer_images($Company_id)
	{
		$this->db->select('Spl_offer_id,Brand_id,Sequence,Spl_Image,First_name,Last_name');
		$this->db->from('igain_brand_offer_images as o');
		$this->db->join('igain_enrollment_master as e','e.Enrollement_id=o.Brand_id');
		$this->db->where('o.Company_id',$Company_id);
		$this->db->group_by('Brand_id');
		$this->db->order_by('Spl_offer_id','desc');
		$sql=$this->db->get();
		 // echo "<br><br>Get_special_offer_images<br>".$this->db->last_query();//die;
		if($sql->num_rows()>0)
		{
			foreach ($sql->result() as $row)
			{
	...
	function Get_brands_special_images_sequence($Brand_id,$Company_id)
	{
		$this->db->select('Spl_offer_id,Sequence,Spl_Image,Brand_id');
		$this->db->from('igain_brand_offer_images as o');
		$this->db->where('o.Company_id',$Company_id);
		$this->db->where('o.Brand_id',$Brand_id);
	...
	function Update_brands_special_images_sequence($id,$Update_data)
	{
		
		$this->db->where(array('Spl_offer_id' => $id));
		$this->db->update("igain_brand_offer_images", $Update_data);
		// echo "<br><br><br>".$this->db->last_query();//die;
		if($this->db->affected_rows() > 0)
		{
			return true;
	...
	function delete_special_offer_images($Brand_id)
	{
		$this->db->where(array('Brand_id'=>$Brand_id));
		$this->db->delete("igain_brand_offer_images");
		// echo "<br><br>".$this->db->last_query();
		if($this->db->affected_rows() > 0)
		{
			return true;
	...
	function delete_single_special_offer_images($Spl_offer_id)
	{
		$this->db->where(array('Spl_offer_id'=>$Spl_offer_id));
		$this->db->delete("igain_brand_offer_images");
		// echo "<br><br>".$this->db->last_query();
		if($this->db->affected_rows() > 0)
		{
	...
	public function Get_dmcc_enrollment_list($User_id,$Business_flag,$Employee_flag,$Company_id)
	{
		$this->db->select('*');
		$this->db->from('igain_enrollment_master as A');
		$this->db->join('igain_tier_master as B','A.Tier_id=B.Tier_id');
		
		$this->db->where(array('User_id'=> $User_id,'Business_flag'=> $Business_flag,'Employee_flag'=> $Employee_flag,'A.Company_id'=> $Company_id,'A.User_activated'=> 1,'B.Active_flag'=> 1));
   	...
   	public function Fetch_schedule_communications_specific($Comm_id)
	{
		$todays=date('Y-m-d');
		$this->db->select('B.Enrollment_id,First_name,Last_name');
		$this->db->from('igain_seller_communication as A');
		$this->db->join('igain_communication_schedule as B', 'B.Comm_id = A.id');
		$this->db->join('igain_enrollment_master as E', 'B.Enrollment_id = E.Enrollement_id');
		$this->db->where(array('B.Comm_id' => $Comm_id));
