...
$_SESSION["Edit_Sub_seller_admin"]=$results->Sub_seller_admin;
	...
	<div class="content-box">
		<div class="row">
			<div class="col-sm-12">
				...
				echo form_open_multipart('Enrollmentc/Update_OutletUserEnrollment/',$attributes); ?>
				<div class="element-wrapper">
					<h6 class="element-header">EDIT OUTLET USER ENROLLMENT</h6>
							...
							<div class="col-sm-6" >
								...
								<div class="form-group" >
									...
									<label class="radio-inline">
										...
										<input type="radio" disabled name="SubSeller_Admin1"  checked  id="inlineRadio1" value="1">Yes
										
										<input type="radio" disabled name="SubSeller_Admin1"  id="inlineRadio2" value="0">No
										<?php } else { ?>
										<input type="radio" disabled name="SubSeller_Admin1"   id="inlineRadio1" value="1">Yes
										
										<input type="radio" disabled name="SubSeller_Admin1" checked id="inlineRadio2" value="0">No
										<?php } ?>
									</label>
								</div>
							...
							?>
								<tr>
									<td class="row-actions">
										<?php if($_SESSION['Edit_Privileges_Edit_flag']==1){ ?>
										<a href="<?php echo base_url()?>index.php/Enrollmentc/Edit_OutletUserEnrollment/?Enrollement_id=<?php echo $Encrypt_Enrollement_id;?>" title="Edit"><i class="os-icon os-icon-ui-49"></i></a>
										<?php } if($_SESSION['Edit_Privileges_Delete_flag']==1){ ?>
										<a class="danger" href="javascript:void(0);" onclick="delete_me('<?php echo $Encrypt_Enrollement_id;?>','<?php echo $row->First_name.' '.$row->Last_name; ?>','<?php if($row->User_id==1 && $row->Parent_flag==1){echo 'Please Note that the Subsidiary & Employee linked to this Parent Member will get Inactive also';}?>','Enrollmentc/delete_Outletenrollment/?Enrollement_id');" data-target="#deleteModal" data-toggle="modal" title="Delete"><i class="os-icon os-icon-ui-15"></i></a>
										<?php } ?>
									</td>
									<td><?php echo $row->First_name.' '.$row->Last_name;?></td>
									<td><?php echo App_string_decrypt($row->Phone_no);?></td>
									<td><?php echo App_string_decrypt($row->User_email_id);?></td>
									<td><?php if($row->Sub_seller_admin==1){echo 'Yes';}else{echo 'No';}?></td>
								</tr>
					<?php 		}
							}	?>
							</tbody>
...
<script>
var Sub_seller_Enrollement_id = '<?php echo $results->Sub_seller_Enrollement_id; ?>';
if(Sub_seller_Enrollement_id == 0){$("#Supervisor_list_div1").hide();$("#Sub_seller_Enrollement_id1").removeAttr("required");}
...
$(document).ready(function()
{
	...
	$('#userEmailId').blur(function()
	{
		if( $("#userEmailId").val() != "" )
		{
			...
			$.ajax({
				  type: "POST",
				  data: {userEmailId: userEmailId, Company_id: Company_id, userId:2},
