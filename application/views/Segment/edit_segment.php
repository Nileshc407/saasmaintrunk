						...
						<?php
								...
								if($segment_edit_results != NULL)
								{
									...
									{	
										...
										if($Segment_type_id!=13)//Except Item Category
										{
											...
											if($Segment_type_id==17)//Tier
											{
												$Get_tier = $ci_object->Igain_model->get_tier_detail($row->Value);
												if($Get_tier != NULL)
												{
													$segment=$row->Segment_type_name." ".$row->Operator." '".$Get_tier->Tier_name."'";
												}
											}
											
											if($Segment_type_id==16)//Outlet
											{
												$Get_outlet = $ci_object->Igain_model->get_seller_details($row->Value);
												if($Get_outlet != NULL)
												{
													foreach($Get_outlet as $Get_outlet){
													$segment=$row->Segment_type_name." ".$row->Operator." '".$Get_outlet->First_name.' '.$Get_outlet->Last_name."'";
													}
												}
											}
											?>
											<tr>
												<td class="row-actions">
													<a href="javascript:void(0);" onclick="delete_segment_id('<?php echo $row->Segment_id;?>','<?php echo $row->Segment_code;?>','<?php echo $row->Segment_type_name;?>','<?php echo $row->Company_id;?>');" class="danger" title="Delete">
										...
										if($Item_Category_flag==1)//Item Category
										{
											...
											if($Get_linked_segment_items != NULL)
											{
												...
												{
													$Segment_operation=" Item Name = '".$row1->Merchandize_item_name."' AND Quantity = ".$row1->Quantity;
													?>
														<tr>
															<td class="row-actions">
																<a href="javascript:void(0);" onclick="delete_segment_child('<?php echo $row1->Segment_child_id;?>','<?php echo $row1->Merchandize_item_name;?>');" class="danger" title="Delete">
																	<i class="os-icon os-icon-ui-15"></i>
																</a>
															</td>
															<td><?php echo $row->Segment_code;?></td>
															...
															<td><?php echo $Segment_operation;  ?></td>
														</tr>
													<?php
...
/* 	function Change_operator2(Segment_type,id)
	...
	function delete_segment_child(Segment_child_id,Segmentname)
	{	
		 
		delete_me(Segment_child_id,Segmentname,'Segment Criteria','SegmentC/delete_segment_child/?Segment_child_id');
