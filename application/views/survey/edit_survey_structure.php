	...
	<div class="content-box">
		<div class="row">
			...
			<?php
				...
				echo form_open_multipart('Survey/update_surveystructure',$attributes); 
			?>
				<div class="element-wrapper">
					<h6 class="element-header">EDIT SURVEY STRUCTURE MASTER
					</h6>
				...
				<?php 	} ?>
						<div class="row">
							<div class="col-sm-6">		
							  <div class="form-group">
								...
								<input type="text" class="form-control" name="survey_name" value="<?php echo $Survey_details->Survey_name ?>" id="survey_name" data-error="Please enter name of survey" placeholder="Enter name of survey" readonly required="required">
							   ...
							   <div class="form-group">
								...
								<select class="form-control" name="Type_of_survey" id="Type_of_survey" data-error="Please select survey type" required="required" readonly>
									<option value="<?php echo $Survey_details->Survey_type; 
									?>"><?php if($Survey_details->Survey_type== 1){ echo 'Feedback Survey'; } else if($Survey_details->Survey_type== 2){ echo 'Service Related Survey';} else{ echo 'Product Survey'; } ?>
								</select>
							  ...
							  <div class="form-group">
								...
								<input type="text" class="form-control" name="no_of_questions" id="no_of_questions" value="<?php echo $Survey_details->No_of_questions ?>" data-error="Please enter number of questions" placeholder="Enter number of questions" onkeyup="this.value=this.value.replace(/\D/g,'')" readonly required="required">
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div> 
							  
							  <div class="form-group">
								<label for=""><span class="required_info">* </span> Number Of Multiple Choice Questions</label>
								<input type="text" class="form-control" name="multiple_choice" id="multiple_choice" data-error="Please enter number of multiple choice questions" placeholder="Enter number of multiple choice questions" onkeyup="this.value=this.value.replace(/\D/g,'')" onchange="javascript:cal_text_no(this.value)" value="<?php echo $Survey_details->No_of_multiple_choice ?>"  readonly required="required">
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div>
							 
							 <div class="form-group ">
								<label for=""><span class="required_info">* </span> Number Of Text Based Questions</label>
								<input type="text" name="text_based" id="text_based" class="form-control" placeholder="Number of text based questions" readonly="readonly" onkeyup="this.value=this.value.replace(/\D/g,'')" value="<?php echo $Survey_details->No_of_text_based ?>" readonly required="required" />
							 </div>									  
							</div>											
							<div class="col-sm-6">
							  <div class="form-group">
							   <label for=""><span class="required_info">* </span>Valid From <span class="required_info">(* click inside textbox)</span></label>
								<input class="single-daterange form-control" placeholder="Start Date" type="text"  name="Valid_from" id="datepicker1" value="<?php echo date("m/d/Y",strtotime($Survey_details->Start_date)) ?>" data-error="Please select from date"/>
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div>
							  
							  <div class="form-group">
							   <label for=""><span class="required_info">* </span>Valid Till <span class="required_info">(* click inside textbox)</span></label>
								<input class="single-daterange form-control" placeholder="End Date" type="text"  name="Valid_till" id="datepicker2" value="<?php echo date("m/d/Y",strtotime($Survey_details->End_date)) ?>" data-error="Please select till date"/>
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div>
							  ...
							  <div class="form-group">
								...
								<input type="text" class="form-control" name="remind_after_validity" id="remind_after_validity" onkeyup="this.value=this.value.replace(/\D/g,'')" placeholder="Enter reminder notification days after validity starts" data-error="Please enter reminder notification days after validity starts" value="<?php echo $Survey_details->Reminder_days_after ?>" required="required">
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div>
							  
							  <div class="form-group">
								<label for=""><span class="required_info">* </span>Reminder Notification Days Before Validity Ends</label>
								<input type="text" class="form-control" name="remind_before_validity" id="remind_before_validity" onkeyup="this.value=this.value.replace(/\D/g,'')" placeholder="Enter reminder notification days before validity ends" value="<?php echo $Survey_details->Reminder_days_before ?>" data-error="Please enter reminder notification days before validity ends" required="required">
								<div class="help-block form-text with-errors form-control-feedback"></div>
							  </div>
							 ...
							 <div class="form-group">
								...
								<div class="col-sm-8" style=" margin-top:7px;">
								  <div class="form-check" style="float:left;">
									...
									<input class="form-check-input" name="Survey_Reward" id="r2" <?php if( $Survey_details->Survey_rewarded == 1){ echo "checked"; } ?> type="radio" onchange="show_survey(this.value)" value="1" required="required">Yes</label>
								  </div>
								  
								  <div class="form-check" style="margin-left:50px;">
									<label class="form-check-label">
									<input class="form-check-input" name="Survey_Reward" id="r1" type="radio" <?php if( $Survey_details->Survey_rewarded == 0){ echo "checked"; } ?> value="0" onchange="show_survey(this.value)" required="required">No</label>
								  </div> 
								</div>
							  </div>
							  ...
							  <div class="form-group" id="Survey_Reward" style="display:none">
								...
								<input type="text" name="Survey_Reward_Points" id="Survey_Reward_Points" class="form-control"  onkeyup="this.value=this.value.replace(/\D/g,'')" value="<?php if( $Survey_details->Survey_rewarded == 1){ echo $Survey_details->Survey_reward_points; }else{ echo '0'; }?>" placeholder="Enter reward points " />
							...
							<div class="form-group">		
								...
								<br>
								<input type="radio" name="Survey_template" id="STemplate" <?php if( $Survey_details->Survey_template == 1) { echo "checked"; } ?> onchange="JavaScript:show_template(this.value);" value="1" required> Preview Template 1	
								...
								<input type="radio" name="Survey_template" id="STemplate" <?php if( $Survey_details->Survey_template == 3) { echo "checked"; } ?> onchange="JavaScript:show_template(this.value);" value="3" required>Preview Template 2
							</div>
						  </div>
						</div>
					   <div class="form-buttons-w" align="center">
							...
							<input type="hidden" name="Survey_id" value="<?php echo App_string_encrypt($Survey_details->Survey_id) ?>"  class="form-control">
					   </div>
					</div>
				</div>
				<?php echo form_close(); ?>	
						...
						$mv_date=date("Y-m-d");
							if($Survey_structure != NULL)
							{
								foreach($Survey_structure as $row)
								{
									...
									$Encrypt_survey_id = App_string_encrypt($row->Survey_id);
									$Encrypt_survey_id = str_replace('+', 'X', $Encrypt_survey_id);
									$_SESSION[$Encrypt_survey_id]=$row->Survey_id;
							?>
								<tr>
									<td class="row-actions">
										<a href="<?php echo base_url()?>index.php/Survey/editsurveystructure/?Survey_id=<?php echo $Encrypt_survey_id;?>" title="Edit"><i class="os-icon os-icon-ui-49"></i></a>
...
{
	Survey_rewarded= '<?php echo $Survey_details->Survey_rewarded ;?>';
...
$('#Register').click(function()
{
	...
	/* if(document.getElementById('r2').checked== false || document.getElementById('r1').checked== false )
	{
		...
		var msg = "Please Check Survey Reward OR Not!";
		runjs(Title,msg);
		return false; 
	} */
	...
	if(document.getElementById('r2').checked)
	{
		...
		if(rate_value =="")
		{
			...
			/* var Title = "Blank Field Validation";
			var msg = "Please Enter Survey Reward Points!";
			runjs(Title,msg);*/
		}	
	}
	
	/* if(document.getElementByName("r2").value==1)
	{
	...
	if($('#survey_name').val() != "" && $('#Type_of_survey').val() != ""&& $('#no_of_questions').val() != "" && $('#multiple_choice').val() != ""&& $('#text_based').val() != "" && $('#datepicker1').val() != "" && $('#datepicker2').val() != "" && $('#remind_after_validity').val() != "" && $('#remind_before_validity').val() != "")
	{
		show_loader();
	}
	/*
